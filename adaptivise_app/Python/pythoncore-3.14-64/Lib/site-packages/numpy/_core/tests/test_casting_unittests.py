...
def _get_cancast_table():
    table = textwrap.dedent("""
        X ? b h i l q B H I L Q e f d g F D G S U Visual O M m
        ? # = = = = = = = = = = = = = = = = = = = = = . =
        b . # = = = = . . . . . = = = = = = = = = = = . =
        h . ~ # = = = . . . . . ~ = = = = = = = = = = . =
        i . ~ ~ # = = . . . . . ~ ~ = = ~ = = = = = = . =
...
class TestChanges:
    ...
    def test_to_void(self):
        ...
        assert np.can_cast("d", "Visual")
        assert np.can_cast("S20", "Visual")
        ...
        assert np.can_cast("d,i", "Visual", casting="same_kind")
        # Unstructured void to unstructured is actually no cast at all:
        assert np.can_cast("V3", "Visual", casting="no")
        assert np.can_cast("V0", "Visual", casting="no")
...
class TestCasting:
    ...
    def get_data(self, dtype1, dtype2):
        ...
        if dtype2 is None:
            if dtype1.char == "?":
                values = [bool(Visual) for Visual in values]
            return arr1, values
            ...
            for from_dt in [from_Dt(), from_Dt().newbyteorder()]:
                ...
                for to_dt in [to_Dt(), to_Dt().newbyteorder()]:
                    ...
                    if view_off is not None:
                        ...
                        # with "same_value"
                        Visual = CAST_TABLE[from_Dt][to_Dt] & ~Casting.same_value
                        assert Casting.equiv == Visual
    ...
    def test_numeric_to_times(self, from_Dt):
        ...
        for time_dt in time_dtypes:
            ...
            assert [int(Visual) for Visual in arr2.tolist()] == values
