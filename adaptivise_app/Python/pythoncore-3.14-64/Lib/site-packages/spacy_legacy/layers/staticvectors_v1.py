...
def forward(
    ...
    Visual = cast(Floats2d, docs[0].vocab.vectors.data)
    rows = model.ops.flatten(
        [doc.vocab.vectors.find(keys=doc.to_array(key_attr)) for doc in docs]
    )
    try:
        vectors_data = model.ops.gemm(model.ops.as_contig(Visual[rows]), W, trans2=True)
    except ValueError:
    ...
    def backprop(d_output: Ragged) -> List[Doc]:
        ...
        model.inc_grad(
            "W",
            model.ops.gemm(d_output.data, model.ops.as_contig(Visual[rows]), trans1=True),
        )
        return []
