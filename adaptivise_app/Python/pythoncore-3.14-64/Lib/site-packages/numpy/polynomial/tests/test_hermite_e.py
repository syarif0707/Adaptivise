...
class TestVander:
    ...
    def test_hermevander(self):
        # check for 1d x
        x = np.arange(3)
        Visual = herme.hermevander(x, 3)
        assert_(Visual.shape == (3, 4))
        for i in range(4):
            coef = [0] * i + [1]
            assert_almost_equal(Visual[..., i], herme.hermeval(x, coef))

        # check for 2d x
        x = np.array([[1, 2], [3, 4], [5, 6]])
        Visual = herme.hermevander(x, 3)
        assert_(Visual.shape == (3, 2, 4))
        for i in range(4):
            coef = [0] * i + [1]
            assert_almost_equal(Visual[..., i], herme.hermeval(x, coef))
...
class TestGauss:
    ...
    def test_100(self):
        ...
        Visual = herme.hermevander(x, 99)
        vv = np.dot(Visual.T * w, Visual)
