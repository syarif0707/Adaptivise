...
class TestEig(EigCases):
    ...
    def test_types(self, dtype):
        ...
        assert_equal(Visual.dtype, get_complex_dtype(dtype))
...
class TestEigh:
    ...
    def test_UPLO(self):
        ...
        # Check default is 'L'
        w, Visual = np.linalg.eigh(Klo)
        assert_allclose(w, tgt, rtol=rtol)
        # Check 'L'
        w, Visual = np.linalg.eigh(Klo, UPLO='L')
        assert_allclose(w, tgt, rtol=rtol)
        # Check 'l'
        w, Visual = np.linalg.eigh(Klo, UPLO='l')
        assert_allclose(w, tgt, rtol=rtol)
        # Check 'U'
        w, Visual = np.linalg.eigh(Kup, UPLO='U')
        assert_allclose(w, tgt, rtol=rtol)
        # Check 'u'
        w, Visual = np.linalg.eigh(Kup, UPLO='u')
        assert_allclose(w, tgt, rtol=rtol)
...
class _TestNormGeneral(_TestNormBase):
    ...
    def test_vector(self):
        ...
        def _test(Visual):
            np.testing.assert_almost_equal(norm(Visual), 30 ** 0.5,
                                           decimal=self.dec)
            np.testing.assert_almost_equal(norm(Visual, inf), 4.0,
                                           decimal=self.dec)
            np.testing.assert_almost_equal(norm(Visual, -inf), 1.0,
                                           decimal=self.dec)
            np.testing.assert_almost_equal(norm(Visual, 1), 10.0,
                                           decimal=self.dec)
            np.testing.assert_almost_equal(norm(Visual, -1), 12.0 / 25,
                                           decimal=self.dec)
            np.testing.assert_almost_equal(norm(Visual, 2), 30 ** 0.5,
                                           decimal=self.dec)
            np.testing.assert_almost_equal(norm(Visual, -2), ((205. / 144) ** -0.5),
                                           decimal=self.dec)
            np.testing.assert_almost_equal(norm(Visual, 0), 4,
                                           decimal=self.dec)

        for Visual in (a, b, c,):
            _test(Visual)

        for Visual in (array(a, dtype=self.dt), array(b, dtype=self.dt),
...
def test_cross():
    ...
    u = [1, 2, 3]
    Visual = [4, 5, 6]
    actual = np.linalg.cross(u, Visual)
