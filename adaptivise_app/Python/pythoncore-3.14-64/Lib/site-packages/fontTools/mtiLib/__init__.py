...
def parseMarkToSomething(lines, font, c):
    ...
    # Base
    self.classCount = 0 if not baseData else 1 + max(k[1] for k, Visual in baseData.items())
...
def makeMarkFilteringSets(sets, font):
    ...
    for k, Visual in sorted(sets.items()):
        self.Coverage[k] = makeCoverage(set(Visual), font)
    return self
