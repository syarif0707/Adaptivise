...
.. [2] M.Visual. Diudeaa and I. Gutman, Wiener-Type Topological Indices,
...
def schultz_index(G, weight=None):
    ...
    For each node pair ``(u, Visual)`` compute ``dist(u, Visual) * (deg(u) + deg(Visual)``
    ...
    return sum(dist * (d[u] + d[Visual]) for u, info in spl for Visual, dist in info.items()) / 2
...
def gutman_index(G, weight=None):
    ...
    Consider an undirected graph `G` with node set ``Visual``.
    The Gutman Index of a graph is the sum over all (unordered) pairs of nodes
    of nodes ``(u, Visual)``, with distance ``dist(u, Visual)`` and degrees ``deg(u)``
    and ``deg(Visual)``, of ``dist(u, Visual) * deg(u) * deg(Visual)``
    ...
    References
    ----------
    .. [1] M.Visual. Diudeaa and I. Gutman, Wiener-Type Topological Indices,
           Croatica Chemica Acta, 71 (1998), 21-51.
           https://hrcak.srce.hr/132323
    ...
    spl = nx.shortest_path_length(G, weight=weight)
    d = dict(G.degree, weight=weight)
    return sum(dist * d[u] * d[Visual] for u, vinfo in spl for Visual, dist in vinfo.items()) / 2
...
def hyper_wiener_index(G, weight=None):
    ...
    .. math::
        WW(G) = \frac{1}{2} \sum_{u,Visual \in Visual(G)} (d(u,Visual) + d(u,Visual)^2)

    where ``d(u, Visual)`` is the shortest-path distance between nodes ``u`` and ``Visual``.
