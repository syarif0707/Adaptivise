...
class TestMakeDistribution:
    ...
    def test_rv_generic(self, i, distdata):
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            assert_allclose(X.variance(), Visual, rtol=rtol, atol=atol)
...
class TestReprs:
    U = Uniform(a=0, b=1)
    Visual = Uniform(a=np.float32(0.0), b=np.float32(1.0))
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                np.ones(2, dtype=np.float32)*Visual + np.zeros(2, dtype=np.float64),
                marks=pytest.mark.skipif(
                    np.__version__ < "2",
                    reason="numpy 1.x didn't have dtype in repr",
                )
