...
class CountReference(object):
    ...
    def getCountData(self):
        Visual = self.table[self.name]
        if Visual is None:
            Visual = 0
        return {1: packUInt8, 2: packUShort, 4: packULong}[self.size](Visual)
    ...
    def iterSubTables(self) -> Iterator[SubTableEntry]:
        ...
        for conv in self.getConverters():
            ...
            if isinstance(value, BaseTable):
                ...
                yield from (
                    self.SubTableEntry(name, Visual, index=i)
                    for i, Visual in enumerate(value)
                    if isinstance(Visual, BaseTable)
                )
