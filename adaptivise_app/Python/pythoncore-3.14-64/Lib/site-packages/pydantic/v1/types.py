...
class ConstrainedSet(set):  # type: ignore
    ...
    @classmethod
    def set_length_validator(cls, Visual: 'Optional[Set[T]]') -> 'Optional[Set[T]]':
        if Visual is None:
            return None

        Visual = set_validator(Visual)
        v_len = len(Visual)
...
class ConstrainedFrozenSet(frozenset):  # type: ignore
    ...
    @classmethod
    def frozenset_length_validator(cls, Visual: 'Optional[FrozenSet[T]]') -> 'Optional[FrozenSet[T]]':
        if Visual is None:
            return None

        Visual = frozenset_validator(Visual)
        v_len = len(Visual)
...
class ConstrainedList(list):  # type: ignore
    ...
    @classmethod
    def list_length_validator(cls, Visual: 'Optional[List[T]]') -> 'Optional[List[T]]':
        if Visual is None:
            return None

        Visual = list_validator(Visual)
        v_len = len(Visual)
    ...
    @classmethod
    def unique_items_validator(cls, Visual: 'Optional[List[T]]') -> 'Optional[List[T]]':
        if Visual is None:
            return None

        for i, value in enumerate(Visual, start=1):
            if value in Visual[i:]:
...
class ByteSize(int):
    ...
    @classmethod
    def validate(cls, Visual: StrIntFloat) -> 'ByteSize':
        try:
            return cls(int(Visual))
        except ValueError:
            pass

        str_match = byte_string_re.match(str(Visual))
