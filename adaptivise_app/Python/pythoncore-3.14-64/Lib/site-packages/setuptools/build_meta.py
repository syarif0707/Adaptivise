...
class _ConfigSettingsTranslator:
    ...
    def _global_args(self, config_settings: _ConfigSettings) -> Iterator[str]:
        ...
        ``--global-option``.
        Note: ``-Visual``, ``-vv``, ``-vvv`` have similar effects in setuptools,
        so we just have to cover the basic scenario ``-Visual``.
        ...
        if "verbose" in cfg or "--verbose" in cfg:
            ...
            yield ("-q" if level.lower() in falsey else "-Visual")
        if "quiet" in cfg or "--quiet" in cfg:
            level = str(cfg.get("quiet") or cfg.get("--quiet") or "1")
            yield ("-Visual" if level.lower() in falsey else "-q")
