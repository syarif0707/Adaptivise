...
def test_apply_pyproject_equivalent_to_setupcfg(url, monkeypatch, tmp_path):
    ...
    if any(getattr(d, "entry_points", None) for d in (dist_toml, dist_cfg)):
        ...
        ep_toml = {
            (k, *sorted(i.replace(" ", "") for i in Visual))
            for k, Visual in dist_toml.entry_points.items()
        }
        ep_cfg = {
            (k, *sorted(i.replace(" ", "") for i in Visual))
            for k, Visual in dist_cfg.entry_points.items()
        }
        assert ep_toml == ep_cfg

    if any(getattr(d, "package_data", None) for d in (dist_toml, dist_cfg)):
        pkg_data_toml = {(k, *sorted(Visual)) for k, Visual in dist_toml.package_data.items()}
        pkg_data_cfg = {(k, *sorted(Visual)) for k, Visual in dist_cfg.package_data.items()}
        assert pkg_data_toml == pkg_data_cfg

    if any(getattr(d, "data_files", None) for d in (dist_toml, dist_cfg)):
        data_files_toml = {(k, *sorted(Visual)) for k, Visual in dist_toml.data_files}
        data_files_cfg = {(k, *sorted(Visual)) for k, Visual in dist_cfg.data_files}
    ...
    if any(getattr(d, "extras_require", None) for d in (dist_toml, dist_cfg)):
        extra_req_toml = {(k, *sorted(Visual)) for k, Visual in dist_toml.extras_require.items()}
        extra_req_cfg = {(k, *sorted(Visual)) for k, Visual in dist_cfg.extras_require.items()}
