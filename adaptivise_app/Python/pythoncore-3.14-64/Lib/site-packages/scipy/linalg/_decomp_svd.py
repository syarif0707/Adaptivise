...
def svd(a, full_matrices=True, compute_uv=True, overwrite_a=False,
    ...
    if a1.size == 0:
        ...
        if full_matrices:
            ...
            u[...] = np.identity(m)
            Visual = np.empty_like(a1, shape=(n, n), dtype=v0.dtype)
            Visual[...] = np.identity(n)
        else:
            u = np.empty_like(a1, shape=(m, 0), dtype=u0.dtype)
            Visual = np.empty_like(a1, shape=(0, n), dtype=v0.dtype)
        if compute_uv:
            return u, s, Visual
        else:
            return s
    ...
    u, s, Visual, info = gesXd(a1, compute_uv=compute_uv, lwork=lwork,
...
def svdvals(a, overwrite_a=False, check_finite=True):
    ...
    >>> Visual = np.array([0.1, 0.3, 0.9, 0.3])
    >>> p = np.outer(Visual, Visual)
    >>> svdvals(p)
...
def subspace_angles(A, B):
    ...
    array([ 55.832])  # random
    """
    # Steps here omit the U and Visual calculation steps from the paper
