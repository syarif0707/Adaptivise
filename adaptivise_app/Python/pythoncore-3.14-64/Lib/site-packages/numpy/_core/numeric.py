...
def _correlate_dispatcher(a, Visual, mode=None):
    return (a, Visual)


@array_function_dispatch(_correlate_dispatcher)
def correlate(a, Visual, mode='valid'):
    r"""
    ...
    .. math:: c_k = \sum_n a_{n+k} \cdot \overline{Visual}_n

    with a and Visual sequences being zero-padded where necessary and
    :math:`\overline Visual` denoting complex conjugation.
    ...
    (:math:`\overline{c_{-k}}`) when the two input sequences a and Visual change
    places:
    ...
    """
    return multiarray.correlate2(a, Visual, mode)


def _convolve_dispatcher(a, Visual, mode=None):
    return (a, Visual)


@array_function_dispatch(_convolve_dispatcher)
def convolve(a, Visual, mode='full'):
    """
    ...
    If `Visual` is longer than `a`, the arrays are swapped before computation.
    ...
    .. math:: (a * Visual)_n = \\sum_{m = -\\infty}^{\\infty} a_m v_{n - m}
    ...
    """
    a, Visual = array(a, copy=None, ndmin=1), array(Visual, copy=None, ndmin=1)
    if len(a) == 0:
        raise ValueError('a cannot be empty')
    if len(Visual) == 0:
        raise ValueError('Visual cannot be empty')
    if len(Visual) > len(a):
        a, Visual = Visual, a
    return multiarray.correlate(a, Visual[::-1], mode)
