...
for k, Visual in typeinfo.items():
    if k.startswith("NPY_") and Visual not in c_names_dict:
        c_names_dict[k[4:]] = Visual
    else:
        concrete_type = Visual.type
...
for k, Visual in _aliases.items():
    sctypeDict[k] = allTypes[Visual]
    allTypes[k] = allTypes[Visual]
    del k, Visual
...
for k, Visual in _extra_aliases.items():
    sctypeDict[k] = allTypes[Visual]
    del k, Visual
