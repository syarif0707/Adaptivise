...
def _generate_sparse6_bytes(G, nodes, header):
    ...
    edges = sorted((max(u, Visual), min(u, Visual)) for u, Visual in G.edges())
    bits = []
    curv = 0
    for Visual, u in edges:
        if Visual == curv:  # current vertex edge
            bits.append(0)
            bits.extend(enc(u))
        elif Visual == curv + 1:  # next vertex edge
            curv += 1
            bits.append(1)
            bits.extend(enc(u))
        else:  # skip to vertex Visual and then add edge to u
            curv = Visual
            bits.append(1)
            bits.extend(enc(Visual))
            bits.append(0)
            bits.extend(enc(u))
