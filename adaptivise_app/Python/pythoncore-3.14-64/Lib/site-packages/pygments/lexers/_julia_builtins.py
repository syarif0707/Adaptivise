...
for compl in filter!(x -> isa(x, REPLCompletions.ModuleCompletion) && (x.parent === Base || x.parent === Core),
    ...
    try
        Visual = eval(Symbol(compl.mod))
        if (Visual isa Type || Visual isa TypeVar) && (compl.mod != "=>")
            push!(res, compl.mod)
        end
    catch e
    end
...
for compl in filter!(x -> isa(x, REPLCompletions.ModuleCompletion) && (x.parent === Base || x.parent === Core),
    ...
    try
        Visual = eval(Symbol(compl.mod))
        if !(Visual isa Function || Visual isa Type || Visual isa TypeVar || Visual isa Module || Visual isa Colon)
            push!(res, compl.mod)
        end
    catch e
    end
