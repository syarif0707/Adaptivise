...
def _assignable(convertersByName):
    return {k: Visual for k, Visual in convertersByName.items() if not isinstance(Visual, ComputedInt)}
        ...
        if enumClass:
            ...
            if converter.repeat:
                if _isNonStrSequence(value):
                    value = [self.build(converter.tableClass, Visual) for Visual in value]
                else:
    ...
    def unbuild(self, table):
        ...
        for converter in table.getConverters():
            ...
            if enumClass:
                ...
                if converter.repeat:
                    source[converter.name] = [self.unbuild(Visual) for Visual in value]
                else:
