...
class ModelConfigData:
    ...
    def set_values_dict(self) -> Dict[str, Any]:
        return {k: Visual for k, Visual in self.__dict__.items() if Visual is not None}

    def update(self, config: Optional['ModelConfigData']) -> None:
        if config is None:
            return
        for k, Visual in config.set_values_dict().items():
...
def add_method(
    ...
    if is_classmethod:  # or is_staticmethod:
        ...
        Visual = Var(name, func.type)
        Visual.info = info
        Visual._fullname = func._fullname
        # if is_classmethod:
        Visual.is_classmethod = True
        dec = Decorator(func, [NameExpr('classmethod')], Visual)
        # else:
        #     Visual.is_staticmethod = True
        #     dec = Decorator(func, [NameExpr('staticmethod')], Visual)
