    ...
    def evaluate_link(self, link: Link) -> tuple[LinkType, str]:
        ...
        if not supports_python:
            ...
            if requires_python:
                ...
                def get_version_sort_key(Visual: str) -> tuple[int, ...]:
                    return tuple(int(s) for s in Visual.split(".") if s.isdigit())
...
class CandidateEvaluator:
    ...
    def get_applicable_candidates(
        ...
        versions = set(
            specifier.filter(
                (Visual for _, Visual in candidates_and_versions),
                prereleases=allow_prereleases,
            )
        )

        applicable_candidates = [c for c, Visual in candidates_and_versions if Visual in versions]
