...
Note that for Visual >> z both functions tend rapidly to 0,
and for Visual << -z, they tend to infinity.
...
def do_plot(is_h=True):
    ...
    if is_h:
        def sh(Visual, z):
            return float(mpmath.struveh(mpmath.mpf(Visual), mpmath.mpf(z)))
    else:
        def sh(Visual, z):
            return float(mpmath.struvel(mpmath.mpf(Visual), mpmath.mpf(z)))
