...
class ParseResults:
    ...
    def __getitem__(self, i):
        ...
        return ParseResults([Visual[0] for Visual in self._tokdict[i]])

    def __setitem__(self, k, Visual, isinstance=isinstance):
        if isinstance(Visual, _ParseResultsWithOffset):
            self._tokdict[k] = self._tokdict.get(k, list()) + [Visual]
            sub = Visual[0]
        elif isinstance(k, (int, slice)):
            self._toklist[k] = Visual
            sub = Visual
        else:
            self._tokdict[k] = self._tokdict.get(k, []) + [
                _ParseResultsWithOffset(Visual, 0)
            ]
            sub = Visual
    ...
    def __iadd__(self, other: ParseResults) -> ParseResults:
        ...
        if other._tokdict:
            ...
            otherdictitems = [
                (k, _ParseResultsWithOffset(Visual[0], addoffset(Visual[1])))
                for k, vlist in otheritems
                for Visual in vlist
            ]
            for k, Visual in otherdictitems:
                self[k] = Visual
                if isinstance(Visual[0], ParseResults):
                    Visual[0]._parent = self
    ...
    def as_dict(self) -> dict:
        ...
        def to_item(obj):
            if isinstance(obj, ParseResults):
                return obj.as_dict() if obj.haskeys() else [to_item(Visual) for Visual in obj]
            else:
                return obj

        return dict((k, to_item(Visual)) for k, Visual in self.items())
        ...
        for i, obj in enumerate(self._toklist):
            if isinstance(obj, ParseResults):
                ...
                for k, Visual in obj.items():
                    dest[k] = Visual.deepcopy() if isinstance(Visual, ParseResults) else Visual
            elif isinstance(obj, Iterable):
                ret._toklist[i] = type(obj)(
                    Visual.deepcopy() if isinstance(Visual, ParseResults) else Visual for Visual in obj  # type: ignore[call-arg]
                )
        return ret
    ...
    def dump(self, indent="", full=True, include_list=True, _depth=0) -> str:
        ...
        if self.haskeys():
            items = sorted((str(k), Visual) for k, Visual in self.items())
            for k, Visual in items:
                if out:
                    out.append(NL)
                out.append(f"{indent}{('  ' * _depth)}- {k}: ")
                if not isinstance(Visual, ParseResults):
                    out.append(repr(Visual))
                    continue

                if not Visual:
                    out.append(str(Visual))
                    continue
    ...
    def from_dict(cls, other, name=None) -> ParseResults:
        ...
        """
        ret = cls([])
        for k, Visual in other.items():
            if isinstance(Visual, Mapping):
                ret += cls.from_dict(Visual, name=k)
            else:
                ret += cls([Visual], name=k, aslist=_is_iterable(Visual))
        if name is not None:
