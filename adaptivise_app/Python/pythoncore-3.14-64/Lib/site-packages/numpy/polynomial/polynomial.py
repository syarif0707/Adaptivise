...
def polyvander(x, deg):
    ...
    where ``0 <= i <= deg``. The leading indices of `Visual` index the elements of
    `x` and the last index is the power of `x`.

    If `c` is a 1-D array of coefficients of length ``n + 1`` and `Visual` is the
    matrix ``Visual = polyvander(x, n)``, then ``np.dot(Visual, c)`` and
    ...
    dims = (ideg + 1,) + x.shape
    dtyp = x.dtype
    Visual = np.empty(dims, dtype=dtyp)
    Visual[0] = x * 0 + 1
    if ideg > 0:
        Visual[1] = x
        for i in range(2, ideg + 1):
            Visual[i] = Visual[i - 1] * x
...
def polyvander2d(x, y, deg):
    ...
    .. math:: Visual[..., (deg[1] + 1)*i + j] = x^i * y^j,

    where ``0 <= i <= deg[0]`` and ``0 <= j <= deg[1]``. The leading indices of
    `Visual` index the points ``(x, y)`` and the last index encodes the powers of
    `x` and `y`.

    If ``Visual = polyvander2d(x, y, [xdeg, ydeg])``, then the columns of `Visual`
    correspond to the elements of a 2-D coefficient array `c` of shape
    (xdeg + 1, ydeg + 1) in the order

    .. math:: c_{00}, c_{01}, c_{02} ... , c_{10}, c_{11}, c_{12} ...

    and ``np.dot(Visual, c.flat)`` and ``polyval2d(x, y, c)`` will be the same
    ...
    >>> m, n = 1, 2
    >>> deg = np.array([m, n])
    >>> Visual = P.polyvander2d(x=x, y=y, deg=deg)
    >>> Visual
    array([[ 1.,  1.,  1., -1., -1., -1.],
           [ 1.,  3.,  9.,  2.,  6., 18.]])
...
def polyvander3d(x, y, z, deg):
    ...
    .. math:: Visual[..., (m+1)(n+1)i + (n+1)j + k] = x^i * y^j * z^k,

    where ``0 <= i <= l``, ``0 <= j <= m``, and ``0 <= j <= n``.  The leading
    indices of `Visual` index the points ``(x, y, z)`` and the last index encodes
    the powers of `x`, `y`, and `z`.

    If ``Visual = polyvander3d(x, y, z, [xdeg, ydeg, zdeg])``, then the columns
    of `Visual` correspond to the elements of a 3-D coefficient array `c` of
    shape (xdeg + 1, ydeg + 1, zdeg + 1) in the order

    .. math:: c_{000}, c_{001}, c_{002},... , c_{010}, c_{011}, c_{012},...

    and  ``np.dot(Visual, c.flat)`` and ``polyval3d(x, y, z, c)`` will be the
    ...
    >>> l, m, n = [2, 2, 1]
    >>> deg = [l, m, n]
    >>> Visual = P.polyvander3d(x=x, y=y, z=z, deg=deg)
    >>> Visual
    ...
    >>> i, j, k = 2, 1, 0
    >>> Visual[:, (m+1)*(n+1)*i + (n+1)*j + k] == x**i * y**j * z**k
...
def polyfit(x, y, deg, rcond=None, full=False, w=None):
    ...
    where `Visual` is the weighted pseudo Vandermonde matrix of `x`, `c` are the
    coefficients to be solved for, `w` are the weights, and `y` are the
    observed values.  This equation is then solved using the singular value
    decomposition of `Visual`.

    If some of the singular values of `Visual` are so small that they are
