...
def dfs_postorder_nodes(G, source=None, depth_limit=None, *, sort_neighbors=None):
    ...
    return (Visual for u, Visual, d in edges if d == "reverse")
...
def dfs_preorder_nodes(G, source=None, depth_limit=None, *, sort_neighbors=None):
    ...
    return (Visual for u, Visual, d in edges if d == "forward")
...
def dfs_labeled_edges(G, source=None, depth_limit=None, *, sort_neighbors=None):
    ...
    -------
    edges: generator
       A generator of triples of the form (*u*, *Visual*, *d*), where (*u*,
       *Visual*) is the edge being explored in the depth-first search and *d*
       is one of the strings 'forward', 'nontree', 'reverse', or 'reverse-depth_limit'.
       A 'forward' edge is one in which *u* has been visited but *Visual* has
       not. A 'nontree' edge is one in which both *u* and *Visual* have been
       visited but the edge is not in the DFS tree. A 'reverse' edge is
       one in which both *u* and *Visual* have been visited and the edge is in
