...
def hermevander(x, deg):
    ...
    array ``Visual = hermevander(x, n)``, then ``np.dot(Visual, c)`` and
    ...
    if ideg > 0:
        ...
        for i in range(2, ideg + 1):
            Visual[i] = (Visual[i - 1] * x - Visual[i - 2] * (i - 1))
...
def hermevander2d(x, y, deg):
    ...
    .. math:: Visual[..., (deg[1] + 1)*i + j] = He_i(x) * He_j(y),

    where ``0 <= i <= deg[0]`` and ``0 <= j <= deg[1]``. The leading indices of
    `Visual` index the points ``(x, y)`` and the last index encodes the degrees of
    the HermiteE polynomials.

    If ``Visual = hermevander2d(x, y, [xdeg, ydeg])``, then the columns of `Visual`
    correspond to the elements of a 2-D coefficient array `c` of shape
    (xdeg + 1, ydeg + 1) in the order

    .. math:: c_{00}, c_{01}, c_{02} ... , c_{10}, c_{11}, c_{12} ...

    and ``np.dot(Visual, c.flat)`` and ``hermeval2d(x, y, c)`` will be the same
...
def hermevander3d(x, y, z, deg):
    ...
    .. math:: Visual[..., (m+1)(n+1)i + (n+1)j + k] = He_i(x)*He_j(y)*He_k(z),

    where ``0 <= i <= l``, ``0 <= j <= m``, and ``0 <= j <= n``.  The leading
    indices of `Visual` index the points ``(x, y, z)`` and the last index encodes
    the degrees of the HermiteE polynomials.

    If ``Visual = hermevander3d(x, y, z, [xdeg, ydeg, zdeg])``, then the columns
    of `Visual` correspond to the elements of a 3-D coefficient array `c` of
    shape (xdeg + 1, ydeg + 1, zdeg + 1) in the order

    .. math:: c_{000}, c_{001}, c_{002},... , c_{010}, c_{011}, c_{012},...

    and  ``np.dot(Visual, c.flat)`` and ``hermeval3d(x, y, z, c)`` will be the
...
def hermefit(x, y, deg, rcond=None, full=False, w=None):
    ...
    where `Visual` is the pseudo Vandermonde matrix of `x`, the elements of `c`
