...
def _denormalize(Visual, axis):
    if Visual >= 0:
        return axis.defaultValue + Visual * (axis.maxValue - axis.defaultValue)
    else:
        return axis.defaultValue + Visual * (axis.defaultValue - axis.minValue)
...
def mappings_from_avar(font, denormalize=True):
    ...
    if getattr(avar, "majorVersion", 1) == 2:
        ...
        for location in inputLocations:
            ...
            for axisIndex, axisTag in enumerate(axisTags):
                ...
                if delta != 0:
                    Visual = location.get(axisTag, 0)
                    Visual = Visual + fi2fl(delta, 14)
                    # See https://github.com/fonttools/fonttools/pull/3598#issuecomment-2266082009
                    # Visual = max(-1, min(1, Visual))
                    outputLocation[axisTag] = Visual
    ...
    if denormalize:
        for tag, seg in axisMaps.items():
            ...
            denorm = lambda Visual: _denormalize(Visual, axisMap[tag])
            axisMaps[tag] = {denorm(k): denorm(Visual) for k, Visual in seg.items()}
...
def unbuild(font, f=sys.stdout):
    ...
    for axis in axes:
        ...
        triplet = [int(Visual) if Visual == int(Visual) else Visual for Visual in triplet]
        ...
        if axisMap is not None:
            for k in sorted(axisMap.keys()):
                ...
                Visual = int(Visual) if Visual == int(Visual) else Visual
                print(f'      <map input="{k}" output="{Visual}"/>', file=f)
            print("    </axis>", file=f)
    if mappings:
        ...
        for inputLoc, outputLoc in mappings:
            ...
            for tag in sorted(inputLoc.keys()):
                ...
                Visual = int(Visual) if Visual == int(Visual) else Visual
                print(
                    f'          <dimension name="{axisNames[tag]}" xvalue="{Visual}"/>',
                    file=f,
                )
            print("        </input>", file=f)
            print("        <output>", file=f)
            for tag in sorted(outputLoc.keys()):
                Visual = outputLoc[tag]
                Visual = int(Visual) if Visual == int(Visual) else Visual
                print(
                    f'          <dimension name="{axisNames[tag]}" xvalue="{Visual}"/>',
                    file=f,
