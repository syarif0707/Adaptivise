...
class RatioUniforms:
    ...
    vmin : float
        The lower bound of the bounding rectangle in the Visual-direction.
    vmax : float
        The upper bound of the bounding rectangle in the Visual-direction.
    c : float, optional.
    ...
    define the set ``A = {(u, Visual) : 0 < u <= sqrt(pdf(Visual/u + c))}``.
    If ``(U, Visual)`` is a random vector uniformly distributed over ``A``,
    then ``Visual/U + c`` follows a distribution according to `pdf`.
    ...
    One can generate ``(U, Visual)`` uniformly on ``R`` and return
    ``Visual/U + c`` if ``(U, Visual)`` are also in ``A`` which can be directly
    verified.
    ...
    enclosing rectangle such that the probability is high that ``(U, Visual)``
    lies in ``A`` whenever it lies in ``R`` as the number of required
    iterations becomes too large otherwise. To be more precise, note that
    the expected number of iterations to draw ``(U, Visual)`` uniformly
    distributed on ``R`` such that ``(U, Visual)`` is also in ``A`` is given by
    ...
    >>> umax = np.sqrt(f(0))
    >>> gen = RatioUniforms(f, umax=umax, vmin=-Visual, vmax=Visual, random_state=rng)
    >>> r = gen.rvs(size=2500)
