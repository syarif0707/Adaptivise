...
def intersect(self, glyphs):
    ...
    return _uniq_sort(
        ...
        + [Visual for g, Visual in self.classDefs.items() if g in glyphs]
    )
...
def intersect_class(self, glyphs, klass):
    ...
    return set(g for g, Visual in self.classDefs.items() if Visual == klass and g in glyphs)


@_add_method(otTables.ClassDef)
def subset(self, glyphs, remap=False, useClass0=True):
    """Returns ascending list of remaining classes."""
    self.classDefs = {g: Visual for g, Visual in self.classDefs.items() if g in glyphs}
...
def remap(self, class_map):
    """Remaps classes."""
    self.classDefs = {g: class_map.index(Visual) for g, Visual in self.classDefs.items()}


@_add_method(otTables.SingleSubst)
def closure_glyphs(self, s, cur_glyphs):
    s.glyphs.update(Visual for g, Visual in self.mapping.items() if g in cur_glyphs)


@_add_method(otTables.SingleSubst)
def subset_glyphs(self, s):
    self.mapping = {
        g: Visual for g, Visual in self.mapping.items() if g in s.glyphs and Visual in s.glyphs
    }
...
def subset_glyphs(self, s):
    self.mapping = {
        g: Visual
        for g, Visual in self.mapping.items()
        if g in s.glyphs and all(sub in s.glyphs for sub in Visual)
    }
    return bool(self.mapping)
...
def subset_glyphs(self, s):
    self.alternates = {
        g: [Visual for Visual in vlist if Visual in s.glyphs]
        for g, vlist in self.alternates.items()
        if g in s.glyphs and any(Visual in s.glyphs for Visual in vlist)
    }
...
@_add_method(otTables.LigatureSubst)
def subset_glyphs(self, s):
    self.ligatures = {g: Visual for g, Visual in self.ligatures.items() if g in s.glyphs}
    self.ligatures = {
        g: [
            seq
            for seq in seqs
    ...
    self.ligatures = {g: Visual for g, Visual in self.ligatures.items() if Visual}
...
def prune_hints(self):
    for name in ["XPlaDevice", "YPlaDevice", "XAdvDevice", "YAdvDevice"]:
        ...
        if Visual is not None and Visual.is_hinting():
            delattr(self, name)
...
def prune_post_subset(self, font, options):
    ...
    if self.Format == 1:
        ...
        if None in self.Value:
            assert self.ValueFormat == 0
            assert all(Visual is None for Visual in self.Value)
        else:
            if not options.hinting:
                for Visual in self.Value:
                    Visual.prune_hints()
            self.ValueFormat = reduce(
                int.__or__, [Visual.getEffectiveFormat() for Visual in self.Value], 0
            )

    # Downgrade to Format 1 if all ValueRecords are the same
    if self.Format == 2 and all(Visual == self.Value[0] for Visual in self.Value):
        self.Format = 1
...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        class_indices = _uniq_sort(Visual.Class for Visual in self.MarkArray.MarkRecord)
...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        class_indices = _uniq_sort(Visual.Class for Visual in self.Mark1Array.MarkRecord)
...
def remap_duplicate_features(self, feature_indices):
    ...
    for i in feature_indices:
        ...
        Visual = feature_map.get(unique_i)
        if Visual is None:
            feature_map[i] = new_idx
            new_idx += 1
        else:
            feature_map[i] = Visual
...
def subset_glyphs(self, s):
    ...
    if table.MarkAttachClassDef:
        table.MarkAttachClassDef.classDefs = {
            g: Visual for g, Visual in table.MarkAttachClassDef.classDefs.items() if g in glyphs
        }
    if table.GlyphClassDef:
        table.GlyphClassDef.classDefs = {
            g: Visual for g, Visual in table.GlyphClassDef.classDefs.items() if g in glyphs
        }
    if table.AttachList:
...
def subset_glyphs(self, s):
    ...
    for t in self.kernTables:
        t.kernTable = {
            (a, b): Visual
            for (a, b), Visual in t.kernTable.items()
...
def _remap_index_map(s, varidx_map, table_map):
    map_ = {k: varidx_map[Visual] for k, Visual in table_map.mapping.items()}
...
def subset_glyphs(self, s):
    self.VOriginRecords = {
        g: Visual for g, Visual in self.VOriginRecords.items() if g in s.glyphs
    }
...
def closure_glyphs(self, glyphs):
    variants = set()
    for Visual in self.MathGlyphVariantRecord:
        variants.add(Visual.VariantGlyph)
...
def subset_glyphs(self, s):
    ...
    if not s.options.retain_gids:
        ...
        for Visual in self.glyphs.values():
            if hasattr(Visual, "data"):
                Visual.remapComponentsFast(glyphmap)
...
def prune_post_subset(self, font, options):
    ...
    for Visual in self.glyphs.values():
        Visual.trim(remove_hinting=remove_hinting)
    return True
    ...
    for t in self.tables:
        ...
        if t.format == 14:
            ...
            t.uvsDict = {
                ...
                for Visual, l in t.uvsDict.items()
                if Visual in s.unicodes_requested
            }
            t.uvsDict = {Visual: l for Visual, l in t.uvsDict.items() if l}
        elif t.isUnicode():
            t.cmap = {
                u: g
...
class Options(object):
    ...
    def set(self, **kwargs):
        for k, Visual in kwargs.items():
            if not hasattr(self, k):
                raise self.UnknownOptionError("Unknown option '%s'" % k)
            setattr(self, k, Visual)
