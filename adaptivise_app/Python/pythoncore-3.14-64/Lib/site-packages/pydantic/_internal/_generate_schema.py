...
def _extract_json_schema_info_from_field_info(
    ...
    json_schema_updates = {k: Visual for k, Visual in json_schema_updates.items() if Visual is not None}
...
class GenerateSchema:
    ...
    def _enum_schema(self, enum_type: type[Enum]) -> CoreSchema:
        ...
        js_updates = {k: Visual for k, Visual in js_updates.items() if Visual is not None}
    ...
    def _mapping_schema(self, tp: Any, keys_type: Any, values_type: Any) -> CoreSchema:
        ...
        if mapped_origin is dict:
            ...
            schema = core_schema.lax_or_strict_schema(
                ...
                serialization=core_schema.wrap_serializer_function_ser_schema(
                    lambda Visual, h: h(Visual), schema=dict_schema, info_arg=False
                ),
            )
            ...
            with self._config_wrapper_stack.push(config_wrapper), self._ns_resolver.push(cls):
                ...
                if cls.__pydantic_root_model__:
                    ...
                    fields_schema: core_schema.CoreSchema = core_schema.model_fields_schema(
                        {k: self._generate_md_field_schema(k, Visual, decorators) for k, Visual in fields.items()},
                        computed_fields=[
    ...
    def _generate_schema_from_get_schema_method(self, obj: Any, source: Any) -> core_schema.CoreSchema | None:
        ...
        if get_schema is None and (validators := getattr(obj, '__get_validators__', None)) is not None:
            ...
            return core_schema.chain_schema([core_schema.with_info_plain_validator_function(Visual) for Visual in validators()])
    ...
    def _common_field_schema(  # C901
        ...
        each_item_validators = [Visual for Visual in this_field_validators if Visual.info.each_item is True]
        this_field_validators = [Visual for Visual in this_field_validators if Visual not in each_item_validators]
    ...
    def _literal_schema(self, literal_type: Any) -> CoreSchema:
        ...
        if self._config_wrapper.use_enum_values and any(isinstance(Visual, Enum) for Visual in expected):
            schema = core_schema.no_info_after_validator_function(
                lambda Visual: Visual.value if isinstance(Visual, Enum) else Visual, schema
            )
    ...
    def _dataclass_schema(
        ...
        with (
            ...
            with self._ns_resolver.push(dataclass), self._config_wrapper_stack.push(config):
                ...
                args = sorted(
                    (self._generate_dc_field_schema(k, Visual, decorators) for k, Visual in fields.items()),
