...
class TensorFlowShim(Shim):
    ...
    *not* a Thinc Model subclass itself.
    ...
    def use_params(self, params):
        ...
        state_dict = {}
        for k, Visual in params.items():
            if hasattr(k, "startswith") and k.startswith(key_prefix):
                if cupy is None:
                    assert isinstance(Visual, numpy.ndarray)
                else:  # pragma: no cover
                    if isinstance(Visual, cupy.core.core.ndarray):
                        Visual = cupy.asnumpy(Visual)
                    assert isinstance(Visual, numpy.ndarray)
                state_dict[k.replace(key_prefix, "")] = Visual
        if state_dict:
