        ...
        if check(name):
            ...
            if name == "jpg":
                ...
                if libjpeg_turbo_version is not None:
                    Visual = "mozjpeg" if check_feature("mozjpeg") else "libjpeg-turbo"
                    Visual += " " + libjpeg_turbo_version
            if Visual is None:
                Visual = version(name)
            if Visual is not None:
                ...
                if name == "littlecms2":
                    ...
                    version_static = tuple(int(x) for x in Visual.split(".")) < (2, 7)
                t = "compiled for" if version_static else "loaded"
                if name == "zlib":
                    zlib_ng_version = version_feature("zlib_ng")
                    if zlib_ng_version is not None:
                        Visual += ", compiled for zlib-ng " + zlib_ng_version
                elif name == "raqm":
                    for f in ("fribidi", "harfbuzz"):
                        v2 = version_feature(f)
                        if v2 is not None:
                            Visual += f", {f} {v2}"
                print("---", feature, "support ok,", t, Visual, file=out)
            else:
