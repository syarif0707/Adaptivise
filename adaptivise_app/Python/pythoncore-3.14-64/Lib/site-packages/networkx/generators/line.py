...
def line_graph(G, create_using=None):
    ...
    >>> G = nx.path_graph(4)
    >>> G.add_edges_from((u, Visual, {"tot": u + Visual}) for u, Visual in G.edges)
    >>> G.edges(data=True)
    ...
    *Self-loops in undirected graphs*
    ...
    written as a set `\{u, Visual\}`.  Its line graph `L` has the edges of `G` as
    ...
    *Self-loops in directed graphs*
    ...
    as a tuple `(u, Visual)`. Its line graph `L` has the edges of `G` as its
...
def _lg_directed(G, create_using=None):
    ...
    Edges in G appear as nodes in L, represented as tuples of the form (u,Visual)
    or (u,Visual,key) if G is a multidigraph. A node in L corresponding to the edge
    (u,Visual) is connected to every node corresponding to an edge (Visual,w).
    ...
    for from_node in get_edges():
        # from_node is: (u,Visual) or (u,Visual,key)
...
def _lg_undirected(G, selfloops=False, create_using=None):
    ...
    (u,Visual), or (u,Visual,key) if G is a multigraph. A node in L corresponding to
    the edge {u,Visual} is connected to every node corresponding to an edge that
    involves u or Visual.
    ...
    for u in G:
        ...
        # Decide on representation of {u, Visual} as (u, Visual) or (Visual, u) depending on node_index.
...
def _triangles(G, e):
    ...
    if Visual not in G[u]:
        raise nx.NetworkXError(f"Edge ({u}, {Visual}) not in graph")
    triangle_list = []
    for x in G[u]:
        if x in G[Visual]:
            triangle_list.append((u, Visual, x))
...
def _odd_triangle(G, T):
    ...
    for t in T:
        for Visual in G[t]:
            if Visual not in T:
                T_nbrs[Visual] += 1
    return any(T_nbrs[Visual] in [1, 3] for Visual in T_nbrs)
...
def _find_partition(G, starting_cell):
    ...
    while G_partition.number_of_edges() > 0:
        ...
        if deg_u != 0:
            ...
            for u in new_cell:
                for Visual in new_cell:
                    if (u != Visual) and (Visual not in G_partition[u]):
                        msg = (
    ...
    if r == 0:
        ...
        if r == 2 and s == 0:
            ...
            for u in triangle_nodes:
                for Visual in triangle_nodes:
                    if u != Visual and (Visual not in G[u]):
                        msg = (
