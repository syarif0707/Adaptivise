...
class TestVariation:
    ...
    def test_sign(self, sgn, xp):
        ...
        Visual = variation(sgn*x)
        expected = xp.asarray(sgn*math.sqrt(2)/3)
        xp_assert_close(Visual, expected, rtol=1e-10)
    ...
    def test_propagate_nan(self, xp):
        ...
        a[1, 0] = xp.nan
        Visual = variation(a, axis=1, nan_policy="propagate")
        xp_assert_close(Visual, [math.sqrt(5/4)/1.5, xp.nan], atol=1e-15)
    ...
    def test_more_nan_policy_omit_tests(self, ddof, expected, xp):
        ...
        x = xp.asarray([[1.0, 2.0, nan, 3.0],
                        ...
                        [0.0, 0.0, 0.0, 0.0]])
        with pytest.warns(SmallSampleWarning, match=too_small_nd_omit):
            Visual = variation(x, axis=1, ddof=ddof, nan_policy='omit')
        xp_assert_close(Visual, expected)
