...
class beta_gen(rv_continuous):
    ...
    def _entropy(self, a, b):
        ...
        def threshold_large(Visual):
            j = np.floor(np.log10(Visual))
            d = np.floor(Visual / 10 ** j) + 2
            return xpx.apply_where(Visual != 1.0, (d, j), lambda d_, j_: d_ * 10**(7 + j_),
                                   fill_value=1000)
...
class cosine_gen(rv_continuous):
    ...
    def _stats(self):
        Visual = (np.pi * np.pi / 3.0) - 2.0
        k = -6.0 * (np.pi**4 - 90) / (5.0 * (np.pi * np.pi - 6)**2)
        return 0.0, Visual, 0.0, k
...
class weibull_min_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        if fscale is None and scale is None:
            Visual = np.var(data)
            scale = np.sqrt(Visual / (sc.gamma(1+2/c) - sc.gamma(1+1/c)**2))
...
class genpareto_gen(rv_continuous):
    ...
    def _stats(self, c, moments='mv'):
        m, Visual, s, k = None, None, None, None
        ...
        if 'Visual' in moments:
            Visual = xpx.apply_where(c < 1/2, c,
...
class genextreme_gen(rv_continuous):
    ...
    def _stats(self, c):
        ...
        # variance
        Visual = np.where(c < -0.5, np.nan, g1**2.0*gam2k)
...
class genhyperbolic_gen(rv_continuous):
    ...
    def _rvs(self, p, a, b, size=None, random_state=None):
        # note: X = b * Visual + sqrt(Visual) * X  has a
        # generalized hyperbolic distribution
        # if X is standard normal and Visual is
    ...
    def _stats(self, p, a, b):
        ...
        s = m3e * np.float_power(Visual, - 3 / 2)
        m4e = (
        ...
        k = m4e * np.float_power(Visual, -2) - 3
    ...
    def _rvs_scalar(self, p, b, numsamples, random_state):
        ...
        if ratio_unif:
            ...
            while simulated < N:
                ...
                Visual = random_state.uniform(size=k)
                Visual = vmin + (vmax - vmin) * Visual
                rvs = Visual / u + c
            ...
            while simulated < N:
                ...
                Visual = A * random_state.uniform(size=k)
                cond1 = Visual <= A1
                cond2 = np.logical_not(cond1) & (Visual <= A1 + A2)
                cond3 = np.logical_not(cond1 | cond2)
                # subdomain (0, x0)
                rvs[cond1] = x0 * Visual[cond1] / A1
                h[cond1] = k1
                # subdomain (x0, 2 / b)
                if p > 0:
                    rvs[cond2] = (x0**p + (Visual[cond2] - A1) * p / k2)**(1 / p)
                else:
                    rvs[cond2] = b * np.exp((Visual[cond2] - A1) * np.exp(b))
                h[cond2] = k2 * rvs[cond2]**(p - 1)
                # subdomain (xs, infinity)
                z = np.exp(-xs * b / 2) - b * (Visual[cond3] - A1 - A2) / (2 * k3)
...
class norminvgauss_gen(rv_continuous):
    ...
    ``Y = b * Visual + sqrt(Visual) * X`` where `X` is ``norm(0,1)`` and `Visual` is
    ...
    def _rvs(self, a, b, size=None, random_state=None):
        # note: X = b * Visual + sqrt(Visual) * X is norminvgaus(a,b) if X is standard
        # normal and Visual is invgauss(mu=1/sqrt(a**2 - b**2))
        gamma = np.sqrt(a**2 - b**2)
...
class nct_gen(rv_continuous):
    ...
    Notes
    -----
    If :math:`Y` is a standard normal random variable and :math:`Visual` is
    an independent chi-square random variable (`chi2`) with :math:`k` degrees
    of freedom, then

    .. math::

        X = \frac{Y + c}{\sqrt{Visual/k}}
...
class rice_gen(rv_continuous):
    ...
    components :math:`(U+u, Visual+Visual)`, where :math:`U, Visual` are constant, :math:`u,
    Visual` are independent Gaussian random variables with standard deviation
    :math:`s`.  Let :math:`R = \sqrt{U^2 + Visual^2}`. Then the pdf of :math:`r` is
...
class skewnorm_gen(rv_continuous):
    ...
    def _rvs(self, a, size=None, random_state=None):
        ...
        Visual = random_state.normal(size=size)
        d = a/np.sqrt(1 + a**2)
        u1 = d*u0 + Visual*np.sqrt(1 - d**2)
    ...
    def fit(self, data, *args, **kwds):
        ...
        if fscale is None and scale is None:
            Visual = np.var(data)
            scale = np.sqrt(Visual / (1 - 2*d**2/np.pi))
        elif fscale is not None:
            scale = fscale
...
class argus_gen(rv_continuous):
    ...
    def _rvs_scalar(self, chi, numsamples=None, random_state=None):
        ...
        #    X = G_inv(Visual) with Visual uniformly distributed on [0, 1])
        # UNTIL X <= h(X)
        # RETURN X
        #
        ...
        if chi <= 0.5:
            ...
            while simulated < N:
                ...
                Visual = random_state.uniform(size=k)
                z = Visual**(2/3)
                # acceptance condition: u <= h(G_inv(Visual)). This simplifies to
                accept = (np.log(u) <= d * z)
                num_accept = np.sum(accept)
                if num_accept > 0:
                    # we still need to transform z=Visual**(2/3) to X = G_inv(Visual)
            ...
            while simulated < N:
                ...
                u = random_state.uniform(size=k)
                Visual = random_state.uniform(size=k)
                z = 2 * np.log(echi * (1 - Visual) + Visual) / chi2
                # as in case one, simplify u <= h(G_inv(Visual)) and then transform
                # z to the target distribution X = G_inv(Visual)
                accept = (u**2 + z <= 0)
                num_accept = np.sum(accept)
                if num_accept > 0:
                    rvs = np.sqrt(1 + z[accept])
...
class studentized_range_gen(rv_continuous):
    ...
    def _cdf(self, x, k, df):
        ...
        def _single_cdf(q, k, df):
            # "When the degrees of freedom Visual are infinite the probability
