...
def test_properties(m, p, q, swap_sign):
    ...
    # We can see that U and Visual are block diagonal matrices like so:
    U = linalg.block_diag(u1, u2)
    Visual = linalg.block_diag(v1, v2)
    ...
    np.testing.assert_allclose(X, U @ Q @ Visual.conj().T)
    ...
    np.testing.assert_allclose(Visual, Vh0.conj().T)
