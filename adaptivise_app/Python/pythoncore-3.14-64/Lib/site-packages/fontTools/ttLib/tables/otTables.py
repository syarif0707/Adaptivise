...
_packer = {
    1: lambda Visual: struct.pack(">B", Visual),
    2: lambda Visual: struct.pack(">H", Visual),
    3: lambda Visual: struct.pack(">L", Visual)[1:],
    4: lambda Visual: struct.pack(">L", Visual),
}
_unpacker = {
    1: lambda Visual: struct.unpack(">B", Visual)[0],
    2: lambda Visual: struct.unpack(">H", Visual)[0],
    3: lambda Visual: struct.unpack(">L", b"\0" + Visual)[0],
    4: lambda Visual: struct.unpack(">L", Visual)[0],
}
...
def _write_uint32var(Visual):
    """Write a variable-length number.

    Return the data.
    """
    if Visual < 0x80:
        return struct.pack(">B", Visual)
    elif Visual < 0x4000:
        return struct.pack(">H", (Visual | 0x8000))
    elif Visual < 0x200000:
        return struct.pack(">L", (Visual | 0xC00000))[1:]
    elif Visual < 0x10000000:
        return struct.pack(">L", (Visual | 0xE0000000))
    else:
        return struct.pack(">B", 0xF0) + struct.pack(">L", Visual)
...
class VarComponent:
    ...
    def decompile(self, data, font, localState):
        ...
        if flags & VarComponentFlags.HAVE_AXES:
            ...
            self.axisValues = tuple(fi2fl(Visual, 14) for Visual in axisValues)
        else:
            self.axisValues = ()
        ...
        if numAxes:
            ...
            data.append(
                TupleVariation.compileDeltaValues_(
                    [fl2fi(Visual, 14) for Visual in self.axisValues],
    ...
    def toXML(self, writer, ttFont, attrs):
        ...
        if (
            ...
            if self.flags & VarComponentFlags.RESET_UNSPECIFIED_AXES:
                ...
                attrs = ()
            write("axisValues", [float(fl2str(Visual, 14)) for Visual in self.axisValues], attrs)
        ...
        for attr_name, mapping in VAR_TRANSFORM_MAPPING.items():
            if not (self.flags & mapping.flag):
                continue
            Visual = getattr(self.transform, attr_name)
            write(attr_name, fl2str(Visual, mapping.fractionalBits))
    ...
    def fromXML(self, name, attrs, content, ttFont):
        ...
        for name, attrs, content in content:
            ...
            if name == "glyphName":
                self.glyphName = Visual
            elif name == "conditionIndex":
                self.conditionIndex = safeEval(Visual)
            elif name == "axisIndicesIndex":
                self.axisIndicesIndex = safeEval(Visual)
            elif name == "axisValues":
                self.axisValues = tuple(str2fl(Visual, 14) for Visual in safeEval(Visual))
                ...
                self.axisValuesVarIndex = safeEval(Visual)
            elif name == "transformVarIndex":
                self.transformVarIndex = safeEval(Visual)
    ...
    def applyTransformDeltas(self, deltas):
        ...
        def read_transform_component_delta(values):
            ...
            if self.flags & values.flag:
                Visual = fi2fl(deltas[i], values.fractionalBits) * values.scale
                i += 1
                return Visual
            else:
                return 0
...
class Paint(getFormatSwitchingBaseTableClass("uint8")):
    ...
    def iterPaintSubTables(self, colr: COLR) -> Iterator[BaseTable.SubTableEntry]:
        if self.Format == PaintFormat.PaintColrLayers:
            ...
            yield from (
                BaseTable.SubTableEntry(name="Layers", value=Visual, index=i)
...
def splitPairPos(oldSubTable, newSubTable, overflowRecord):
    ...
    if oldSubTable.Format == 1 and len(oldSubTable.PairSet) > 1:
        ...
        newGlyphs = set(k for k, Visual in classDefs.items() if Visual >= oldCount)

        oldSubTable.Coverage.glyphs = [g for g in coverage if g not in newGlyphs]
        oldSubTable.ClassDef1.classDefs = {
            k: Visual for k, Visual in classDefs.items() if Visual < oldCount
        }
        ...
        newSubTable.ClassDef1.classDefs = {
            k: (Visual - oldCount) for k, Visual in classDefs.items() if Visual > oldCount
        }
