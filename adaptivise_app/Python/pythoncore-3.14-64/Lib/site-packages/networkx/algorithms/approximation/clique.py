...
def maximum_independent_set(G):
    ...
    Notes
    -----
    Finds the $O(|Visual|/(log|Visual|)^2)$ apx of independent set in the worst case.
...
def max_clique(G):
    ...
    Finds the $O(|Visual|/(log|Visual|)^2)$ apx of maximum clique/independent set
    in the worst case.
    ...
    Notes
    -----
    A clique in an undirected graph G = (Visual, E) is a subset of the vertex set
    `C \subseteq Visual` such that for every two vertices in C there exists an edge
...
def clique_removal(G):
    ...
    Results in a $O(|Visual|/(\log |Visual|)^2)$ approximation of maximum clique
