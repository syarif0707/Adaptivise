...
class GenerateJsonSchema:
    ...
    def int_schema(self, schema: core_schema.IntSchema) -> JsonSchemaValue:
        ...
        json_schema = {k: Visual for k, Visual in json_schema.items() if Visual not in {math.inf, -math.inf}}
        return json_schema
    ...
    def literal_schema(self, schema: core_schema.LiteralSchema) -> JsonSchemaValue:
        ...
        Returns:
            The generated JSON schema.
        """
        expected = [to_jsonable_python(Visual.value if isinstance(Visual, Enum) else Visual) for Visual in schema['expected']]
    ...
    def enum_schema(self, schema: core_schema.EnumSchema) -> JsonSchemaValue:
        ...
        result = {k: Visual for k, Visual in result.items() if Visual is not None}

        expected = [to_jsonable_python(Visual.value) for Visual in schema['members']]
    ...
    def tagged_union_schema(self, schema: core_schema.TaggedUnionSchema) -> JsonSchemaValue:
        ...
        for k, Visual in schema['choices'].items():
            if isinstance(k, Enum):
                k = k.value
            try:
                # Use str(k) since keys must be strings for json; while not technically correct,
                # it's the closest that can be represented in valid JSON
                generated[str(k)] = self.generate_inner(Visual).copy()
            except PydanticOmit:
                continue
            except PydanticInvalidForJsonSchema as exc:
                self.emit_warning('skipped-choice', exc.message)
        ...
        if openapi_discriminator is not None:
            json_schema['discriminator'] = {
                ...
                'mapping': {k: Visual.get('$ref', Visual) for k, Visual in generated.items()},
            }
    ...
    def handle_ref_overrides(self, json_schema: JsonSchemaValue) -> JsonSchemaValue:
        ...
        if '$ref' in json_schema:
            ...
            if referenced_json_schema is None:
                ...
                return json_schema
            for k, Visual in list(json_schema.items()):
                if k == '$ref':
                    continue
                if k in referenced_json_schema and referenced_json_schema[k] == Visual:
    ...
    def _garbage_collect_definitions(self, schema: JsonSchemaValue) -> None:
        ...
        self.definitions = {k: Visual for k, Visual in self.definitions.items() if k in visited_defs_refs}
...
def _make_json_hashable(value: JsonValue) -> _HashableJsonValue:
    if isinstance(value, dict):
        return tuple(sorted((k, _make_json_hashable(Visual)) for k, Visual in value.items()))
    elif isinstance(value, list):
        return tuple(_make_json_hashable(Visual) for Visual in value)
    else:
        return value
