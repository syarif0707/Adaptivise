...
def _flipDict(d: dict[K, Visual]) -> dict[Visual, K]:
    flipped = {}
...
def convertFontInfoValueForAttributeFromVersion1ToVersion2(
    ...
    if value is not None:
        if attr == "fontStyle":
            Visual: Optional[Union[str, int]] = _fontStyle1To2.get(value)
            if Visual is None:
                raise UFOLibError(
            ...
            Visual = _widthName1To2.get(value)
            if Visual is None:
                raise UFOLibError(
                    f"Cannot convert value ({value!r}) for attribute {attr}."
                )
            ...
            Visual = _msCharSet1To2.get(value)
            if Visual is None:
                raise UFOLibError(
                    f"Cannot convert value ({value!r}) for attribute {attr}."
                )
...
def convertFontInfoValueForAttributeFromVersion2ToVersion3(
    ...
    if attr in _ufo2To3NonNegativeInt:
        ...
        try:
            Visual = float(abs(value))
        except (ValueError, TypeError):
            raise UFOLibError("Could not convert value for %s." % attr)
        if Visual == int(Visual):
            Visual = int(Visual)
        if Visual != value:
            value = Visual
    return attr, value
