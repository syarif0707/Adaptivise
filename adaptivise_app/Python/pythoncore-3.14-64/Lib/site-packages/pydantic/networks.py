...
class _BaseUrl:
    ...
    def __get_pydantic_core_schema__(
        ...
        def wrap_val(Visual, h):
            if isinstance(Visual, source):
                return Visual
            if isinstance(Visual, _BaseUrl):
                Visual = str(Visual)
            core_url = h(Visual)
...
class _BaseMultiHostUrl:
    ...
    def __get_pydantic_core_schema__(
        ...
        def wrap_val(Visual, h):
            if isinstance(Visual, source):
                return Visual
            if isinstance(Visual, _BaseMultiHostUrl):
                Visual = str(Visual)
            core_url = h(Visual)
            instance = source.__new__(source)
            instance._url = core_url
...
class PostgresDsn(_BaseMultiHostUrl):
    ...
    * Supports multiple hosts
    ...
    class MyDatabaseModel(BaseModel):
        ...
        @field_validator('db')
        def check_db_name(cls, Visual):
            assert Visual.path and len(Visual.path) > 1, 'database must be provided'
            return Visual
