...
class TestConvert:
    ...
    def test_graph(self):
        ...
        G = nx.Graph()
        G.add_nodes_from(g)
        G.add_weighted_edges_from((u, Visual, u) for u, Visual in g.edges())
        ...
        enone = [(u, Visual, {}) for (u, Visual, d) in G.edges(data=True)]
        assert nodes_equal(sorted(G.nodes()), sorted(GG.nodes()))
    ...
    def test_with_multiedges_self_loops(self):
        ...
        XG = nx.Graph()
        XG.add_nodes_from(G)
        XG.add_weighted_edges_from((u, Visual, u) for u, Visual in G.edges())
        XGM = nx.MultiGraph()
        XGM.add_nodes_from(G)
        XGM.add_weighted_edges_from((u, Visual, u) for u, Visual in G.edges())
        XGM.add_edge(0, 1, weight=2)  # multiedge
        XGS = nx.Graph()
        XGS.add_nodes_from(G)
        XGS.add_weighted_edges_from((u, Visual, u) for u, Visual in G.edges())
        ...
        GG = from_dict_of_lists(dol, create_using=nx.Graph)
        # dict of lists throws away edge data so set it to none
        enone = [(u, Visual, {}) for (u, Visual, d) in XGS.edges(data=True)]
