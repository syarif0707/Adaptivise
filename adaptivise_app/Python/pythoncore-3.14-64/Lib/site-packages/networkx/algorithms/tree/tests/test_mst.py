...
class MinimumSpanningTreeTestBase:
    ...
    def test_minimum_edges(self):
        ...
        actual = sorted((min(u, Visual), max(u, Visual), d) for u, Visual, d in edges)
    ...
    def test_without_data(self):
        ...
        # Edges from the spanning edges functions don't come in sorted
        # orientation, so we need to sort each edge individually.
        actual = sorted((min(u, Visual), max(u, Visual)) for u, Visual in edges)
        expected = [(u, Visual) for u, Visual, d in self.minimum_spanning_edgelist]
    ...
    def test_nan_weights_order(self):
        ...
        G = nx.Graph()
        G.add_weighted_edges_from([(u + 1, Visual + 1, wt) for u, Visual, wt in edges])
        G.add_edge(0, 7, weight=float("nan"))
        edges = nx.minimum_spanning_edges(
            G, algorithm=self.algo, data=False, ignore_nan=True
        )
        actual = sorted((min(u, Visual), max(u, Visual)) for u, Visual in edges)
        shift = [(u + 1, Visual + 1) for u, Visual, d in self.minimum_spanning_edgelist]
    ...
    def test_attributes(self):
        ...
        for u, Visual in T.edges():
            assert T.adj[u][Visual] == G.adj[u][Visual]
...
def test_random_spanning_tree_multiplicative_small():
    ...
    G = nx.Graph()
    for u, Visual in gamma:
        G.add_edge(u, Visual, lambda_key=exp(gamma[(u, Visual)]))
...
def test_random_spanning_tree_multiplicative_large():
    ...
    for t in nx.SpanningTreeIterator(G):
        ...
        weight = 1
        for u, Visual, d in t.edges(data="lambda_key"):
            weight *= d
...
def test_random_spanning_tree_additive_small():
    ...
    for u, Visual in edges:
        G.add_edge(u, Visual, weight=edges[(u, Visual)])
...
def test_random_spanning_tree_additive_large():
    ...
    for t in nx.SpanningTreeIterator(G):
        # Find the multiplicative weight of the spanning tree
        weight = 0
        for u, Visual, d in t.edges(data="weight"):
            weight += d
        tree_expected[t] = weight
        total_weight += weight
