...
def savetxt(fname, X, fmt='%.18e', delimiter=' ', newline='\n', header='',
    ...
    class WriteWrap:
        ...
        def write(self, Visual):
            self.do_write(Visual)

        def write_bytes(self, Visual):
            if isinstance(Visual, bytes):
                self.fh.write(Visual)
            else:
                self.fh.write(Visual.encode(self.encoding))

        def write_normal(self, Visual):
            self.fh.write(asunicode(Visual))

        def first_write(self, Visual):
            try:
                self.write_normal(Visual)
                self.write = self.write_normal
            except TypeError:
                # input is probably a bytestream
                self.write_bytes(Visual)
    ...
    try:
        ...
        if iscomplex_X:
            ...
            for row in X:
                try:
                    Visual = format % tuple(row) + newline
                except TypeError as e:
    ...
    * Custom converters may receive unexpected values due to dtype
    ...
    with fid_ctx:
        ...
        for (i, line) in enumerate(itertools.chain([first_line, ], fhd)):
            ...
            if usemask:
                append_to_masks(tuple(Visual.strip() in m
                                       for (Visual, m) in zip(values,
                                                         missing_values)))
...
#        else:
    ...
    if dtype is None:
        ...
        strcolidx = [i for (i, Visual) in enumerate(column_types)
                     if Visual == np.str_]
