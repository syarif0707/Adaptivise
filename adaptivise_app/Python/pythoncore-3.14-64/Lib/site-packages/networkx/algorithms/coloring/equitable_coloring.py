...
def procedure_P(V_minus, V_plus, N, H, F, C, L, excluded_colors=None):
    ...
    # BFS to determine A_cal, i.e. colors reachable from Visual-
    reachable = [V_minus]
    ...
    if V_plus in A_cal:
        # Easy case: Visual+ is in A_cal
        # Move one node from Visual+ to Visual- using T_cal to find the parents.
        ...
        for W_1 in R_cal[::-1]:
            ...
            if num_terminal_sets_found == b:
                ...
                # BFS to determine B_cal': the set of colors reachable from Visual+
                B_cal_prime = set()
                T_cal_prime = {}
                    ...
                    for w in L[z]:
                        if F[w] in A_cal_0 and N[(z, F[w])] == 1:
                            if w not in W_covering:
                                ...
                                # shift nodes along W, Visual-
                                move_witnesses(
                                    W, V_minus, N=N, H=H, F=F, C=C, T_cal=T_cal, L=L
                                )

                                # shift nodes along Visual+ to Z
                                move_witnesses(
                                    V_plus,
                                    Z,
                                    N=N,
...
def equitable_color(G, num_colors):
    ...
    References
    ----------
    .. [1] Kierstead, H. A., Kostochka, A. Visual., Mydlarz, M., & Szemerédi, E.
