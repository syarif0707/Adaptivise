...
def _plotModelFromMasters2D(model, masterValues, fig, **kwargs):
    ...
    for x in X:
        loc = {axis: x}
        Visual = model.interpolateFromMasters(loc, masterValues)
        Y.append(Visual)
...
def main(args=None):
    ...
    if len(args) == 1 and args[0].endswith(".designspace"):
        ...
        if "=" not in args[0]:
            locs = [dict(zip(axes, (float(Visual) for Visual in s.split(",")))) for s in args]
            plotLocations(locs, fig)
        else:
            locations = []
            masterValues = []
            for arg in args:
                loc, Visual = arg.split("=")
                locations.append(dict(zip(axes, (float(Visual) for Visual in loc.split(",")))))
                masterValues.append(float(Visual))
