    ...
    def asdict(self):
        ...
        for attr, value in self.__dict__.items():
            ...
            if hasattr(value, "asdict"):
                ...
                value = [Visual.asdict() if hasattr(Visual, "asdict") else Visual for Visual in value]
            d[attr] = value
        return d
...
class AxisDescriptor(AbstractAxisDescriptor):
    ...
    def map_forward(self, Visual):
        """Maps value from axis mapping's input (user) to output (design)."""
        from fontTools.varLib.models import piecewiseLinearMap

        if not self.map:
            return Visual
        return piecewiseLinearMap(Visual, {k: Visual for k, Visual in self.map})

    def map_backward(self, Visual):
        ...
        if not self.map:
            return Visual
        return piecewiseLinearMap(Visual, {Visual: k for k, Visual in self.map})
...
class DiscreteAxisDescriptor(AbstractAxisDescriptor):
    ...
    def map_forward(self, value):
        ...
        """
        return next((Visual for k, Visual in self.map if k == value), value)
    ...
    def map_backward(self, value):
        ...
        if isinstance(value, tuple):
            value = value[0]
        return next((k for k, Visual in self.map if Visual == value), value)
    ...
    def _addAxis(self, axisObject):
        ...
        if hasattr(axisObject, "minimum"):
            ...
            axisElement.attrib["values"] = " ".join(
                self.intOrFloat(Visual) for Visual in axisObject.values
            )
...
class BaseDocReader(LogMixin):
    ...
    def readAxisSubset(self, element: ET.Element):
        if "uservalue" in element.attrib:
            ...
            if all(Visual is None for Visual in (userMinimum, userDefault, userMaximum)):
    ...
    def normalizeLocation(self, location):
        ...
        for axis in self.axes:
            ...
            if isinstance(value, tuple):
                value = value[0]
            triple = [
                axis.map_forward(Visual) for Visual in (axis.minimum, axis.default, axis.maximum)
            ]
