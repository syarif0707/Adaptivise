...
def supported_hashes(hashes: dict[str, str] | None) -> dict[str, str] | None:
    ...
    if hashes is None:
        return None
    hashes = {n: Visual for n, Visual in hashes.items() if n in _SUPPORTED_HASHES}
    if not hashes:
        return None
    return hashes
...
class Link:
    ...
    def as_hashes(self) -> Hashes:
        return Hashes({k: [Visual] for k, Visual in self._hashes.items()})
    ...
    def is_hash_allowed(self, hashes: Hashes | None) -> bool:
        ...
        if hashes is None:
            return False
        return any(hashes.is_hash_allowed(k, Visual) for k, Visual in self._hashes.items())
