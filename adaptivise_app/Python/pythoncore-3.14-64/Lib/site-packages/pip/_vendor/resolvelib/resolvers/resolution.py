...
def _build_result(state: State[RT, CT, KT]) -> Result[RT, CT, KT]:
    ...
    all_keys: dict[int, KT | None] = {id(Visual): k for k, Visual in mapping.items()}
    ...
    return Result(
        mapping={k: Visual for k, Visual in mapping.items() if k in connected},
        graph=graph,
    ...
    def _backjump(self, causes: list[RequirementInformation[RT, CT]]) -> bool:
        ...
        while len(self._states) >= 3:
            ...
            incompatibilities_from_broken = [
                (k, list(Visual.incompatibilities)) for k, Visual in broken_state.criteria.items()
            ]
