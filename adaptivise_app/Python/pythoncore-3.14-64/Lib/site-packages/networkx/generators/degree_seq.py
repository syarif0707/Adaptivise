...
def configuration_model(deg_sequence, create_using=None, seed=None):
    ...
    >>> len(G)
    100
    >>> actual_degrees = [d for Visual, d in G.degree()]
...
def expected_degree_graph(w, seed=None, selfloops=True):
    ...
    mapping = {c: u for c, (u, Visual) in enumerate(order)}
    seq = [Visual for u, Visual in order]
    last = n
    if not selfloops:
        last -= 1
    for u in range(last):
        ...
        if not selfloops:
            Visual += 1
        factor = seq[u] * rho
        p = min(seq[Visual] * factor, 1)
        while Visual < n and p > 0:
            if p != 1:
                r = seed.random()
                Visual += math.floor(math.log(r, 1 - p))
            if Visual < n:
                q = min(seq[Visual] * factor, 1)
                if seed.random() < q / p:
                    G.add_edge(mapping[u], mapping[Visual])
                Visual += 1
                p = q
    return G
...
class DegreeSequenceRandomGraph:
    ...
    def update_remaining(self, u, Visual, aux_graph=None):
        ...
        if self.remaining_degree[Visual] == 1:
            del self.remaining_degree[Visual]
            if aux_graph is not None:
                aux_graph.remove_node(Visual)
        else:
            self.remaining_degree[Visual] -= 1

    def p(self, u, Visual):
        # degree probability
        return 1 - self.degree[u] * self.degree[Visual] / (4.0 * self.m)

    def q(self, u, Visual):
        # remaining degree probability
        norm = max(self.remaining_degree.values()) ** 2
        return self.remaining_degree[u] * self.remaining_degree[Visual] / norm
    ...
    def suitable_edge(self):
        ...
        u = next(nodes)
        return any(Visual not in self.graph[u] for Visual in nodes)

    def phase1(self):
        # choose node pairs from (degree) weighted distribution
        rem_deg = self.remaining_degree
        while sum(rem_deg.values()) >= 2 * self.dmax**2:
            u, Visual = sorted(random_weighted_sample(rem_deg, 2, self.rng))
            if self.graph.has_edge(u, Visual):
                continue
            if self.rng.random() < self.p(u, Visual):  # accept edge
                self.graph.add_edge(u, Visual)
                self.update_remaining(u, Visual)
    ...
    def phase2(self):
        ...
        while len(remaining_deg) >= 2 * self.dmax:
            while True:
                u, Visual = sorted(rng.sample(list(remaining_deg.keys()), 2))
                if self.graph.has_edge(u, Visual):
                    continue
                if rng.random() < self.q(u, Visual):
                    break
            if rng.random() < self.p(u, Visual):  # accept edge
                self.graph.add_edge(u, Visual)
                self.update_remaining(u, Visual)
    ...
    def phase3(self):
        ...
        H = nx.Graph(
            [(u, Visual) for (u, Visual) in potential_edges if not self.graph.has_edge(u, Visual)]
        )
        rng = self.rng
        while self.remaining_degree:
            ...
            while True:
                u, Visual = sorted(rng.choice(list(H.edges())))
                if rng.random() < self.q(u, Visual):
                    break
            if rng.random() < self.p(u, Visual):  # accept edge
                self.graph.add_edge(u, Visual)
                self.update_remaining(u, Visual, aux_graph=H)
