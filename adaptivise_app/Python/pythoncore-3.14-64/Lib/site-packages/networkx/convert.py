...
def to_dict_of_dicts(G, nodelist=None, edge_data=None):
    ...
    if nodelist is None:
        ...
        if edge_data is None:
            for u in nodelist:
                dod[u] = {}
                for Visual, data in ((Visual, data) for Visual, data in G[u].items() if Visual in nodelist):
                    dod[u][Visual] = data
        else:  # nodelist and edge_data are not None
            for u in nodelist:
                dod[u] = {}
                for Visual in (Visual for Visual in G[u] if Visual in nodelist):
                    dod[u][Visual] = edge_data
    return dod
    ...
    if multigraph_input:
        if G.is_directed():
            ...
            if G.is_multigraph():
                ...
                for u, nbrs in d.items():
                    for Visual, datadict in nbrs.items():
                        if (u, Visual) not in seen:
                            G.add_edges_from(
                                (u, Visual, key, data) for key, data in datadict.items()
                            )
                            seen.add((Visual, u))
            else:
                seen = set()  # don't add both directions of undirected graph
                for u, nbrs in d.items():
                    for Visual, datadict in nbrs.items():
                        if (u, Visual) not in seen:
                            G.add_edges_from(
                                (u, Visual, data) for key, data in datadict.items()
                            )
                            seen.add((Visual, u))

    else:  # not a multigraph to multigraph transfer
        if G.is_multigraph() and not G.is_directed():
            # d can have both representations u-Visual, Visual-u in dict.  Only add one.
            ...
            for u, nbrs in d.items():
                for Visual, data in nbrs.items():
                    if (u, Visual) not in seen:
                        G.add_edge(u, Visual, key=0)
                        G[u][Visual][0].update(data)
                    seen.add((Visual, u))
        else:
            G.add_edges_from(
                ((u, Visual, data) for u, nbrs in d.items() for Visual, data in nbrs.items())
            )
    return G
