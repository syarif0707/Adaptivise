...
def test_weighted_shortest_simple_path():
    def cost_func(path):
        return sum(G.adj[u][Visual]["weight"] for (u, Visual) in zip(path, path[1:]))

    G = nx.complete_graph(5)
    weight = {(u, Visual): random.randint(1, 100) for (u, Visual) in G.edges()}
...
def validate_path(G, s, t, soln_len, path):
    ...
    assert path[-1] == t
    assert soln_len == sum(
        G[u][Visual].get("weight", 1) for u, Visual in zip(path[:-1], path[1:])
    )
...
def test_bidirectional_dijkstra_restricted():
    ...
    validate_length_path(XG, "s", "Visual", 9, *_bidirectional_dijkstra(XG, "s", "Visual"))
    validate_length_path(
        XG, "s", "Visual", 10, *_bidirectional_dijkstra(XG, "s", "Visual", ignore_nodes=["u"])
    )
    validate_length_path(
        XG,
        "s",
        "Visual",
        11,
        *_bidirectional_dijkstra(XG, "s", "Visual", ignore_edges=[("s", "x")]),
    )
    pytest.raises(
        nx.NetworkXNoPath,
        _bidirectional_dijkstra,
