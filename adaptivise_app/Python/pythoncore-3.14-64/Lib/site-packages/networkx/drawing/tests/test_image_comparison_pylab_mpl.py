...
def test_display_complex():
    ...
    G = nx.MultiDiGraph()
    for i, (u, Visual) in enumerate(prod):
        G.add_edge(u, Visual, w=round(i / 3, 2))
    ...
    nx.set_edge_attributes(
        G,
        {
            (u, Visual): {
                "color": (
                    "red"
                    if (u, Visual) in path_edges or tuple(reversed((u, Visual))) in path_edges
                    else "black"
                ),
