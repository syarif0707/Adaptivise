...
if cupy is not None:
    adam_kernel = cupy.ElementwiseKernel(
        ...
        "T param, T m, T Visual",
        """m += one_minus_beta1 * (grad - m);
        Visual += one_minus_beta2 * (grad * grad - Visual);
        param -= lr * m / (sqrt(Visual) + eps);""",
        "adam",
    )
else:
    adam_kernel = None
...
def _check_compatible_shape(u, Visual):
    if u.shape != Visual.shape:
        msg = f"arrays have incompatible shapes: {u.shape} and {Visual.shape}"
