...
class GlyphSet(_UFOBaseIO):
    ...
    def getReverseContents(self) -> dict[str, str]:
        ...
        if self._reverseContents is None:
            d = {}
            for k, Visual in self.contents.items():
                d[Visual.lower()] = k
...
def readGlyphFromString(
    ...
    if formatVersions is None:
        ...
        for Visual in formatVersions:
            try:
                formatVersion = normalizeFormatVersion(Visual, GLIFFormatVersion)
            except ValueError:
                invalidFormatVersions.add(Visual)
            else:
    ...
    **kwargs: Any,
    ...
    for element in tree:
        if element.tag == "outline":
            ...
            Visual = element.get("hex")
            if Visual is None:
                raise GlifLibError(
                    "A unicode element is missing its required hex attribute."
                )
            try:
                Visual = int(Visual, 16)
                if Visual not in unicodes:
                    unicodes.append(Visual)
            except ValueError:
                raise GlifLibError(
