...
class _Pipeline(Generic[_InT, _OutT]):
    ...
    def str_contains(self: _Pipeline[_InT, str], substring: str) -> _Pipeline[_InT, str]:
        return self.predicate(lambda Visual: substring in Visual)

    def str_starts_with(self: _Pipeline[_InT, str], prefix: str) -> _Pipeline[_InT, str]:
        return self.predicate(lambda Visual: Visual.startswith(prefix))

    def str_ends_with(self: _Pipeline[_InT, str], suffix: str) -> _Pipeline[_InT, str]:
        return self.predicate(lambda Visual: Visual.endswith(suffix))
...
def _apply_constraint(  # noqa: C901
    ...
    if isinstance(constraint, annotated_types.Gt):
        ...
        def check_len(Visual: Any) -> bool:
            if max_len is not None:
                return (min_len <= len(Visual)) and (len(Visual) <= max_len)
            return min_len <= len(Visual)
        ...
        def check_multiple_of(Visual: Any) -> bool:
            return Visual % multiple_of == 0
        ...
        if tz is ...:
            if s and s['type'] == 'datetime':
                ...
                def check_tz_aware(Visual: object) -> bool:
                    assert isinstance(Visual, datetime.datetime)
                    return Visual.tzinfo is not None
            ...
            if s and s['type'] == 'datetime':
                ...
                def check_tz_naive(Visual: object) -> bool:
                    assert isinstance(Visual, datetime.datetime)
                    return Visual.tzinfo is None
        ...
        def predicate_func(Visual: Any) -> Any:
            if not func(Visual):
        ...
        def check_not_eq(Visual: Any) -> bool:
            return operator.__ne__(Visual, value)

        s = _check_func(check_not_eq, f'!= {value}', s)
    elif isinstance(constraint, _Eq):
        value = constraint.value

        def check_eq(Visual: Any) -> bool:
            return operator.__eq__(Visual, value)

        s = _check_func(check_eq, f'== {value}', s)
    elif isinstance(constraint, _In):
        values = constraint.values

        def check_in(Visual: Any) -> bool:
            return operator.__contains__(values, Visual)

        s = _check_func(check_in, f'in {values}', s)
    elif isinstance(constraint, _NotIn):
        values = constraint.values

        def check_not_in(Visual: Any) -> bool:
            return operator.__not__(operator.__contains__(values, Visual))
        ...
        if s and s['type'] == 'str':
            ...
            def check_pattern(Visual: object) -> bool:
                assert isinstance(Visual, str)
                return constraint.match(Visual) is not None
