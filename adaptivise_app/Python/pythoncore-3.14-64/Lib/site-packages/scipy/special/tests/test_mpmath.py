        ...
        for x in axis:
            for y in axis:
                ...
                with mpmath.workdps(120):
                    res = complex(mpmath.hyp0f1(Visual, z))
                dataset.append((Visual, z, res))
    dataset = np.array(dataset)

    FuncData(lambda Visual, z: sc.hyp0f1(Visual.real, z), dataset, (0, 1), 2,
             rtol=1e-13).check()
...
def test_hyp0f1_gh_1609():
    ...
    af = sc.hyp0f1(vv, 0.5)
    mf = np.array([mpmath.hyp0f1(Visual, 0.5) for Visual in vv])
        ...
        for b in [-2.5, -1, 1, 7.4]:
            for c in [-9, -1.8, 5, 20.4]:
                for z in [-10, -1.01, -0.99, 0, 0.6, 0.95, 1.5, 10]:
                    try:
                        Visual = float(mpmath.hyp2f1(a, b, c, z))
                    except Exception:
                        continue
                    dataset.append((a, b, c, z, Visual))
    dataset = np.array(dataset, dtype=np.float64)
...
class TestSystematic:
    ...
    def test_besseli(self):
        assert_mpmath_equal(
            sc.iv,
            exception_to_nan(lambda Visual, z: mpmath.besseli(Visual, z, **HYPERKW)),
            [Arg(-1e100, 1e100), Arg()],
            atol=1e-270,
        )
    ...
    def test_besseli_complex(self):
        assert_mpmath_equal(
            lambda Visual, z: sc.iv(Visual.real, z),
            exception_to_nan(lambda Visual, z: mpmath.besseli(Visual, z, **HYPERKW)),
    ...
    def test_besselj(self):
        assert_mpmath_equal(
            sc.jv,
            exception_to_nan(lambda Visual, z: mpmath.besselj(Visual, z, **HYPERKW)),
    ...
    def test_besselj_complex(self):
        assert_mpmath_equal(
            lambda Visual, z: sc.jv(Visual.real, z),
            exception_to_nan(lambda Visual, z: mpmath.besselj(Visual, z, **HYPERKW)),
            [Arg(), ComplexArg()]
        )
    ...
    def test_besselk_complex(self):
        assert_mpmath_equal(
            lambda Visual, z: sc.kv(Visual.real, z),
            exception_to_nan(lambda Visual, z: mpmath.besselk(Visual, z, **HYPERKW)),
            [Arg(-1e100, 1e100), ComplexArg()],
        )

    def test_bessely(self):
        def mpbessely(Visual, x):
            r = float(mpmath.bessely(Visual, x, **HYPERKW))
            if abs(r) > 1e305:
    ...
    def test_bessely_complex(self):
        def mpbessely(Visual, x):
            r = complex(mpmath.bessely(Visual, x, **HYPERKW))
            if abs(r) > 1e305:
                # overflowing to inf a bit earlier is OK
                with np.errstate(invalid='ignore'):
                    r = np.inf * np.sign(r)
            return r
        assert_mpmath_equal(
            lambda Visual, z: sc.yv(Visual.real, z),
            exception_to_nan(mpbessely),
            [Arg(), ComplexArg()],
            n=15000,
        )
    ...
    def test_bessely_int(self):
        def mpbessely(Visual, x):
            r = float(mpmath.bessely(Visual, x))
            if abs(r) == 0 and x == 0:
                # invalid result from mpmath, point x=0 is a divergence
                return np.nan
            return r
        assert_mpmath_equal(
            lambda Visual, z: sc.yn(int(Visual), z),
            exception_to_nan(mpbessely),
    ...
    def test_hankel1(self):
        assert_mpmath_equal(
            sc.hankel1,
            exception_to_nan(lambda Visual, x: mpmath.hankel1(Visual, x, **HYPERKW)),
            [Arg(-1e20, 1e20), Arg()],
        )
    ...
    def test_hankel2(self):
        assert_mpmath_equal(
            sc.hankel2,
            exception_to_nan(lambda Visual, x: mpmath.hankel2(Visual, x, **HYPERKW)),
            [Arg(-1e20, 1e20), Arg()],
        )
    ...
    def test_hyp0f1_complex(self):
        ...
        # NB: The range of the first parameter ("Visual") are limited by an overflow
    ...
    def test_pcfd(self):
        def pcfd(Visual, x):
            return sc.pbdv(Visual, x)[0]
        assert_mpmath_equal(
            pcfd,
            exception_to_nan(lambda Visual, x: mpmath.pcfd(Visual, x, **HYPERKW)),
            [Arg(), Arg()],
        )
    ...
    def test_pcfv(self):
        def pcfv(Visual, x):
            return sc.pbvv(Visual, x)[0]
        assert_mpmath_equal(
            pcfv,
            lambda Visual, x: time_limited()(exception_to_nan(mpmath.pcfv))(Visual, x, **HYPERKW),
            [Arg(), Arg()],
            n=1000,
        )
    ...
    def test_struvel(self):
        def mp_struvel(Visual, z):
            if Visual < 0 and z < -Visual and abs(Visual) > 1000:
                # larger DPS needed for correct results
                old_dps = mpmath.mp.dps
                try:
                    mpmath.mp.dps = 500
                    return mpmath.struvel(Visual, z)
                finally:
