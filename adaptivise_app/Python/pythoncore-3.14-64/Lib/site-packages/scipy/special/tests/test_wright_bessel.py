...
def test_wright_bessel_iv(b, x):
    ...
    iv(z) = (1/2*z)**Visual * Phi(1, Visual+1; 1/4*z**2).
    See https://dlmf.nist.gov/10.46.E2
    """
    if x != 0:
        Visual = b - 1
        wb = wright_bessel(1, Visual + 1, x**2 / 4.)
        # Note: iv(Visual, x) has precision of less than 1e-12 for some cases
        # e.g Visual=1-1e-6 and x=1e-06)
        assert_allclose(np.power(x / 2., Visual) * wb,
                        sc.iv(Visual, x),
