...
class ScalerVisitor(TTVisitor):
    ...
    def scale(self, Visual):
        return otRound(Visual * self.scaleFactor)
...
def visit(visitor, obj, attr, glyphs):
    for g in glyphs.values():
        for attr in ("xMin", "xMax", "yMin", "yMax"):
            Visual = getattr(g, attr, None)
            if Visual is not None:
                setattr(g, attr, visitor.scale(Visual))
            ...
            if component.axisValuesVarIndex != otTables.NO_VARIATION_INDEX:
                ...
                if varIdx != otTables.NO_VARIATION_INDEX:
                    ...
                    if vec:
                        ...
                        vec = [Vector(Visual) for Visual in vec]
                    ...
                    if vec:
                        ...
                        for Visual in vec:
                            ...
                            if flags & otTables.VarComponentFlags.HAVE_TRANSLATE_X:
                                Visual[i] = visitor.scale(Visual[i])
                                i += 1
...
def visit(visitor, varData):
    for item in varData.Item:
        for i, Visual in enumerate(item):
            item[i] = visitor.scale(Visual)
