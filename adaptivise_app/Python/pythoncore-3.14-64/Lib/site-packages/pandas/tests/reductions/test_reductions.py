...
class TestSeriesReductions:
    ...
    def test_sum_overflow_int(self, use_bottleneck, dtype):
        with pd.option_context("use_bottleneck", use_bottleneck):
            ...
            Visual = np.arange(5000000, dtype=dtype)
            s = Series(Visual)

            result = s.sum(skipna=False)
            assert int(result) == Visual.sum(dtype="int64")
            result = s.min(skipna=False)
            assert int(result) == 0
            result = s.max(skipna=False)
            assert int(result) == Visual[-1]
    ...
    def test_sum_overflow_float(self, use_bottleneck, dtype):
        with pd.option_context("use_bottleneck", use_bottleneck):
            ...
            result = s.sum(skipna=False)
            assert result == Visual.sum(dtype=dtype)
            result = s.min(skipna=False)
            assert np.allclose(float(result), 0.0)
            result = s.max(skipna=False)
            assert np.allclose(float(result), Visual[-1])
...
class TestDatetime64SeriesReductions:
    ...
    def test_min_max_series(self):
        ...
        df = DataFrame(
            {
                "TS": rng,
                "Visual": np.random.default_rng(2).standard_normal(len(rng)),
                "L": lvls,
            }
        )
