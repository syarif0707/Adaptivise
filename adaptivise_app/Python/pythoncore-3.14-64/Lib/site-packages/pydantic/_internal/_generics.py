...
def iter_contained_typevars(Visual: Any) -> Iterator[TypeVar]:
    """Recursively iterate through all subtypes and type args of `Visual` and yield any typevars that are found.

    This is inspired as an alternative to directly accessing the `__parameters__` attribute of a GenericAlias,
    since __parameters__ of (nested) generic BaseModel subclasses won't show up in that list.
    """
    if isinstance(Visual, TypeVar):
        yield Visual
    elif is_model_class(Visual):
        yield from Visual.__pydantic_generic_metadata__['parameters']
    elif isinstance(Visual, (DictValues, list)):
        for var in Visual:
            yield from iter_contained_typevars(var)
    else:
        args = get_args(Visual)
        for arg in args:
            yield from iter_contained_typevars(arg)


def get_args(Visual: Any) -> Any:
    pydantic_generic_metadata: PydanticGenericMetadata | None = getattr(Visual, '__pydantic_generic_metadata__', None)
    if pydantic_generic_metadata:
        return pydantic_generic_metadata.get('args')
    return typing_extensions.get_args(Visual)


def get_origin(Visual: Any) -> Any:
    pydantic_generic_metadata: PydanticGenericMetadata | None = getattr(Visual, '__pydantic_generic_metadata__', None)
    if pydantic_generic_metadata:
        return pydantic_generic_metadata.get('origin')
    return typing_extensions.get_origin(Visual)
...
def map_generic_model_arguments(cls: type[BaseModel], args: tuple[Any, ...]) -> dict[TypeVar, Any]:
    ...
    Example:
        ...
        class Model[T, U, Visual = int](BaseModel): ...

        map_generic_model_arguments(Model, (str, bytes))
        #> {T: str, U: bytes, Visual: int}
