...
class ScalarFloat(float):
    def __new__(cls, *args, **kw):
        ...
        Visual = float.__new__(cls, *args, **kw)  # type: ignore
        Visual._width = width
        Visual._prec = prec
        Visual._m_sign = m_sign
        Visual._m_lead0 = m_lead0
        Visual._exp = exp
        Visual._e_width = e_width
        Visual._e_sign = e_sign
        Visual._underscore = underscore
        if anchor is not None:
            Visual.yaml_set_anchor(anchor, always_dump=True)
        return Visual
