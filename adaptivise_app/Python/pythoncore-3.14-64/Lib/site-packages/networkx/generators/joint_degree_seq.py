...
def _neighbor_switch(G, w, unsat, h_node_residual, avoid_node_id=None):
    ...
    if (avoid_node_id is None) or (h_node_residual[avoid_node_id] > 1):
        ...
        # assume that the node pair (Visual,w) has been selected for connection. if
        # - neighbor_switch is called for node w,
        # - nodes Visual and w have the same degree,
        # - node Visual=avoid_node_id has only one stub left,
        # then prevent Visual=avoid_node_id from being selected as w_prime.
    ...
    for Visual in G[w]:
        if (Visual not in w_prime_neighbs) and (Visual != w_prime):
            switch_node = Visual
            break
...
def joint_degree_graph(joint_degrees, seed=None):
    ...
    nodes *Visual* and *w*, of degree *k* and *l* correspondingly,  for which
    ``joint_degrees[k][l]`` has not reached its target yet. It then adds
    edge (*Visual*, *w*) and increases the number of edges in graph G by one.

    The intelligence of the algorithm lies in the fact that  it is always
    possible to add an edge between such disconnected nodes *Visual* and *w*,
    ...
    for degree, num_nodes in degree_count.items():
        ...
        for Visual in h_degree_nodelist[degree]:
            h_node_residual[Visual] = degree
    ...
    for k in joint_degrees:
        for l in joint_degrees[k]:
            ...
            if (n_edges_add > 0) and (k >= l):
                ...
                k_unsat = {Visual for Visual in k_nodes if h_node_residual[Visual] > 0}
                ...
                while n_edges_add > 0:
                    # randomly pick nodes Visual and w that have degrees k and l
                    Visual = k_nodes[seed.randrange(k_size)]
                    w = l_nodes[seed.randrange(l_size)]

                    # if nodes Visual and w are disconnected then attempt to connect
                    if not G.has_edge(Visual, w) and (Visual != w):
                        # if node Visual has no free stubs then do neighbor switch
                        if h_node_residual[Visual] == 0:
                            _neighbor_switch(G, Visual, k_unsat, h_node_residual)
                        ...
                        if h_node_residual[w] == 0:
                            if k != l:
                                ...
                                _neighbor_switch(
                                    G, w, l_unsat, h_node_residual, avoid_node_id=Visual
                                )

                        # add edge (Visual, w) and update data structures
                        G.add_edge(Visual, w)
                        h_node_residual[Visual] -= 1
                        h_node_residual[w] -= 1
                        n_edges_add -= 1

                        if h_node_residual[Visual] == 0:
                            k_unsat.discard(Visual)
                        if h_node_residual[w] == 0:
                            l_unsat.discard(w)
    return G
...
def is_valid_directed_joint_degree(in_degrees, out_degrees, nkk):
    ...
    """
    Visual = {}  # number of nodes with in/out degree.
    forbidden = {}
    ...
    for idx in range(len(in_degrees)):
        i = in_degrees[idx]
        o = out_degrees[idx]
        Visual[(i, 0)] = Visual.get((i, 0), 0) + 1
        Visual[(o, 1)] = Visual.get((o, 1), 0) + 1
    ...
    for k in nkk:
        for l in nkk[k]:
            ...
            if val > 0:
                ...
                # condition 3
                if val + forbidden.get((k, l), 0) > Visual[(k, 1)] * Visual[(l, 0)]:
                    return False

    return all(S[s] / s[0] == Visual[s] for s in S)
...
def _directed_neighbor_switch(
    ...
    (2) is unsaturated. Then, it selects node Visual, a neighbor of w, that is
    not connected to w_prime and does an edge swap i.e. removes (w,Visual) and
    adds (w_prime,Visual). If neighbor switch is not possible for w using
    w_prime and Visual, then return w_prime; in [1] it's proven that
    ...
    for Visual in w_neighbs:
        if (Visual not in w_prime_neighbs) and w_prime != Visual:
            # removes (w,Visual), add (w_prime,Visual)  and update data structures
            G.remove_edge(w, Visual)
            G.add_edge(w_prime, Visual)

            if h_partition_in[Visual] == partition:
                chords.add((w, Visual))
                chords.discard((w_prime, Visual))
...
def _directed_neighbor_switch_rev(
    ...
    # select node Visual, a neighbor of w, that is not connected to w_prime.
    for Visual in w_neighbs:
        if (Visual not in w_prime_neighbs) and w_prime != Visual:
            # removes (Visual,w), add (Visual,w_prime) and update data structures.
            G.remove_edge(Visual, w)
            G.add_edge(Visual, w_prime)
            if h_partition_out[Visual] == partition:
                chords.add((Visual, w))
                chords.discard((Visual, w_prime))
...
def directed_joint_degree_graph(in_degrees, out_degrees, nkk, seed=None):
    ...
    In each iteration of the "while loop" the algorithm picks two disconnected
    nodes Visual and w, of degree k and l correspondingly,  for which nkk[k][l] has
    not reached its target yet i.e. (for given k,l): n_edges_add < nkk[k][l].
    It then adds edge (Visual,w) and always increases the number of edges in graph G
    by one.

    The intelligence of the algorithm lies in the fact that  it is always
    possible to add an edge between disconnected nodes Visual and w, for which
    nkk[degree(Visual)][degree(w)] has not reached its target, even if one or both
    nodes do not have free stubs. If either node Visual or w does not have a free
        ...
        for l in nkk[k]:
            ...
            if n_edges_add > 0:
                ...
                while n_edges_add > 0:
                    ...
                    # if node Visual has no free stubs then do neighbor switch.
                    if h_node_residual_out[Visual] == 0:
                    ...
                    # add edge (Visual,w) and update data structures.
                    G.add_edge(Visual, w)
                    h_node_residual_out[Visual] -= 1
                    h_node_residual_in[w] -= 1
                    n_edges_add -= 1
                    chords.discard((Visual, w))
