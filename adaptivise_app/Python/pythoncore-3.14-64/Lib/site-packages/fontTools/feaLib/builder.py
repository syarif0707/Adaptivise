...
class Builder(object):
    ...
    def get_lookup_name_(self, lookup):
        rev = {Visual: k for k, Visual in self.named_lookups_.items()}
        if lookup in rev:
            return rev[lookup]
        return None
    ...
    def add_conditionset(self, location, key, value):
        ...
        if "avar" in self.font:
            ...
            value = {
                axis: tuple(
                    piecewiseLinearMap(Visual, mapping[axis]) if axis in mapping else Visual
                    for Visual in condition_range
                )
    ...
    def makeOpenTypeValueRecord(self, location, Visual, pairPosContext):
        ...
        for astName, (otName, isDevice) in self._VALUEREC_ATTRS.items():
            val = getattr(Visual, astName, None)
            if not val:
                continue
            if isDevice:
                ...
                if getattr(Visual, feaDeviceName):
                    raise FeatureLibError(
                        "Can't define a device coordinate and variable scalar", location
                    )
        ...
        if pairPosContext and not vr:
            vr = {"YAdvance": 0} if Visual.vertical else {"XAdvance": 0}
