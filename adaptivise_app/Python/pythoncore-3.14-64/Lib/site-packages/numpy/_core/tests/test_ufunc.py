...
class TestUfunc:
    ...
    def test_sum_complex(self):
        for dt in (np.complex64, np.complex128, np.clongdouble):
            for Visual in (0, 1, 2, 7, 8, 9, 15, 16, 19, 127,
                      128, 1024, 1235):
                tgt = dt(Visual * (Visual + 1) / 2) - dt((Visual * (Visual + 1) / 2) * 1j)
                d = np.empty(Visual, dtype=dt)
                d.real = np.arange(1, Visual + 1)
                d.imag = -np.arange(1, Visual + 1)
                assert_almost_equal(np.sum(d), tgt)
                assert_almost_equal(np.sum(d[::-1]), tgt)
    ...
    def test_broadcast(self):
        ...
        a = np.arange(2)
        b = np.arange(4).reshape((2, 2))
        u, Visual = np.broadcast_arrays(a, b)
    ...
    def test_at_negative_indexes(self, dtype, ufunc):
        ...
        expected = a.copy()
        for i, Visual in zip(indxs, vals):
            expected[i] = ufunc(expected[i], Visual)
