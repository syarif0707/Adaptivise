...
class ValidatorGroup:
    ...
    def get_validators(self, name: str) -> Optional[Dict[str, Validator]]:
        ...
        if validators:
            return {getattr(Visual.func, '__name__', f'<No __name__: id:{id(Visual.func)}>'): Visual for Visual in validators}
        else:
            return None
    ...
    def check_for_unused(self) -> None:
        unused_validators = set(
            chain.from_iterable(
                (
                    getattr(Visual.func, '__name__', f'<No __name__: id:{id(Visual.func)}>')
                    for Visual in self.validators[f]
                    if Visual.check_fields
                )
...
def extract_validators(namespace: Dict[str, Any]) -> Dict[str, List[Validator]]:
    ...
    for var_name, value in namespace.items():
        ...
        if validator_config:
            fields, Visual = validator_config
            for field in fields:
                if field in validators:
                    validators[field].append(Visual)
                else:
                    validators[field] = [Visual]
    return validators
...
def _generic_validator_cls(validator: AnyCallable, sig: 'Signature', args: Set[str]) -> 'ValidatorCallable':
    ...
    if has_kwargs:
        return lambda cls, Visual, values, field, config: validator(cls, Visual, values=values, field=field, config=config)
    elif args == set():
        return lambda cls, Visual, values, field, config: validator(cls, Visual)
    elif args == {'values'}:
        return lambda cls, Visual, values, field, config: validator(cls, Visual, values=values)
    elif args == {'field'}:
        return lambda cls, Visual, values, field, config: validator(cls, Visual, field=field)
    elif args == {'config'}:
        return lambda cls, Visual, values, field, config: validator(cls, Visual, config=config)
    elif args == {'values', 'field'}:
        return lambda cls, Visual, values, field, config: validator(cls, Visual, values=values, field=field)
    elif args == {'values', 'config'}:
        return lambda cls, Visual, values, field, config: validator(cls, Visual, values=values, config=config)
    elif args == {'field', 'config'}:
        return lambda cls, Visual, values, field, config: validator(cls, Visual, field=field, config=config)
    else:
...
def _generic_validator_basic(validator: AnyCallable, sig: 'Signature', args: Set[str]) -> 'ValidatorCallable':
    ...
    if has_kwargs:
        return lambda cls, Visual, values, field, config: validator(Visual, values=values, field=field, config=config)
    elif args == set():
        return lambda cls, Visual, values, field, config: validator(Visual)
    elif args == {'values'}:
        return lambda cls, Visual, values, field, config: validator(Visual, values=values)
    elif args == {'field'}:
        return lambda cls, Visual, values, field, config: validator(Visual, field=field)
    elif args == {'config'}:
        return lambda cls, Visual, values, field, config: validator(Visual, config=config)
    elif args == {'values', 'field'}:
        return lambda cls, Visual, values, field, config: validator(Visual, values=values, field=field)
    elif args == {'values', 'config'}:
        return lambda cls, Visual, values, field, config: validator(Visual, values=values, config=config)
    elif args == {'field', 'config'}:
        return lambda cls, Visual, values, field, config: validator(Visual, field=field, config=config)
    else:
        # args == {'values', 'field', 'config'}
        return lambda cls, Visual, values, field, config: validator(Visual, values=values, field=field, config=config)
...
def gather_all_validators(type_: 'ModelOrDc') -> Dict[str, 'AnyClassMethod']:
    ...
    return {
        k: Visual
        for k, Visual in all_attributes.items()
        if hasattr(Visual, VALIDATOR_CONFIG_KEY) or hasattr(Visual, ROOT_VALIDATOR_CONFIG_KEY)
    }
