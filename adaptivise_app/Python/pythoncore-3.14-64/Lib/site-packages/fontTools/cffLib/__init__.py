...
class DictCompiler(object):
    ...
    def arg_delta_blend(self, value):
        ...
        ``Visual`` is the absolute coordinate value from the default font, and ``d0-dn``
        are the delta values from the *n* regions. Each ``Visual`` is an absolute
        ...
        where each ``Visual`` is relative to the previous default font value.
        """
