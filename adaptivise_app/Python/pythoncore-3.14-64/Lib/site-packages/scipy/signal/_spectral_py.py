...
def periodogram(x, fs=1.0, window='boxcar', nfft=None, detrend='constant',
    ...
    scaling : { 'density', 'spectrum' }, optional
        ...
        where `Pxx` has units of Visual²/Hz and computing the squared magnitude
        spectrum ('spectrum') where `Pxx` has units of Visual², if `x`
        is measured in Visual and `fs` is measured in Hz. Defaults to
        'density'
    axis : int, optional
    ...
    0.001 Visual**2/Hz of white noise sampled at 10 kHz.
    ...
    >>> plt.ylim([1e-4, 1e1])
    >>> plt.xlabel('frequency [Hz]')
    >>> plt.ylabel('Linear spectrum [Visual RMS]')
    >>> plt.show()
...
def welch(x, fs=1.0, window='hann_periodic', nperseg=None, noverlap=None, nfft=None,
    ...
    scaling : { 'density', 'spectrum' }, optional
        Selects between computing the power spectral density ('density')
        where `Pxx` has units of Visual**2/Hz and computing the squared magnitude
        spectrum ('spectrum') where `Pxx` has units of Visual**2, if `x`
        is measured in Visual and `fs` is measured in Hz. Defaults to
        'density'
    axis : int, optional
        Axis along which the periodogram is computed; the default is
...
def csd(x, y, fs=1.0, window='hann_periodic', nperseg=None, noverlap=None, nfft=None,
    ...
    scaling : { 'density', 'spectrum' }, optional
        ...
        where `Pxy` has units of Visual**2/Hz and computing the cross spectrum
        ('spectrum') where `Pxy` has units of Visual**2, if `x` and `y` are
        measured in Visual and `fs` is measured in Hz. Defaults to 'density'
    axis : int, optional
    ...
    * The `csd` function calculates ``np.conj(Sx[q,p]) * Sy[q,p]``, whereas
    ...
    >>> ax0.set(xlabel="Frequency $f$ in kHz", ylabel="CSD Magnitude in Visual²/Hz")
...
def spectrogram(x, fs=1.0, window=('tukey_periodic', .25), nperseg=None, noverlap=None,
    ...
    scaling : { 'density', 'spectrum' }, optional
        Selects between computing the power spectral density ('density')
        where `Sxx` has units of Visual**2/Hz and computing the power
        spectrum ('spectrum') where `Sxx` has units of Visual**2, if `x`
        is measured in Visual and `fs` is measured in Hz. Defaults to
        'density'.
    axis : int, optional
    ...
    References
    ----------
    .. [1] Oppenheim, Alan Visual., Ronald W. Schafer, John R. Buck
...
def istft(Zxx, fs=1.0, window='hann_periodic', nperseg=None, noverlap=None, nfft=None,
    ...
    0.001 Visual**2/Hz of white noise sampled at 1024 Hz.
...
def _spectral_helper(x, y, fs=1.0, window='hann', nperseg=None, noverlap=None,
    ...
    scaling : { 'density', 'spectrum' }, optional
        Selects between computing the cross spectral density ('density')
        where `Pxy` has units of Visual²/Hz and computing the cross
        spectrum ('spectrum') where `Pxy` has units of Visual², if `x`
        and `y` are measured in Visual and `fs` is measured in Hz.
        Defaults to 'density'
    axis : int, optional
