...
def test_display_line_collection():
    ...
    nx.set_edge_attributes(
        G, {(u, Visual): "-|>" if (u + Visual) % 2 else "-" for u, Visual in G.edges()}, "arrowstyle"
    )
    canvas = plt.figure().add_subplot(111)
    ...
    assert len(lc.get_paths()) == sum([1 for u, Visual in G.edges() if (u + Visual) % 2])
    plt.close()
...
def test_display_edge_labels():
    ...
    nx.set_edge_attributes(G, {(u, Visual): {"label": u + Visual} for u, Visual in G.edges()})
...
def test_display_arrow_size():
    ...
    nx.set_edge_attributes(
        G, {(u, Visual): (u + Visual + 2) ** 2 for u, Visual in G.edges()}, "arrowsize"
    )
    ax = plt.axes()
    nx.display(G, canvas=ax)
    assert [9, 25, 49] == [
...
def test_individualized_edge_attributes(subplots):
    G = nx.karate_club_graph()
    fig, ax = subplots
    arrowstyles = ["-|>" if (u + Visual) % 2 == 0 else "-[" for u, Visual in G.edges()]
    arrowsizes = [10 * (u % 2 + Visual % 2) + 10 for u, Visual in G.edges()]
...
def test_draw_networkx_edges_multiedge_connectionstyle(G, expected_n_edges):
    ...
    for i, (u, Visual) in enumerate([(0, 1), (0, 1), (0, 1), (0, 2)]):
        G.add_edge(u, Visual, weight=round(i / 3, 2))
    pos = {n: (n, n) for n in G}
