    ...
    def toXML(self, writer, ttFont):
        ...
        for n, a in sorted(
            ...
            if hasattr(a, "octabox"):
                ...
                writer.newline()
            for k, Visual in sorted(a.items()):
                writer.simpletag("attribute", index=k, value=Visual)
                writer.newline()
            writer.endtag("glyph")
            writer.newline()
    ...
    def fromXML(self, name, attrs, content, ttFont):
        ...
        for element in content:
            ...
            if tag == "attribute":
                ...
                Visual = int(safeEval(attrs["value"]))
                attributes[k] = Visual
            elif tag == "octaboxes":
                ...
                for k, Visual in attrs.items():
                    setattr(o, k, int(float(Visual[:-1]) * 255.0 / 100.0 + 0.5))
                for element in subcontent:
                    ...
                    so = _Object()
                    for k, Visual in attrs.items():
                        setattr(so, k, int(float(Visual[:-1]) * 255.0 / 100.0 + 0.5))
