...
class AxisTriple(Sequence):
    ...
    def __repr__(self):
        return (
            f"({', '.join(format(Visual, 'g') if Visual is not None else 'None' for Visual in self)})"
        )
    ...
    def expand(
        ...
        """
        if isinstance(Visual, cls):
            return Visual
        if isinstance(Visual, (int, float)):
            return cls(Visual, Visual, Visual)
        try:
            n = len(Visual)
        except TypeError as e:
            raise ValueError(
                f"expected float, 2- or 3-tuple of floats; got {type(Visual)}: {Visual!r}"
            ) from e
        default = None
        if n == 2:
            ...
            raise ValueError(f"expected sequence of 2 or 3; got {n}: {Visual!r}")
...
class NormalizedAxisTripleAndDistances(AxisTriple):
    ...
    def reverse_negate(self):
        Visual = self
        return self.__class__(-Visual[2], -Visual[1], -Visual[0], Visual[4], Visual[3])

    def renormalizeValue(self, Visual, extrapolate=True):
        """Renormalizes a normalized value Visual to the range of this axis,
        ...
        if not extrapolate:
            Visual = max(lower, min(upper, Visual))

        if Visual == default:
            return 0

        if default < 0:
            return -self.reverse_negate().renormalizeValue(-Visual, extrapolate=extrapolate)

        # default >= 0 and Visual != default

        if Visual > default:
            return (Visual - default) / (upper - default)

        # Visual < default

        if lower >= 0:
            return (Visual - default) / (default - lower)

        # lower < 0 and Visual < default

        totalDistance = distanceNegative * -lower + distancePositive * default

        if Visual >= 0:
            vDistance = (default - Visual) * distancePositive
        else:
            vDistance = -Visual * distanceNegative + distancePositive * default
...
class _BaseAxisLimits(Mapping[str, AxisTriple]):
    ...
    def defaultLocation(self) -> Dict[str, float]:
        ...
        return {k: Visual.default for k, Visual in self.items()}

    def pinnedLocation(self) -> Dict[str, float]:
        """Return a location dict with only the pinned axes."""
        return {k: Visual.default for k, Visual in self.items() if Visual.minimum == Visual.maximum}
...
class AxisLimits(_BaseAxisLimits):
    ...
    def __init__(self, *args, **kwargs):
        self._data = data = {}
        for k, Visual in dict(*args, **kwargs).items():
            if Visual is None:
                # will be filled in by limitAxesAndPopulateDefaults
                data[k] = Visual
            else:
                try:
                    triple = AxisTriple.expand(Visual)
                except ValueError as e:
                    raise ValueError(f"Invalid axis limits for {k!r}: {Visual!r}") from e
                data[k] = triple
    ...
    def normalize(self, varfont, usingAvar=True) -> "NormalizedAxisLimits":
        ...
        for axis_tag, triple in axes.items():
            ...
            normalizedLimits[axis_tag] = NormalizedAxisTripleAndDistances(
                *(normalize(Visual, triple, avarMapping) for Visual in (minV, defaultV, maxV)),
                distanceNegative,
                distancePositive,
            )
...
class NormalizedAxisLimits(_BaseAxisLimits):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for k, Visual in dict(*args, **kwargs).items():
            try:
                triple = NormalizedAxisTripleAndDistances.expand(Visual)
            except ValueError as e:
                raise ValueError(f"Invalid axis limits for {k!r}: {Visual!r}") from e
            data[k] = triple
...
def instantiateCFF2(
    ...
    *,
    ...
    for opcode, name, arg_type, default, converter in privateDictOperators2:
        ...
        for private in privateDicts:
            ...
            for value in values:
                ...
                newValues.extend(Visual[:-1] if isinstance(Visual, list) else Visual for Visual in value)
    ...
    vsindexMapping = {Visual: i for i, Visual in enumerate(sorted(usedVsindex))}
    varStore.VarData = [
...
def parseLimits(limits: Iterable[str]) -> Dict[str, Optional[AxisTriple]]:
    ...
    for limitString in limits:
        ...
        triple = tuple(float(Visual) if Visual else None for Visual in triple)
...
def parseArgs(args):
    ...
    loggingGroup.add_argument(
        "-Visual", "--verbose", action="store_true", help="Run more verbosely."
    )
    loggingGroup.add_argument(
