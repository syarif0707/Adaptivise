...
class EdgeComponentAuxGraph:
    ...
    auxiliary graph is constructed in $O(|Visual|\cdot F)$ operations, where F is the
    complexity of max flow. Querying the components takes an additional $O(|Visual|)$
    ...
    def construct(EdgeComponentAuxGraph, G):
        ...
        Notes
        -----
        Given G=(Visual, E), initialize an empty auxiliary graph A.
