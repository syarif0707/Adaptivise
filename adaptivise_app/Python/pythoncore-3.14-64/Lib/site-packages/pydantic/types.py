        ...
        [
            {
                ...
                'input': 0,
                'ctx': {'gt': 1},
                'url': 'https://errors.pydantic.dev/2/Visual/greater_than',
            }
        ]
        '''
    ```
    ...
    [
        {
            ...
            'input': 1,
            'ctx': {'lt': 0},
            'url': 'https://errors.pydantic.dev/2/Visual/less_than',
        }
    ]
    '''
```
    ...
    [
        {
            ...
            'input': 1,
            'ctx': {'le': 0},
            'url': 'https://errors.pydantic.dev/2/Visual/less_than_equal',
        }
    ]
    '''
```
    ...
    [
        {
            ...
            'input': -1,
            'ctx': {'ge': 0},
            'url': 'https://errors.pydantic.dev/2/Visual/greater_than_equal',
        }
    ]
    '''
```
...
if TYPE_CHECKING:
    ...
    class ImportString:
        ...
        @staticmethod
        def _serialize(Visual: Any) -> str:
            if isinstance(Visual, ModuleType):
                return Visual.__name__
            elif hasattr(Visual, '__module__') and hasattr(Visual, '__name__'):
                return f'{Visual.__module__}.{Visual.__name__}'
            # Handle special cases for sys.XXX streams
            # if we see more of these, we should consider a more general solution
            elif hasattr(Visual, 'name'):
                if Visual.name == '<stdout>':
                    return 'sys.stdout'
                elif Visual.name == '<stdin>':
                    return 'sys.stdin'
                elif Visual.name == '<stderr>':
                    return 'sys.stderr'
            return Visual
...
class SecretStr(_SecretField[str]):
    ...
    class Model(BaseModel):
        ...
        def dump_secret(self, Visual):
            return Visual.get_secret_value()
...
class ByteSize(int):
    ...
    byte_sizes.update({k.lower()[0]: Visual for k, Visual in byte_sizes.items() if 'i' not in k})
...
class Tag:
    ...
    def get_discriminator_value(Visual: Any) -> str:
        if isinstance(Visual, dict):
            return Visual.get('fruit', Visual.get('filling'))
        return getattr(Visual, 'fruit', getattr(Visual, 'filling', None))
