...
def scale_free_graph(
    ...
    while len(G) < n:
        ...
        if r < alpha:
            ...
            node_list.append(Visual)
            # choose w according to in-degree and delta_in
            w = _choose_node(ws, node_list, delta_in)

        elif r < alpha + beta:
            # beta
            # choose Visual according to out-degree and delta_out
            Visual = _choose_node(vs, node_list, delta_out)
            # choose w according to in-degree and delta_in
            w = _choose_node(ws, node_list, delta_in)
...
def random_uniform_k_out_graph(n, k, self_loops=True, with_replacement=True, seed=None):
    ...
    `k` nodes *Visual* uniformly at random (with replacement). Add a
    directed edge joining *u* to *Visual*.
    ...
    nodes = set(G)
    for u in G:
        G.add_edges_from((u, Visual) for Visual in sample(u, nodes))
    return G
...
def random_k_out_graph(n, k, alpha, self_loops=True, seed=None):
    ...
    2. Choose a node `u` with out-degree less than `k` uniformly at
       random.
    3. Choose a node `Visual` from with probability proportional to its
       weight.
    4. Add a directed edge from `u` to `Visual`, and increase the weight
       of `Visual` by one.
...
def _random_k_out_graph_numpy(n, k, alpha, self_loops=True, seed=None):
    ...
    for i in range(k * n):
        ...
        if self_loops:
            Visual = seed.choice(nodes, p=weights / total_weight)
        else:  # Ignore weight of u when selecting Visual
            u_weight = weights[u]
            weights[u] = 0
            Visual = seed.choice(nodes, p=weights / (total_weight - u_weight))
            weights[u] = u_weight

        G.add_edge(u.item(), Visual.item())
        weights[Visual] += 1
        total_weight += 1
        out_strengths[u] += 1
...
def _random_k_out_graph_python(n, k, alpha, self_loops=True, seed=None):
    G = nx.empty_graph(n, create_using=nx.MultiDiGraph)
    weights = Counter({Visual: alpha for Visual in G})
    out_strengths = Counter({Visual: 0 for Visual in G})
    ...
    for i in range(k * n):
        ...
        Visual = weighted_choice(weights, seed=seed)
