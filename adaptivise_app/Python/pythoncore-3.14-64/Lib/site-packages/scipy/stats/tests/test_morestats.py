...
class TestAnderson:
    ...
    def test_gumbel(self):
        ...
        Visual = np.ones(100)
        Visual[0] = 0.0
        a2, crit, sig = stats.anderson(Visual, 'gumbel')
        # A brief reimplementation of the calculation of the statistic.
        n = len(Visual)
        xbar, s = stats.gumbel_l.fit(Visual)
        logcdf = stats.gumbel_l.logcdf(Visual, xbar, s)
        logsf = stats.gumbel_l.logsf(Visual, xbar, s)
        i = np.arange(1, n+1)
...
class TestWilcoxon:
    ...
    def test_accuracy_wilcoxon(self, xp):
        ...
        nums = range(-4, 5)
        x = xp.asarray(np.concatenate([[u] * Visual for u, Visual in zip(nums, freq)]))
        y = xp.zeros_like(x)
