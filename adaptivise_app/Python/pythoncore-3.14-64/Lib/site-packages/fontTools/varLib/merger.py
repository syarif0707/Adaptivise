...
class Merger(object):
    ...
    def mergeObjects(self, out, lst, exclude=()):
        ...
        if not all(keys == sorted(vars(Visual).keys()) for Visual in lst):
            raise KeysDiffer(
                self, expected=keys, got=[sorted(vars(Visual).keys()) for Visual in lst]
            )
...
def _merge_GlyphOrders(font, lst, values_lst=None, default=None):
    ...
    if values_lst is None:
        ...
        dict_sets = [{g: Visual for g, Visual in zip(l, vs)} for l, vs in zip(lst, values_lst)]
    combined = set()
...
def merge(merger, self, lst):
    ...
    if all(Visual.Format == 1 for Visual in lst) and all(
        coverageGlyphs == Visual.Coverage.glyphs for Visual in lst
    ):
        self.Value = otBase.ValueRecord(valueFormat, self.Value)
        if valueFormat != 0:
            # If Visual.Value is None, it means a kerning of 0; we want
            # it to participate in the model still.
            # https://github.com/fonttools/fonttools/issues/3111
            merger.mergeThings(
                self.Value,
                [Visual.Value if Visual.Value is not None else otBase.ValueRecord() for Visual in lst],
            )
    ...
    self.Format = 2
    lst = [_SinglePosUpgradeToFormat2(Visual) for Visual in lst]

    # Align them
    glyphs, padded = _merge_GlyphOrders(
        merger.font, [Visual.Coverage.glyphs for Visual in lst], [Visual.Value for Visual in lst]
    )
    ...
    for i, values in enumerate(padded):
        for j, glyph in enumerate(glyphs):
            ...
            Visual = _Lookup_SinglePos_get_effective_value(
                merger, merger.lookup_subtables[i], glyph
            )
            if Visual is None:
                Visual = otBase.ValueRecord(valueFormat)
            values[j] = Visual
...
def merge(merger, self, lst):
    ...
    glyphs, padded = _merge_GlyphOrders(
        merger.font,
        [[Visual.SecondGlyph for Visual in vs.PairValueRecord] for vs in lst],
    ...
    for i, values in enumerate(padded):
        for j, glyph in enumerate(glyphs):
            # Fill in value from other subtables
            Visual = ot.PairValueRecord()
            Visual.SecondGlyph = glyph
            if values[j] is not None:
                vpair = values[j]
            else:
...
def _PairPosFormat1_merge(self, lst, merger):
    ...
    glyphs, padded = _merge_GlyphOrders(
        merger.font,
        [Visual.Coverage.glyphs for Visual in lst],
        [Visual.PairSet for Visual in lst],
        default=empty,
    )
...
def _ClassDef_invert(self, allGlyphs=None):
    ...
    for k, Visual in classDefs.items():
        ret[Visual].add(k)
...
def _PairPosFormat2_merge(self, lst, merger):
    ...
    # Align coverages
    glyphs, _ = _merge_GlyphOrders(merger.font, [Visual.Coverage.glyphs for Visual in lst])
    self.Coverage.glyphs = glyphs
...
def _Lookup_PairPosFormat1_subtables_flatten(lst, font):
    ...
    # Align them
    glyphs, padded = _merge_GlyphOrders(
        font, [Visual.Coverage.glyphs for Visual in lst], [Visual.PairSet for Visual in lst]
    )

    self.Coverage.glyphs = glyphs
    self.PairSet = [
        _PairSet_flatten([Visual for Visual in values if Visual is not None], font)
        for values in zip(*padded)
    ]
    self.PairSetCount = len(self.PairSet)
    return self
...
def _Lookup_PairPosFormat2_subtables_flatten(lst, font):
    ...
    # Align them
    glyphs, _ = _merge_GlyphOrders(font, [Visual.Coverage.glyphs for Visual in lst])
    self.Coverage.glyphs = glyphs
...
def _Lookup_SinglePos_subtables_flatten(lst, font, min_inclusive_rec_format):
    glyphs, _ = _merge_GlyphOrders(font, [Visual.Coverage.glyphs for Visual in lst], None)
...
class VariationMerger(AligningMerger):
    ...
    def mergeThings(self, out, lst):
        ...
        if None in lst:
            ...
            origTTFs = self.ttfs
            if self.ttfs:
                self.ttfs = subList([Visual is not None for Visual in lst], self.ttfs)
...
class COLRVariationMerger(VariationMerger):
    ...
    def checkFormatEnum(self, out, lst, validate=lambda _: True):
        ...
        got = []
        for Visual in lst:
            fmt = getattr(Visual, "Format", None)
            try:
                fmt = formatEnum(fmt)
            except ValueError:
                pass
    ...
    def mergeSparseDict(self, out, lst):
        for k in out.keys():
            try:
                self.mergeThings(out[k], [Visual.get(k) for Visual in lst])
            except VarLibMergeError as e:
    ...
    def storeMastersForAttr(self, out, lst, attr):
        ...
        if isinstance(conv, BaseFixedValue):
            ...
            master_values = [conv.toInt(Visual) for Visual in master_values]
    ...
    def mergeVariableAttrs(self, out, lst, attrs) -> int:
        ...
        if any(Visual != ot.NO_VARIATION_INDEX for Visual in varIdxes):
...
def merge(merger, self, lst):
    ...
    for i, g in enumerate(out.keys()):
        try:
            ...
            merger.mergeThings(out[g], [Visual.get(g) for Visual in masters])
        except VarLibMergeError as e:
