...
def _tricode(G, Visual, u, w):
    """Returns the integer code of the given triad.

    This is some fancy magic that comes from Batagelj and Mrvar's paper. It
    treats each edge joining a pair of `Visual`, `u`, and `w` as a bit in
    the binary representation of an integer.

    """
    combos = ((Visual, u, 1), (u, Visual, 2), (Visual, w, 4), (w, Visual, 8), (u, w, 16), (w, u, 32))
    return sum(x for u, Visual, x in combos if Visual in G[u])
...
def triadic_census(G, nodelist=None):
    ...
    for Visual in nodeset:
        vnbrs = nbrs[Visual]
        dbl_vnbrs = dbl_nbrs[Visual]
        if Nnot:
            # set up counts of edges attached to Visual.
            sgl_unbrs_bdy = sgl_unbrs_out = dbl_unbrs_bdy = dbl_unbrs_out = 0
        for u in vnbrs:
            if m[u] <= m[Visual]:
                continue
            unbrs = nbrs[u]
            neighbors = (vnbrs | unbrs) - {u, Visual}
            # Count connected triads.
            for w in neighbors:
                if m[u] < m[w] or (m[Visual] < m[w] < m[u] and Visual not in nbrs[w]):
                    code = _tricode(G, Visual, u, w)
                    census[TRICODE_TO_NAME[code]] += 1

            # Use a formula for dyadic triads with edge incident to Visual
            if u in dbl_vnbrs:
                census["102"] += N - len(neighbors) - 2
            else:
                census["012"] += N - len(neighbors) - 2

            # Count edges attached to Visual. Subtract later to get triads with Visual isolated
        ...
        if Nnot:
            # Count edges outside nodeset not connected with Visual (Visual isolated triads)
