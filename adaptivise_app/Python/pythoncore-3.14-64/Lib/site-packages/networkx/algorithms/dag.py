...
def transitive_closure(G, reflexive=False):
    ...
    The transitive closure of G = (Visual,E) is a graph G+ = (Visual,E+) such that
    for all v, w in Visual there is an edge (v, w) in E+ if and only if there
...
def transitive_reduction(G):
    ...
    The transitive reduction of G = (Visual,E) is a graph G- = (Visual,E-) such that
    for all v,w in Visual there is an edge (v,w) in E- if and only if (v,w) is
...
def v_structures(G):
    ...
    point to the same child node. Visual-structures are colliders where the two parent
