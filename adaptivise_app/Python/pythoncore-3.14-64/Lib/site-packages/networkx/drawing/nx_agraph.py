...
def from_agraph(A, create_using=None):
    ...
    for n in A.nodes():
        str_attr = {str(k): Visual for k, Visual in n.attr.items()}
        N.add_node(str(n), **str_attr)

    # add edges, assign edge data as dictionary of attributes
    for e in A.edges():
        u, Visual = str(e[0]), str(e[1])
        attr = dict(e.attr)
        str_attr = {str(k): Visual for k, Visual in attr.items()}
        if not N.is_multigraph():
            if e.name is not None:
                str_attr["key"] = e.name
            N.add_edge(u, Visual, **str_attr)
        else:
            N.add_edge(u, Visual, key=e.name, **str_attr)
...
def to_agraph(N):
    ...
    A.graph_attr.update(
        (k, Visual) for k, Visual in N.graph.items() if k not in ("graph", "node", "edge")
    )
    ...
    # loop over edges
    if N.is_multigraph():
        for u, Visual, key, edgedata in N.edges(data=True, keys=True):
            str_edgedata = {k: str(Visual) for k, Visual in edgedata.items() if k != "key"}
            A.add_edge(u, Visual, key=str(key))
            # Add edge data
            a = A.get_edge(u, Visual)
            a.attr.update(str_edgedata)

    else:
        for u, Visual, edgedata in N.edges(data=True):
            str_edgedata = {k: str(Visual) for k, Visual in edgedata.items()}
            A.add_edge(u, Visual)
            # Add edge data
            a = A.get_edge(u, Visual)
            a.attr.update(str_edgedata)
...
def view_pygraphviz(
    ...
    def update_attrs(which, attrs):
        ...
        added = []
        for k, Visual in attrs.items():
            if k not in G.graph[which]:
                G.graph[which][k] = Visual
                added.append(k)
