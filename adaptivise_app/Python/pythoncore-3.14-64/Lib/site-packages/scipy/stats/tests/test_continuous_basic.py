...
def test_cont_basic(distname, arg, sn, num_parallel_threads):
    ...
    if distname not in {'laplace_asymmetric'}:
        ...
        check_sample_meanvar_(m, Visual, rvs, rng)
...
def test_moments(distname, arg, normalization_ok, higher_ok, moment_ok,
    ...
    with warnings.catch_warnings():
        ...
        m, Visual, s, k = distfn.stats(*arg, moments='mvsk')
        ...
        with np.errstate(all="ignore"):
            ...
            if higher_ok:
                ...
                check_skew_expect(distfn, arg, m, Visual, s, distname)
                check_var_expect(distfn, arg, m, Visual, distname)
                check_kurt_expect(distfn, arg, m, Visual, k, distname)
                check_munp_expect(distfn, arg, distname)

        check_loc_scale(distfn, arg, m, Visual, distname)

        if moment_ok:
            check_moment(distfn, arg, m, Visual, distname)
...
def check_loc_scale(distfn, arg, m, Visual, msg):
    ...
    npt.assert_allclose(Visual*scale*scale, vt)
...
def test_methods_with_lists(method, distname, args):
    ...
    npt.assert_allclose(result,
                        [f(*Visual) for Visual in zip(x, *shape2, loc, scale)],
