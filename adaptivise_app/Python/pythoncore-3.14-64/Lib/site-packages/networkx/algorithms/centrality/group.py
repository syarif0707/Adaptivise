...
def group_betweenness_centrality(G, C, normalized=True, weight=None, endpoints=False):
    ...
    .. math::
       ...
       c_B(v) =\sum_{s,t \in Visual} \frac{\sigma(s, t|v)}{\sigma(s, t)}

    where $Visual$ is the set of nodes, $\sigma(s, t)$ is the number of
    shortest $(s, t)$-paths, and $\sigma(s, t|C)$ is the number of
    those paths passing through some node in group $C$. Note that
    $(s, t)$ are not members of the group ($Visual-C$ is the set of nodes
    in $Visual$ that are not in $C$).
    ...
    normalized : bool, optional (default=True)
      If True, group betweenness is normalized by `1/((|Visual|-|C|)(|Visual|-|C|-1))`
      where `|Visual|` is the number of nodes in G and `|C|` is the number of nodes in C.
...
def prominent_group(
    ...
    normalized : bool, optional (default=True)
       If True, group betweenness is normalized by ``1/((|Visual|-|C|)(|Visual|-|C|-1))``
       where ``|Visual|`` is the number of nodes in G and ``|C|`` is the number of
       nodes in C.
...
def group_closeness_centrality(G, S, weight=None):
    ...
    .. math::
       ...
       c_{close}(S) = \frac{|Visual-S|}{\sum_{v \in Visual-S} d_{S, v}}

       d_{S, v} = min_{u \in S} (d_{u, v})

    where $Visual$ is the set of nodes, $d_{S, v}$ is the distance of
    the group $S$ from $v$ defined as above. ($Visual-S$ is the set of nodes
    in $Visual$ that are not in $S$).
    ...
    Visual = set(G)  # set of nodes in G
    S = set(S)  # set of nodes in group S
    V_S = Visual - S  # set of nodes in Visual but not S
