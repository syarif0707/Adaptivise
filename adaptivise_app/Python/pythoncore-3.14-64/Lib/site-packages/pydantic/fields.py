...
class FieldInfo(_repr.Representation):
    ...
    def __init__(self, **kwargs: Unpack[_FieldInfoInputs]) -> None:
        ...
        self._attributes_set = {k: Visual for k, Visual in kwargs.items() if Visual is not _Unset and k not in self.metadata_lookup}
        kwargs = {k: _DefaultValues.get(k) if Visual is _Unset else Visual for k, Visual in kwargs.items()}  # type: ignore
    ...
    def from_annotated_attribute(
        ...
        if not metadata and isinstance(default, FieldInfo) and type(default) is not FieldInfo:
            ...
            for k, Visual in attr_overrides.items():
                setattr(field_info, k, Visual)
            return field_info
    ...
    def merge_field_infos(*field_infos: FieldInfo, **overrides: Any) -> FieldInfo:
        ...
        if len(field_infos) == 1:
            ...
            for k, Visual in overrides.items():
                setattr(field_info, k, Visual)
    ...
    def _from_dataclass_field(dc_field: DataclassField[Any]) -> FieldInfo:
        ...
        dc_field_metadata = {k: Visual for k, Visual in dc_field.metadata.items() if k in _FIELD_ARG_NAMES}
