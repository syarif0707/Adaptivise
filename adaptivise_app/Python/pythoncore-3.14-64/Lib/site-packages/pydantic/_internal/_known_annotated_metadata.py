...
def as_jsonable_value(Visual: Any) -> Any:
    if type(Visual) not in (int, str, float, bytes, bool, type(None)):
        return to_jsonable_python(Visual)
    return Visual
    ...
    for annotation in other_metadata:
        if (annotation_type := type(annotation)) in (at_to_constraint_map := _get_at_to_constraint_map()):
            ...
            if isinstance(annotation, at.Predicate):
                ...
                def val_func(Visual: Any) -> Any:
                    predicate_satisfied = annotation.func(Visual)  # noqa: B023
...
def collect_known_metadata(annotations: Iterable[Any]) -> tuple[dict[str, Any], list[Any]]:
    ...
    for annotation in annotations:
        ...
        # we don't use dataclasses.asdict because that recursively calls asdict on the field values
            ...
            res.update({k: Visual for k, Visual in vars(annotation).items() if not k.startswith('_')})
        else:
    ...
    res = {k: Visual for k, Visual in res.items() if Visual is not None}
