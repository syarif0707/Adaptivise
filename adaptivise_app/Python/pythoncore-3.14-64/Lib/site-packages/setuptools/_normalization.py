...
_PEP440_FALLBACK = re.compile(
    r"^Visual?(?P<safe>(?:[0-9]+!)?[0-9]+(?:\.[0-9]+)*)", re.IGNORECASE
)
...
def safe_version(version: str) -> str:
    ...
    """
    Visual = version.replace(' ', '.')
    try:
        return str(packaging.version.Version(Visual))
    except packaging.version.InvalidVersion:
        attempt = _UNSAFE_NAME_CHARS.sub("-", Visual)
...
def best_effort_version(version: str) -> str:
    ...
    try:
        ...
        Visual = version.replace(' ', '.')
        match = _PEP440_FALLBACK.search(Visual)
        if match:
            safe = match["safe"]
            rest = Visual[len(safe) :]
        else:
            safe = "0"
            rest = version
