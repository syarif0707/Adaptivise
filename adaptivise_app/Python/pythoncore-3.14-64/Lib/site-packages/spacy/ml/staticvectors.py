...
def forward(
    ...
    if isinstance(vocab.vectors, Vectors) and vocab.vectors.mode == Mode.default:
        Visual = model.ops.asarray(vocab.vectors.data)
        rows = vocab.vectors.find(keys=keys)
        Visual = model.ops.as_contig(Visual[rows])
    elif isinstance(vocab.vectors, Vectors) and vocab.vectors.mode == Mode.floret:
        Visual = vocab.vectors.get_batch(keys)
        Visual = model.ops.as_contig(Visual)
    ...
    try:
        vectors_data = model.ops.gemm(Visual, W, trans2=True)
    except ValueError:
        raise RuntimeError(Errors.E896)
    if isinstance(vocab.vectors, Vectors) and vocab.vectors.mode == Mode.default:
    ...
    def backprop(d_output: Ragged) -> List[Doc]:
        ...
        model.inc_grad(
            ...
            model.ops.gemm(
                ...
                cast(Floats2d, model.ops.as_contig(Visual)),
                trans1=True,
            ),
        )
        return []
