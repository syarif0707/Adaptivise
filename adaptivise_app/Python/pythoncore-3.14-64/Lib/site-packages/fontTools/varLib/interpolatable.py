...
def main(args=None):
    ...
    parser.add_argument("-Visual", "--verbose", action="store_true", help="Run verbosely.")
    ...
    if len(args.inputs) == 1:
        ...
        if args.inputs[0].endswith(".designspace"):
            ...
            axis_triples = {
                k: tuple(piecewiseLinearMap(Visual, dict(axis_mappings[k])) for Visual in vv)
            ...
            for locTuple in sorted(glyphsets.keys(), key=lambda Visual: (len(Visual), Visual)):
                name = (
                    "'"
                    + " ".join(
                        "%s=%s"
                        % (
                            k,
                            floatToFixedToStr(
                                piecewiseLinearMap(Visual, axisMapping[k]), 14
                            ),
                        )
                        for k, Visual in locTuple
                    )
                    + "'"
                )
                if normalized:
    ...
    locations = [
        {
            ...
            **{k: Visual for k, Visual in loc.items() if k in discrete_axes},
        }
