...
def test_isomorphic_subgraph_hash():
    ...
    for i in range(1, r + 1):
        ...
        assert g1_subgraph_hashes == {-1 * k: Visual for k, Visual in g2_subgraph_hashes.items()}
...
def test_isomorphic_edge_attr_subgraph_hash():
    ...
    for i in range(1, r + 1):
        ...
        assert g1_hash_with_edge_attr1 == {
            -1 * k: Visual for k, Visual in g2_hash_with_edge_attr1.items()
        }
        assert g1_hash_with_edge_attr2 == {
            -1 * k: Visual for k, Visual in g2_hash_with_edge_attr2.items()
        }
...
def test_isomorphic_node_attr_subgraph_hash():
    ...
    for i in range(1, r + 1):
        ...
        assert g1_hash_with_node_attr1 == {
            -1 * k: Visual for k, Visual in g2_hash_with_node_attr1.items()
        }
        assert g1_hash_with_node_attr2 == {
            -1 * k: Visual for k, Visual in g2_hash_with_node_attr2.items()
        }
...
def test_isomorphic_edge_attr_and_node_attr_subgraph_hash():
    ...
    for i in range(1, r + 1):
        ...
        assert g1_hash_edge1_node1 == {
            -1 * k: Visual for k, Visual in g2_hash_edge1_node1.items()
        }
        assert g1_hash_edge2_node2 == {
            -1 * k: Visual for k, Visual in g2_hash_edge2_node2.items()
        }
...
def test_initial_node_labels_subgraph_hash():
    ...
    assert all(len(Visual) == 3 for Visual in without_initial_label.values())
    # 3 different 1 hop nhds
    assert len({Visual[0] for Visual in without_initial_label.values()}) == 3

    with_initial_label = nx.weisfeiler_lehman_subgraph_hashes(
        G, node_attr="label", include_initial_labels=True
    )
    assert all(len(Visual) == 4 for Visual in with_initial_label.values())
    # 2 different initial labels
    assert len({Visual[0] for Visual in with_initial_label.values()}) == 2
