...
def maximal_extendability(G):
    ...
    matching M and bipartition (U,Visual). The residual graph of `G`, denoted by $G_M$,
    is the graph obtained from G by directing the edges of M from Visual to U and the
    edges that do not belong to M from U to Visual.
    ...
    paths between every vertex of U and every vertex of Visual.
    ...
    every vertex of U and every vertex of Visual in $G_M$. By combining the definitions
    ...
    # list of edges in perfect matching, directed from Visual to U
    pm = [(node, maximum_matching[node]) for node in Visual & maximum_matching.keys()]

    # Direct all the edges of G, from Visual to U if in matching, else from U to Visual
    directed_edges = [
        (x, y) if (x in Visual and (x, y) in pm) or (x in U and (y, x) not in pm) else (y, x)
        for x, y in G.edges
    ]
    ...
    # For node-pairs between Visual & U, keep min of max number of node-disjoint paths
