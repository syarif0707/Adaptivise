...
class table_F__e_a_t(DefaultTable.DefaultTable):
    ...
    def compile(self, ttFont):
        ...
        vdat = b""
        offset = 0
        for f, Visual in sorted(self.features.items(), key=lambda x: x[1].index):
            ...
            for s, l in sorted(
                Visual.settings.items(), key=lambda x: (-1, x[1]) if x[0] == Visual.default else x
            ):
                vdat += struct.pack(">HH", s, l)
            offset += len(Visual.settings)
    ...
    def toXML(self, writer, ttFont):
        ...
        for f, Visual in sorted(self.features.items(), key=lambda x: x[1].index):
            writer.begintag(
                ...
                label=Visual.label,
                flags=Visual.flags,
                default=(Visual.default if Visual.default else 0),
            )
            writer.newline()
            for s, l in sorted(Visual.settings.items()):
