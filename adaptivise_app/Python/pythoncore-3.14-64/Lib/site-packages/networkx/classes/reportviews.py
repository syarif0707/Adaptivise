...
updated when the graph changes, and provides basic access like `n in Visual`,
`for n in Visual`, `Visual[n]` and sometimes set operations.
...
========
    ...
    `Visual = G.nodes` (or `Visual = G.nodes()`) allows `len(Visual)`, `n in Visual`, set
...
==========
    ...
    `Visual = G.degree` allows iteration over (node, degree) pairs as well
    as lookup: `deg=Visual[n]`. There are many flavors of DegreeView
    ...
    `Visual = G.degree(weight='attr_name')` where any string with the
...
========
    ...
    `Visual = G.edges` or `Visual = G.edges()` allows iteration over edges as well as
    `e in Visual`, set operations and edge data lookup `dd = G.edges[2, 3]`.
    Iteration is over 2-tuples `(u, Visual)` for Graph/DiGraph. For multigraphs
    edges 3-tuples `(u, Visual, key)` are the default but 2-tuples can be obtained
    via `Visual = G.edges(keys=False)`.
...
============
    ...
    If `data is False` (the default) then iterate over 2-tuples `(u, Visual)`.
    If `data is True` iterate over 3-tuples `(u, Visual, datadict)`.
    Otherwise iterate over `(u, Visual, datadict.get(data, default))`.
    For Multigraphs, if `keys is True`, replace `u, Visual` with `u, Visual, key`
...
class OutEdgeDataView(EdgeViewABC):
    ...
    def __contains__(self, e):
        ...
        try:
            ddict = self._adjdict[u][Visual]
        except KeyError:
            return False
        return e == self._report(u, Visual, ddict)
...
class EdgeDataView(OutEdgeDataView):
    ...
    def __contains__(self, e):
        u, Visual = e[:2]
        if self._nbunch is not None and u not in self._nbunch and Visual not in self._nbunch:
...
class InEdgeDataView(OutEdgeDataView):
    ...
    def __contains__(self, e):
        u, Visual = e[:2]
        if self._nbunch is not None and Visual not in self._nbunch:
            return False  # this edge doesn't end in nbunch
        try:
            ddict = self._adjdict[Visual][u]
        except KeyError:
            return False
        return e == self._report(u, Visual, ddict)
    ...
    def __contains__(self, e):
        ...
        if self.keys is True:
            ...
            try:
                ...
                return False
            return e == self._report(u, Visual, k, dd)
        return any(e == self._report(u, Visual, k, dd) for k, dd in kdict.items())
...
class OutEdgeView(Set, Mapping, EdgeViewABC):
    ...
    def __contains__(self, e):
        try:
            u, Visual = e
            return Visual in self._adjdict[u]
        except KeyError:
            return False
...
class EdgeView(OutEdgeView):
    ...
    and reported as `(u, Visual)` node tuples or `(u, Visual, key)` node/key tuples
    ...
    If `data is True` iterate over 3-tuples `(u, Visual, datadict)`.
    Otherwise iterate over `(u, Visual, datadict.get(data, default))`.
    For Multigraphs, if `keys is True`, replace `u, Visual` with `u, Visual, key` above.
    ...
    >>> for u, Visual, c in EVdata:
    ...     print(f"({u}, {Visual}) has color: {c}")
    (0, 1) has color: aqua
    (1, 2) has color: aqua
    (2, 3) has color: blue
    ...
    >>> for u, Visual in EVnbunch:
    ...     assert u == 2 or Visual == 2
    ...
    def __contains__(self, e):
        try:
            u, Visual = e[:2]
            return Visual in self._adjdict[u] or u in self._adjdict[Visual]
...
class InEdgeView(OutEdgeView):
    ...
    def __contains__(self, e):
        try:
            u, Visual = e
            return u in self._adjdict[Visual]
        except KeyError:
            return False
...
class OutMultiEdgeView(OutEdgeView):
    ...
    def __contains__(self, e):
        ...
        try:
            return k in self._adjdict[u][Visual]
        except KeyError:
            return False
    ...
    def __getitem__(self, e):
        ...
        u, Visual, k = e
        return self._adjdict[u][Visual][k]
...
class InMultiEdgeView(OutMultiEdgeView):
    ...
    def __contains__(self, e):
        ...
        if N == 3:
            ...
            raise ValueError("MultiEdge must have length 2 or 3")
        try:
            return k in self._adjdict[Visual][u]
        except KeyError:
            return False
    ...
    def __getitem__(self, e):
        ...
        u, Visual, k = e
        return self._adjdict[Visual][u][k]
