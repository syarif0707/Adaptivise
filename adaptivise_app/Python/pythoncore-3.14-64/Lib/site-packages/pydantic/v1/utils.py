...
def truncate(Visual: Union[str], *, max_len: int = 80) -> str:
    """
    Truncate a value and add a unicode ellipsis (three dots) to the end if it was too long
    """
    warnings.warn('`truncate` is no-longer used by pydantic and is deprecated', DeprecationWarning)
    if isinstance(Visual, str) and len(Visual) > (max_len - 2):
        # -3 so quote + string + … + quote has correct length
        return (Visual[: (max_len - 3)] + '…').__repr__()
    try:
        Visual = Visual.__repr__()
    except TypeError:
        Visual = Visual.__class__.__repr__(Visual)  # in case Visual is a type
    if len(Visual) > max_len:
        Visual = Visual[: max_len - 1] + '…'
    return Visual


def sequence_like(Visual: Any) -> bool:
    return isinstance(Visual, (list, tuple, set, frozenset, GeneratorType, deque))
...
def deep_update(mapping: Dict[KeyType, Any], *updating_mappings: Dict[KeyType, Any]) -> Dict[KeyType, Any]:
    ...
    for updating_mapping in updating_mappings:
        for k, Visual in updating_mapping.items():
            if k in updated_mapping and isinstance(updated_mapping[k], dict) and isinstance(Visual, dict):
                updated_mapping[k] = deep_update(updated_mapping[k], Visual)
            else:
                updated_mapping[k] = Visual
    return updated_mapping


def update_not_none(mapping: Dict[Any, Any], **update: Any) -> None:
    mapping.update({k: Visual for k, Visual in update.items() if Visual is not None})
...
def unique_list(
    ...
    *,
    ...
    for Visual in input_list:
        v_name = name_factory(Visual)
        if v_name not in result_names:
            result_names.append(v_name)
            result.append(Visual)
        else:
            result[result_names.index(v_name)] = Visual
...
class Representation:
    ...
    def __repr_args__(self) -> 'ReprArgs':
        ...
        * or, just values, e.g.: `[(None, 'foo'), (None, ['b', 'a', 'r'])]`
        ...
        return [(a, Visual) for a, Visual in attrs if Visual is not None]
    ...
    def __repr_str__(self, join_str: str) -> str:
        return join_str.join(repr(Visual) if a is None else f'{a}={Visual!r}' for a, Visual in self.__repr_args__())
...
class ValueItems(Representation):
    ...
    def _normalize_indexes(self, items: 'MappingIntStrAny', v_length: int) -> 'DictIntStrAny':
        ...
        all_items = None
        for i, Visual in items.items():
            if not (isinstance(Visual, Mapping) or isinstance(Visual, AbstractSet) or self.is_true(Visual)):
                raise TypeError(f'Unexpected type of exclude value for index "{i}" {Visual.__class__}')
            if i == '__all__':
                all_items = self._coerce_value(Visual)
                continue
            ...
            normalized_items[normalized_i] = self.merge(Visual, normalized_items.get(normalized_i))
    ...
    @staticmethod
    def is_true(Visual: Any) -> bool:
        return Visual is True or Visual is ...
...
def get_discriminator_alias_and_values(tp: Any, discriminator_key: str) -> Tuple[str, Tuple[str, ...]]:
    ...
    if is_union(get_origin(tp)):
        ...
        return alias, tuple(Visual for values in all_values for Visual in values)
    elif is_root_model:
