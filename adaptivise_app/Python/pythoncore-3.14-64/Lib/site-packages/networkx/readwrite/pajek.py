...
def generate_pajek(G):
    ...
    for n in nodes:
        ...
        for k, Visual in na.items():
            if isinstance(Visual, str) and Visual.strip() != "":
                s += f" {make_qstr(k)} {make_qstr(Visual)}"
            else:
                warnings.warn(
                    f"Node attribute {k} is not processed. {('Empty attribute' if isinstance(Visual, str) else 'Non-string attribute')}."
                )
        yield s
    ...
    if G.is_directed():
        ...
        yield "*edges"
    for u, Visual, edgedata in G.edges(data=True):
        d = edgedata.copy()
        value = d.pop("weight", 1.0)  # use 1 as default edge value
        s = " ".join(map(make_qstr, (nodenumber[u], nodenumber[Visual], value)))
        for k, Visual in d.items():
            if isinstance(Visual, str) and Visual.strip() != "":
                s += f" {make_qstr(k)} {make_qstr(Visual)}"
            else:
                warnings.warn(
                    f"Edge attribute {k} is not processed. {('Empty attribute' if isinstance(Visual, str) else 'Non-string attribute')}."
                )
        yield s
    ...
    while lines:
        ...
        if l.lower().startswith("*network"):
            ...
            for l in lines:
                ...
                # if G.has_edge(u,Visual):
                #     multigraph=True
                G.add_edge(u, Visual, **edge_data)
