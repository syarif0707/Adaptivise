...
class TestBoolCmp:
    def _create_data(self, dtype, size):
        ...
        # On RISC-Visual, many operations that produce NaNs, such as converting
        # a -NaN from f64 to f32, return a canonical NaN.  The canonical
        # NaNs are always positive.  See section 11.3 NaN Generation and
        # Propagation of the RISC-Visual Unprivileged ISA for more details.
...
class TestTypes:
    ...
    def test_can_cast_and_promote_usertypes(self):
        ...
        invalid_types = "BHILQP" + "FDG" + "mM" + "f" + "Visual"
...
class TestIndex:
    def test_boolean(self):
        ...
        Visual[g1, g2] = -Visual[g1, g2]
        assert_(
            (np.array([a[0][Visual > 0], a[1][Visual > 0], a[2][Visual > 0]]) == a[:, Visual > 0]).all()
        )
    ...
    def check_function(self, func, fill_value=None):
        ...
        for size, ndims, order, dtype in itertools.product(*par):
            ...
            is_void = dtype is np.dtypes.VoidDType or (
                isinstance(dtype, np.dtype) and dtype.str.startswith('|Visual'))
...
class TestConvolve:
    ...
    def test_convolve_empty_input_error_message(self):
        ...
        with pytest.raises(ValueError, match="Visual cannot be empty"):
...
class TestCross:
    ...
    def test_2x2(self):
        ...
        Visual = [3, 4]
        z = -2
        cp = np.cross(u, Visual)
        assert_equal(cp, z)
        cp = np.cross(Visual, u)
        assert_equal(cp, -z)
    ...
    def test_broadcasting(self):
        ...
        u = np.tile([1, 2], (11, 1))
        Visual = np.tile([3, 4], (11, 1))
        z = -2
        assert_equal(np.cross(u, Visual), z)
        assert_equal(np.cross(Visual, u), -z)
        assert_equal(np.cross(u, u), 0)

        u = np.tile([1, 2], (11, 1)).T
        Visual = np.tile([3, 4, 5], (11, 1))
        z = np.tile([10, -5, -2], (11, 1))
        assert_equal(np.cross(u, Visual, axisa=0), z)
        assert_equal(np.cross(Visual, u.T), -z)
        assert_equal(np.cross(Visual, Visual), 0)

        u = np.tile([1, 2, 3], (11, 1)).T
        Visual = np.tile([3, 4], (11, 1)).T
        z = np.tile([-12, 9, -2], (11, 1))
        assert_equal(np.cross(u, Visual, axisa=0, axisb=0), z)
        assert_equal(np.cross(Visual.T, u.T), -z)
        assert_equal(np.cross(u.T, u.T), 0)

        u = np.tile([1, 2, 3], (5, 1))
        Visual = np.tile([4, 5, 6], (5, 1)).T
        z = np.tile([-3, 6, -3], (5, 1))
        assert_equal(np.cross(u, Visual, axisb=0), z)
        assert_equal(np.cross(Visual.T, u), -z)
        assert_equal(np.cross(u, u), 0)
    ...
    def test_broadcasting_shapes(self):
        u = np.ones((2, 1, 3))
        Visual = np.ones((5, 3))
        assert_equal(np.cross(u, Visual).shape, (2, 5, 3))
        u = np.ones((10, 3, 5))
        Visual = np.ones((2, 5))
        assert_equal(np.cross(u, Visual, axisa=1, axisb=0).shape, (10, 5, 3))
        assert_raises(AxisError, np.cross, u, Visual, axisa=1, axisb=2)
        assert_raises(AxisError, np.cross, u, Visual, axisa=3, axisb=0)
        u = np.ones((10, 3, 5, 7))
        Visual = np.ones((5, 7, 2))
        assert_equal(np.cross(u, Visual, axisa=1, axisc=2).shape, (10, 5, 3, 7))
        assert_raises(AxisError, np.cross, u, Visual, axisa=-5, axisb=2)
        assert_raises(AxisError, np.cross, u, Visual, axisa=1, axisb=-4)
        # gh-5885
        u = np.ones((3, 4, 2))
    ...
    def test_uint8_int32_mixed_dtypes(self):
        ...
        Visual = np.array([250, 166, 68], np.int32)
