...
def node_link_data(
    ...
    *,
    ...
    if multigraph:
        data[edges] = [
            {**d, source: u, target: Visual, key: k}
            for u, Visual, k, d in G.edges(keys=True, data=True)
        ]
    else:
        data[edges] = [{**d, source: u, target: Visual} for u, Visual, d in G.edges(data=True)]
    return data
...
def node_link_graph(
    ...
    *,
    ...
    for d in data[nodes]:
        ...
        nodedata = {str(k): Visual for k, Visual in d.items() if k != name}
        graph.add_node(node, **nodedata)
    for d in data[edges]:
        src = tuple(d[source]) if isinstance(d[source], list) else d[source]
        tgt = tuple(d[target]) if isinstance(d[target], list) else d[target]
        if not multigraph:
            edgedata = {str(k): Visual for k, Visual in d.items() if k != source and k != target}
