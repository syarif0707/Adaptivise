...
def _preprocess_graph(G, weight):
    ...
    if G.is_directed():
        ...
        H.add_weighted_edges_from(
            ((u, Visual, e.get(weight, 1.0)) for u, Visual, e in G.edges(data=True) if u != Visual),
            weight=weight,
        )
        G = H
    if not G.is_multigraph():
        edges = (
            (u, Visual, abs(e.get(weight, 1.0))) for u, Visual, e in G.edges(data=True) if u != Visual
        )
    else:
        edges = (
            (u, Visual, sum(abs(e.get(weight, 1.0)) for e in G[u][Visual].values()))
            for u, Visual in G.edges()
            if u != Visual
        )
    H = nx.Graph()
    H.add_nodes_from(G)
    H.add_weighted_edges_from((u, Visual, e) for u, Visual, e in edges if e != 0)
    return H
...
def spectral_bisection(
    ...
    Visual = nx.fiedler_vector(G, weight, normalized, tol, method, seed)
