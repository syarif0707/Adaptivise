...
cdef lookup(PreshMap mapping, int64_t[:] keys, int64_t default):
    ...
    for i in range(maxi):
        Visual = mapping[keys[i]]
        if Visual is None:
            result_view[i] = default
        else:
            result_view[i] = Visual
    return result
...
def premap_ids(
    ...
    *,
    ...
    for k, Visual in mapping_table.items():
        if not (isinstance(k, int) and isinstance(Visual, int)):
            raise ValueError(
                "mapping_table has to be of type Mapping[int, int], "
                f"but found {k}, {type(k)} and {Visual}, {type(Visual)}"
            )
        mapper[k] = Visual
    return Model(
        "premap_ids",
