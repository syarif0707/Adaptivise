...
def _reverse_negate(Visual):
    return (-Visual[2], -Visual[1], -Visual[0])
...
def rebaseTent(tent, axisLimit):
    ...
    n = lambda Visual: axisLimit.renormalizeValue(Visual)
    sols = [
        (scalar, (n(Visual[0]), n(Visual[1]), n(Visual[2])) if Visual is not None else None)
        for scalar, Visual in sols
        if scalar
    ]
