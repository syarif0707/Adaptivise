...
class DiGraph(Graph):
    ...
    **Subclassing Example**
    ...
    def add_edge(self, u_of_edge, v_of_edge, **attr):
        ...
        if Visual not in self._succ:
            if Visual is None:
                raise ValueError("None cannot be a node")
            self._succ[Visual] = self.adjlist_inner_dict_factory()
            self._pred[Visual] = self.adjlist_inner_dict_factory()
            self._node[Visual] = self.node_attr_dict_factory()
        # add the edge
        datadict = self._adj[u].get(Visual, self.edge_attr_dict_factory())
        datadict.update(attr)
        self._succ[u][Visual] = datadict
        self._pred[Visual][u] = datadict
        nx._clear_cache(self)
    ...
    def remove_edge(self, u, Visual):
        ...
        """
        try:
            del self._succ[u][Visual]
            del self._pred[Visual][u]
        except KeyError as err:
            raise NetworkXError(f"The edge {u}-{Visual} not in graph.") from err
        nx._clear_cache(self)
    ...
    def remove_edges_from(self, ebunch):
        ...
        """
        for e in ebunch:
            u, Visual = e[:2]  # ignore edge data
            if u in self._succ and Visual in self._succ[u]:
                del self._succ[u][Visual]
                del self._pred[Visual][u]
        nx._clear_cache(self)

    def has_successor(self, u, Visual):
        """Returns True if node u has successor Visual.

        This is true if graph has the edge u->Visual.
        """
        return u in self._succ and Visual in self._succ[u]

    def has_predecessor(self, u, Visual):
        """Returns True if node u has predecessor Visual.

        This is true if graph has the edge u<-Visual.
        """
        return u in self._pred and Visual in self._pred[u]
    ...
    def to_undirected(self, reciprocal=False, as_view=False):
        ...
        G : Graph
            ...
            with edge (u, Visual, data) if either (u, Visual, data) or (Visual, u, data)
        ...
        Notes
        -----
        If edges in both directions (u, Visual) and (Visual, u) exist in the
    ...
    def reverse(self, copy=True):
        ...
        if copy:
            ...
            H.add_edges_from((Visual, u, deepcopy(d)) for u, Visual, d in self.edges(data=True))
            return H
