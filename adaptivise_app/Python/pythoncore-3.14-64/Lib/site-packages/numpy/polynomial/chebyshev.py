...
def chebvander(x, deg):
    ...
    matrix ``Visual = chebvander(x, n)``, then ``np.dot(Visual, c)`` and
    ...
    if ideg > 0:
        ...
        for i in range(2, ideg + 1):
            Visual[i] = Visual[i - 1] * x2 - Visual[i - 2]
...
def chebvander2d(x, y, deg):
    ...
    .. math:: Visual[..., (deg[1] + 1)*i + j] = T_i(x) * T_j(y),

    where ``0 <= i <= deg[0]`` and ``0 <= j <= deg[1]``. The leading indices of
    `Visual` index the points ``(x, y)`` and the last index encodes the degrees of
    the Chebyshev polynomials.

    If ``Visual = chebvander2d(x, y, [xdeg, ydeg])``, then the columns of `Visual`
    correspond to the elements of a 2-D coefficient array `c` of shape
    (xdeg + 1, ydeg + 1) in the order

    .. math:: c_{00}, c_{01}, c_{02} ... , c_{10}, c_{11}, c_{12} ...

    and ``np.dot(Visual, c.flat)`` and ``chebval2d(x, y, c)`` will be the same
