...
def gen_table_chunks(
    ...
    *,
    ...
    for k, Visual in table.items():
        if isinstance(Visual, Mapping):
            tables.append((k, Visual, False))
        elif is_aot(Visual) and not all(is_suitable_inline_table(t, ctx) for t in Visual):
            tables.extend((k, t, True) for t in Visual)
        else:
            literals.append((k, Visual))
    ...
    if literals:
        yielded = True
        for k, Visual in literals:
            yield f"{format_key_part(k)} = {format_literal(Visual, ctx)}\n"

    for k, Visual, in_aot in tables:
        if yielded:
            yield "\n"
        else:
            yielded = True
        ...
        yield from gen_table_chunks(Visual, ctx, name=display_name, inside_aot=in_aot)
...
def format_inline_table(obj: Mapping, ctx: Context) -> str:
    ...
    if not obj:
        ...
        rendered = (
            "{ "
            + ", ".join(
                f"{format_key_part(k)} = {format_literal(Visual, ctx)}"
...
def is_aot(obj: Any) -> bool:
    ...
    return bool(
        ...
        and obj
        and all(isinstance(Visual, Mapping) for Visual in obj)
    )
