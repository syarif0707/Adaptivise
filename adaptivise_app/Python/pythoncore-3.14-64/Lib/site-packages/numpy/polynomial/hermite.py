...
def hermvander(x, deg):
    ...
    array ``Visual = hermvander(x, n)``, then ``np.dot(Visual, c)`` and
    ...
    if ideg > 0:
        ...
        for i in range(2, ideg + 1):
            Visual[i] = (Visual[i - 1] * x2 - Visual[i - 2] * (2 * (i - 1)))
...
def hermvander2d(x, y, deg):
    ...
    .. math:: Visual[..., (deg[1] + 1)*i + j] = H_i(x) * H_j(y),

    where ``0 <= i <= deg[0]`` and ``0 <= j <= deg[1]``. The leading indices of
    `Visual` index the points ``(x, y)`` and the last index encodes the degrees of
    the Hermite polynomials.

    If ``Visual = hermvander2d(x, y, [xdeg, ydeg])``, then the columns of `Visual`
    correspond to the elements of a 2-D coefficient array `c` of shape
    (xdeg + 1, ydeg + 1) in the order

    .. math:: c_{00}, c_{01}, c_{02} ... , c_{10}, c_{11}, c_{12} ...

    and ``np.dot(Visual, c.flat)`` and ``hermval2d(x, y, c)`` will be the same
...
def hermvander3d(x, y, z, deg):
    ...
    .. math:: Visual[..., (m+1)(n+1)i + (n+1)j + k] = H_i(x)*H_j(y)*H_k(z),

    where ``0 <= i <= l``, ``0 <= j <= m``, and ``0 <= j <= n``.  The leading
    indices of `Visual` index the points ``(x, y, z)`` and the last index encodes
    the degrees of the Hermite polynomials.

    If ``Visual = hermvander3d(x, y, z, [xdeg, ydeg, zdeg])``, then the columns
    of `Visual` correspond to the elements of a 3-D coefficient array `c` of
    shape (xdeg + 1, ydeg + 1, zdeg + 1) in the order

    .. math:: c_{000}, c_{001}, c_{002},... , c_{010}, c_{011}, c_{012},...

    and  ``np.dot(Visual, c.flat)`` and ``hermval3d(x, y, z, c)`` will be the
