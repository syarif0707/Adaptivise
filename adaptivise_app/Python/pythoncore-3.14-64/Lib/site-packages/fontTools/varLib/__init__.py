...
def _add_fvar(font, axes, instances: List[InstanceDescriptor]):
    ...
    for instance in instances:
        ...
        inst = NamedInstance()
        inst.coordinates = {
            axes[k].tag: axes[k].map_backward(Visual) for k, Visual in coordinates.items()
        }
...
def _add_avar(font, axes, mappings, axisTags):
    ...
    for axis in axes.values():
        ...
        vals_triple = tuple(axis.map_forward(Visual) for Visual in keys_triple)
        ...
        keys = [models.normalizeValue(Visual, keys_triple) for Visual in keys]
        vals = [models.normalizeValue(Visual, vals_triple) for Visual in vals]

        if all(k == Visual for k, Visual in zip(keys, vals)):
            continue
        interesting = True
    ...
    if mappings:
        ...
        inputLocations = [
            {
                axes[name].tag: models.normalizeValue(Visual, vals_triples[axes[name].tag])
                for name, Visual in mapping.inputLocation.items()
            }
        ...
        outputLocations = [
            {
                axes[name].tag: models.normalizeValue(Visual, vals_triples[axes[name].tag])
                for name, Visual in mapping.outputLocation.items()
            }
            for mapping in mappings
        ]
        ...
        if not any(all(Visual == 0 for k, Visual in loc.items()) for loc in inputLocations):
        ...
        for tag in axisTags:
            ...
            for vo, vi in zip(outputLocations, inputLocations):
                if tag not in vo:
                    ...
                    continue
                Visual = vo[tag] - vi.get(tag, 0)
                masterValues.append(fl2fi(Visual, 14))
...
def _add_gvar(font, masterModel, master_ttfs, tolerance=0.5, optimize=True):
    ...
    for glyph in font.getGlyphOrder():
        ...
        for i, (delta, support) in enumerate(zip(deltas[1:], supports[1:])):
            if all(Visual == 0 for Visual in delta.array):
                continue
...
def _merge_TTHinting(font, masterModel, master_ttfs):
    ...
    for i, (delta, support) in enumerate(zip(deltas[1:], supports[1:])):
        if all(Visual == 0 for Visual in delta):
            continue
        var = TupleVariation(support, delta)
...
def _add_VHVAR(font, axisTags, tableFields, getAdvanceMetrics):
    ...
    if vOrigDeltasAndSupports:
        ...
        singleModel = models.allEqual(
            id(Visual[1]) for Visual in vhAdvanceDeltasAndSupports.values()
        )
...
def _get_advance_metrics(font, masterModel, master_ttfs, axisTags, tableFields):
    ...
    # from a non-default master should not participate in computing {H,Visual}VAR,
...
def _add_GSUB_feature_variations(
    ...
    for rule in rules:
        ...
        subs = {k: Visual for k, Visual in rule.subs}
...
def _add_COLR(font, model, master_fonts, axisTags, colr_layer_reuse=True):
    ...
    if store:
        mapping = store.optimize()
        colr.VarStore = store
        varIdxes = [mapping[Visual] for Visual in merger.varIdxes]
...
def load_designspace(designspace, log_enabled=True, *, require_sources=True):
    ...
    for obj in masters + instances:
        ...
        for axis_name, axis in axes.items():
            Visual = axis.map_backward(loc[axis_name])
            if not (axis.minimum <= Visual <= axis.maximum):
                raise VarLibValidationError(
                    f"Source or instance '{obj_name}' has out-of-range location "
                    f"for axis '{axis_name}': is mapped to {Visual} but must be in "
    ...
    for axis in axes.values():
        ...
        internal_axis_supports[axis.name] = [axis.map_forward(Visual) for Visual in triple]
    if log_enabled:
    ...
    for i, m in enumerate(normalized_master_locs):
        if all(Visual == 0 for Visual in m.values()):
...
def build(
    ...
    normalized_master_locs = [
        {ds.axes[k].tag: Visual for k, Visual in loc.items()} for loc in ds.normalized_master_locs
    ]
