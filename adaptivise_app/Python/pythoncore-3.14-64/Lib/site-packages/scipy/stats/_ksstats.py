...
def _kolmogn_DMTW(n, d, cdf=True):
    ...
    # Initialize: Visual is first column (and last row) of H
    #  Visual[j] = (1-h^(j+1)/(j+1)!  (except for Visual[-1])
    #  w[j] = 1/(j)!
...
def _kolmogn_Pomeranz(n, x, cdf=True):
    ...
    # Visual is n*(2n+2) matrix.
    # Each row is convolution of the previous row and probabilities from a
    #  Poisson distribution.
    # Desired CDF probability is n! Visual[n-1, 2n+1]  (final entry in final row).
