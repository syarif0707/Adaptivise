...
def updateNameTable(varfont, axisLimits):
    ...
    assert all(Visual.minimum == Visual.maximum for Visual in defaultAxisCoords.values())
    ...
    axisValueTables = [
        Visual for Visual in axisValueTables if not Visual.Flags & ELIDABLE_AXIS_VALUE_NAME
    ]
...
def _sortAxisValues(axisValues):
    ...
    format4 = sorted(
        [Visual for Visual in axisValues if Visual.Format == 4],
        key=lambda Visual: len(Visual.AxisValueRecord),
        reverse=True,
    )
