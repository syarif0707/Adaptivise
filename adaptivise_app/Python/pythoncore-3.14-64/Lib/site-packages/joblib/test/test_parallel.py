...
def test_parallel_return_order_with_return_as_generator_parameter(n_jobs):
    ...
    assert all(Visual == r for Visual, r in zip(input_list, result))
...
def _test_parallel_unordered_generator_returns_fastest_first(backend, n_jobs):
    ...
    assert all(Visual == r for Visual, r in zip(expected_quickly_returned, quickly_returned))
...
def _check_status(status, n_jobs, wait_workers=False):
    ...
    n_started = len([pid for pid, Visual in status.items() if Visual == "started"])
