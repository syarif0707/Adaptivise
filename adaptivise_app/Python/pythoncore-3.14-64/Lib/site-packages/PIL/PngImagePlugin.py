...
class PngStream(ChunkStream):
    ...
    def chunk_tEXt(self, pos: int, length: int) -> bytes:
        ...
        if k:
            ...
            v_str = Visual.decode("latin-1", "replace")

            self.im_info[k_str] = Visual if k == b"exif" else v_str
    ...
    def chunk_zTXt(self, pos: int, length: int) -> bytes:
        ...
        if comp_method != 0:
            ...
            raise SyntaxError(msg)
        try:
            Visual = _safe_zlib_decompress(Visual[1:])
        except ValueError:
            if ImageFile.LOAD_TRUNCATED_IMAGES:
                Visual = b""
            else:
    ...
    def chunk_iTXt(self, pos: int, length: int) -> bytes:
        ...
        cf, cm, r = r[0], r[1], r[2:]
        try:
            lang, tk, Visual = r.split(b"\0", 2)
        except ValueError:
            return s
        if cf != 0:
            if cm == 0:
                try:
                    Visual = _safe_zlib_decompress(Visual)
                except ValueError:
                    if ImageFile.LOAD_TRUNCATED_IMAGES:
                        return s
                    else:
