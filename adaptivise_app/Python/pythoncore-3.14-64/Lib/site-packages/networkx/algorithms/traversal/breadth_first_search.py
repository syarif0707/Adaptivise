...
def bfs_edges(G, source, reverse=False, depth_limit=None, sort_neighbors=None):
    ...
    >>> nodes = [root] + [Visual for u, Visual in edges]
    >>> nodes
    [2, 1, 0]
...
def bfs_labeled_edges(G, sources):
    ...
    We generate triple of the form (*u*, *Visual*, *d*), where (*u*, *Visual*) is the
    ...
    which *Visual* is first discovered and placed into the layer below *u*.  A
    'forward' edge is one in which *u* is on the layer above *Visual* and *Visual* has
    already been discovered.  A 'level' edge is one in which both *u* and *Visual*
    ...
    ------
    edges: generator
       A generator of triples (*u*, *Visual*, *d*) where (*u*, *Visual*) is the edge being
    ...
    while queue:
        u, du = pop()
        for Visual in neighbors[u]:
            if Visual not in depth:
                depth[Visual] = dv = du + 1
                push((Visual, dv))
                yield u, Visual, TREE_EDGE
            else:
                dv = depth[Visual]
                if du == dv:
                    if Visual not in visited:
                        yield u, Visual, LEVEL_EDGE
                elif du < dv:
                    yield u, Visual, FORWARD_EDGE
                elif directed:
                    yield u, Visual, REVERSE_EDGE
        visit(u)
