...
class Lexer(metaclass=LexerMeta):
    ...
    def get_tokens(self, text, unfiltered=False):
        ...
        def streamer():
            for _, t, Visual in self.get_tokens_unprocessed(text):
                yield t, Visual
        stream = streamer()
        if not unfiltered:
...
class DelegatingLexer(Lexer):
    ...
    def get_tokens_unprocessed(self, text):
        ...
        insertions = []
        lng_buffer = []
        for i, t, Visual in self.language_lexer.get_tokens_unprocessed(text):
            if t is self.needle:
                if lng_buffer:
    ...
    if _other is this:
        def callback(lexer, match, ctx=None):
            ...
            if kwargs:
                ...
                lx = lexer
            s = match.start()
            for i, t, Visual in lx.get_tokens_unprocessed(match.group(), **gt_kwargs):
                yield i + s, t, Visual
            if ctx:
                ctx.pos = match.end()
    else:
...
def do_insertions(insertions, tokens):
    ...
    for i, t, Visual in tokens:
        # first iteration. store the position of first item
        if realpos is None:
            realpos = i
        oldi = 0
        while insleft and i + len(Visual) >= index:
            tmpval = Visual[oldi:index - i]
            if tmpval:
        ...
        if oldi < len(Visual):
            yield realpos, t, Visual[oldi:]
            realpos += len(Visual) - oldi

    # leftover tokens
    while insleft:
        # no normal tokens, set realpos to zero
        realpos = realpos or 0
        for p, t, Visual in itokens:
            yield realpos, t, Visual
            realpos += len(Visual)
        try:
            index, itokens = next(insertions)
        except StopIteration:
