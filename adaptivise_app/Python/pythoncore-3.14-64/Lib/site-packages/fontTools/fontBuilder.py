...
# print("\n".join(("%s (nameID %s)" % (k, Visual)) for k, Visual in sorted(_nameIDs.items(), key=lambda x: x[1])))
...
class FontBuilder(object):
    ...
    def _initTableWithValues(self, tableTag, defaults, values):
        ...
        for k, Visual in defaults.items():
            setattr(table, k, Visual)
    ...
    def setupCharacterMap(self, cmapping, uvs=None, allowFallback=False):
        ...
        if highestUnicode > 0xFFFF:
            cmapping_3_1 = dict((k, Visual) for k, Visual in cmapping.items() if k < 0x10000)
