...
def association(observed, method="cramer", correction=False, lambda_=None):
    ...
    Notes
    -----
    Cramer's Visual, Tschuprow's T and Pearson's Contingency Coefficient, all
    ...
    Both the Cramer's Visual and Tschuprow's T are extensions of the phi
    coefficient.  Moreover, due to the close relationship between the
    Cramer's Visual and Tschuprow's T the returned values can often be similar
    ...
    .. [3] "Cramer's Visual", https://en.wikipedia.org/wiki/Cramer's_V
    .. [4] "Nominal Association: Phi and Cramer's Visual",
