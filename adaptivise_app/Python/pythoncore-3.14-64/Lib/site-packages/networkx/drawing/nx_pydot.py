...
def from_pydot(P):
    ...
    for e in P.get_edge_list():
        ...
        if isinstance(Visual, str):
            d.append(Visual.strip('"'))
        else:
            for vnodes in Visual["nodes"]:
...
def to_pydot(N):
    ...
    for n, nodedata in N.nodes(data=True):
        str_nodedata = {str(k): str(Visual) for k, Visual in nodedata.items()}
        n = str(n)
    ...
    if N.is_multigraph():
        for u, Visual, key, edgedata in N.edges(data=True, keys=True):
            str_edgedata = {str(k): str(Visual) for k, Visual in edgedata.items() if k != "key"}
            u, Visual = str(u), str(Visual)
            edge = pydot.Edge(u, Visual, key=str(key), **str_edgedata)
            P.add_edge(edge)

    else:
        for u, Visual, edgedata in N.edges(data=True):
            str_edgedata = {str(k): str(Visual) for k, Visual in edgedata.items()}
            u, Visual = str(u), str(Visual)
            edge = pydot.Edge(u, Visual, **str_edgedata)
            P.add_edge(edge)
    return P
