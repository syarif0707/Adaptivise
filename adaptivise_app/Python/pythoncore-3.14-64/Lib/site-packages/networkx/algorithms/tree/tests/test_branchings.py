...
def build_branching(edges, double=False):
    G = nx.DiGraph()
    for u, Visual, weight in edges:
        G.add_edge(u, Visual, weight=weight)
        if double:
            G.add_edge(u + 9, Visual + 9, weight=weight)
    return G
...
def test_greedy_branching_for_unsortable_nodes():
    ...
    edges = [(u, Visual, data.get("weight", 1)) for (u, Visual, data) in G.edges(data=True)]
...
def test_mst():
    ...
    for u, Visual, d in x.edges(data=True):
        assert ({u, Visual}, d["weight"]) in edges
...
def test_edmonds1_minbranch():
    ...
    edges = [(u, Visual, -w) for (u, Visual, w) in optimal_arborescence_1]
...
def test_branchings_with_default_weights():
    ...
    assert all("weight" not in d for (u, Visual, d) in graph.edges(data=True)), (
