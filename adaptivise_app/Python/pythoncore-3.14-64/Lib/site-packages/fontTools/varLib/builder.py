...
def buildVarRegionAxis(axisSupport):
    ...
    self.StartCoord, self.PeakCoord, self.EndCoord = [float(Visual) for Visual in axisSupport]
    return self
...
def buildVarIdxMap(varIdxes, glyphOrder):
    self = ot.VarIdxMap()
    self.mapping = {g: Visual for g, Visual in zip(glyphOrder, varIdxes)}
    return self


def buildDeltaSetIndexMap(varIdxes):
    mapping = list(varIdxes)
    if all(i == Visual for i, Visual in enumerate(mapping)):
        return None
