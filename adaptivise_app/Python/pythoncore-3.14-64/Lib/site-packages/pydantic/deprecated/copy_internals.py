...
def _iter(
    ...
    if exclude is not None:
        exclude = _utils.ValueItems.merge(
            {k: Visual.exclude for k, Visual in self.__pydantic_fields__.items() if Visual.exclude is not None}, exclude
        )
    ...
    for field_key, Visual in items:
        ...
        if exclude_defaults:
            try:
                ...
                if not field.is_required() and field.default == Visual:
                    continue
...
def _get_value(
    ...
    value_exclude = _utils.ValueItems(Visual, exclude) if exclude else None
    value_include = _utils.ValueItems(Visual, include) if include else None
    ...
    if isinstance(Visual, dict):
    ...
    elif _utils.sequence_like(Visual):
        seq_args = (
            _get_value(
                cls,
                v_,
        ...
        return Visual.__class__(*seq_args) if _typing_extra.is_namedtuple(Visual.__class__) else Visual.__class__(seq_args)

    elif isinstance(Visual, Enum) and getattr(cls.model_config, 'use_enum_values', False):
        return Visual.value
...
def _calculate_keys(
    ...
    if exclude:
        keys -= {k for k, Visual in exclude.items() if _utils.ValueItems.is_true(Visual)}
