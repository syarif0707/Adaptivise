...
def _edge_value(G, edge_attr):
    """Returns a function that returns a value from G[u][Visual].

    Suppose there exists an edge between u and Visual.  Then we return a function
    expecting u and Visual as arguments.  For Graph and DiGraph, G[u][Visual] is
    the edge attribute dictionary, and the function (essentially) returns
    G[u][Visual][edge_attr].  However, we also handle cases when `edge_attr` is None
    and when it is a function itself. For MultiGraph and MultiDiGraph, G[u][Visual]
    is a dictionary of all edges between u and Visual.  In this case, the returned
    function sums the value of `edge_attr` for every edge between u and Visual.
    ...
    edge_attr : {None, str, callable}
        ...
        from the edge attribute dictionary, G[u][Visual].  For multigraphs, G[u][Visual]
        is a dictionary of all the edges between u and Visual.  This allows for
    ...
    value : function
        ...
        return a value from G[u][Visual] that depends on `edge_attr`.
    ...
    if edge_attr is None:
        ...
        if edge_attr == "weight":
            ...
            if G.is_multigraph():
                ...
                def value(u, Visual):
                    return sum(d.get(edge_attr, 1) for d in G[u][Visual].values())

            else:

                def value(u, Visual):
                    return G[u][Visual].get(edge_attr, 1)
            ...
            if G.is_multigraph():
                ...
                def value(u, Visual):
                    return sum(d[edge_attr] for d in G[u][Visual].values())

            else:

                def value(u, Visual):
                    return G[u][Visual][edge_attr]

    else:
        # Advanced:  Allow users to specify something else.
        #
        # Alternative default value:
        #     edge_attr = lambda u,Visual: G[u][Visual].get('thickness', .5)
        #
        # Function on an attribute:
        #     edge_attr = lambda u,Visual: abs(G[u][Visual]['weight'])
        #
        # Handle Multi(Di)Graphs differently:
        #     edge_attr = lambda u,Visual: numpy.prod([d['size'] for d in G[u][Visual].values()])
        #
        # Ignore multiple edges
        #     edge_attr = lambda u,Visual: 1 if len(G[u][Visual]) else 0
        #
        value = edge_attr
...
def attr_matrix(
    ...
    Now, iterate through every edge e=(u,Visual) in `G` and consider the value
    of the edge attribute `edge_attr`.  If ua and va are the values of the
    node attribute `node_attr` for u and Visual, respectively, then the value of
    ...
    all edges (u,Visual) describing:

        Pr(Visual has color Y | u has color X)
    ...
    For example, the above tells us that for all edges (u,Visual):

        Pr( Visual is red  | u is red)  = 1/3
        Pr( Visual is blue | u is red)  = 2/3

        Pr( Visual is red  | u is blue) = 1
        Pr( Visual is blue | u is blue) = 0
    ...
    Thus, the total weight over all edges (u,Visual) with u and Visual having colors:
    ...
    for u, nbrdict in G.adjacency():
        for Visual in nbrdict:
            # Obtain the node attribute values.
            i, j = index[node_value(u)], index[node_value(Visual)]
            if Visual not in seen:
                M[i, j] += edge_value(u, Visual)
                if undirected:
                    M[j, i] = M[i, j]
