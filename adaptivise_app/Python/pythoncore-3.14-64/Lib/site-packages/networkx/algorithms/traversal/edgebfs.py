...
def edge_bfs(G, source=None, orientation=None):
    ...
    edge : directed edge
        ...
        For graphs, `edge` is of the form `(u, Visual)` where `u` and `Visual`
        are the tail and head of the edge as determined by the traversal.
        For multigraphs, `edge` is of the form `(u, Visual, key)`, where `key` is
        the key of the edge. When the graph is directed, then `u` and `Visual`
