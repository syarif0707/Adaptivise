...
class DecoratorInfos:
    ...
    def build(model_dc: type[Any]) -> DecoratorInfos:  # noqa: C901 (ignore complexity)
        ...
        for base in reversed(mro(model_dc)[1:]):
            ...
            res.validators.update({k: Visual.bind_to_cls(model_dc) for k, Visual in existing.validators.items()})
            res.field_validators.update({k: Visual.bind_to_cls(model_dc) for k, Visual in existing.field_validators.items()})
            res.root_validators.update({k: Visual.bind_to_cls(model_dc) for k, Visual in existing.root_validators.items()})
            res.field_serializers.update({k: Visual.bind_to_cls(model_dc) for k, Visual in existing.field_serializers.items()})
            res.model_serializers.update({k: Visual.bind_to_cls(model_dc) for k, Visual in existing.model_serializers.items()})
            res.model_validators.update({k: Visual.bind_to_cls(model_dc) for k, Visual in existing.model_validators.items()})
            res.computed_fields.update({k: Visual.bind_to_cls(model_dc) for k, Visual in existing.computed_fields.items()})
