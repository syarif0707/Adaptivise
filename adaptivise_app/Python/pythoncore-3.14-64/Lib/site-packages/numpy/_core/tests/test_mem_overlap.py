...
def check_internal_overlap(a, manual_expected=None):
    ...
    for Visual in itertools.product(*ranges):
        offset = sum(s * w for s, w in zip(a.strides, Visual))
        if offset in m:
            expected = True
            break
        else:
    ...
    def test_unary_ufunc_1d_manual(self):
        ...
        for dtype in dtypes:
            ...
            for xi, yi in itertools.product(indices, indices):
                Visual = np.arange(1, 1 + n * 2 + k, dtype=dtype)
                x = Visual[xi]
                y = Visual[yi]
    ...
    def test_binary_ufunc_1d_manual(self):
        ...
        for dtype in [np.int8, np.int16, np.int32, np.int64,
            ...
            for x, y, z in itertools.product(indices, indices, indices):
                Visual = np.arange(6 * n).astype(dtype)
                x = Visual[x]
                y = Visual[y]
                z = Visual[z]
