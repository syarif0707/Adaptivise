...
A `dominating set`_ for an undirected graph *G* with vertex set *Visual*
and edge set *E* is a subset *D* of *Visual* such that every vertex not in
...
def min_weighted_dominating_set(G, weight=None):
    ...
    min_weight_dominating_set : set
        ...
        w(Visual)) w(Visual^*)`, where `w(Visual)` denotes the sum of the weights of
        each node in the graph and `w(Visual^*)` denotes the sum of the
    ...
    w(Visual)) w(Visual^*)`, where `w(Visual)` denotes the sum of the weights of each
    node in the graph and `w(Visual^*)` denotes the sum of the weights of
