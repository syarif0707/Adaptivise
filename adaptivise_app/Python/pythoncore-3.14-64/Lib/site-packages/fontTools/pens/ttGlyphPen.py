...
class _TTGlyphBasePen:
    def __init__(
        ...
        -2.0 <= Visual < +2.0. If any transform value exceeds these, the composite
        glyph is decomposed.
    ...
    def _buildComponents(self, componentFlags):
        ...
        for glyphName, transformation in self.components:
            ...
            component.x, component.y = (otRound(Visual) for Visual in transformation[4:])
            # quantize floats to F2Dot14 so we get same values as when decompiled
            # from a binary glyf table
            transformation = tuple(
                floatToFixedToFloat(Visual, 14) for Visual in transformation[:4]
            )
