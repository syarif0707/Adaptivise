...
class Glyph(object):
    ...
    def draw(self, pen, glyfTable, offset=0):
        ...
        start = 0
        maybeInt = lambda Visual: int(Visual) if Visual == int(Visual) else Visual
        for end in endPts:
            end = end + 1
...
class GlyphCoordinates(object):
    ...
    def __setitem__(self, k, Visual):
        ...
        if isinstance(k, slice):
            indices = range(*k.indices(len(self)))
            # XXX This only works if len(Visual) == len(indices)
            for j, i in enumerate(indices):
                self[i] = Visual[j]
            return
        self._a[2 * k], self._a[2 * k + 1] = Visual
    ...
    def calcIntBounds(self, round=otRound):
        return tuple(round(Visual) for Visual in self.calcBounds())
