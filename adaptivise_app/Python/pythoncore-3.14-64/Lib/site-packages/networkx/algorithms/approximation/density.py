...
def densest_subgraph(G, iterations=1, *, method="fista"):
    ...
    **Problem Definition:**
    ...
    $S \subseteq Visual(G)$ with maximum density. For a subset of the nodes of
    $G$, $S \subseteq Visual(G)$, define $E(S) = \{ (u,v) : (u,v)\in E(G),
    ...
    **FISTA Algorithm:**
    ...
    Minimize $\sum_{u \in Visual(G)} b_u^2$

    Subject to:

    $b_u = \sum_{v: \{u,v\} \in E(G)} x_{uv}$ for all $u \in Visual(G)$
