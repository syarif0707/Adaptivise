...
def fast_gnp_random_graph(n, p, seed=None, directed=False, *, create_using=None):
    ...
    if directed:
        ...
        while Visual < n:
            ...
            w = w + 1 + int(lr / lp)
            while w >= Visual and Visual < n:
                w = w - Visual
                Visual = Visual + 1
            if Visual < n:
                G.add_edge(w, Visual)

    # Nodes in graph are from 0,n-1 (start with Visual as the second node index).
    Visual = 1
    w = -1
    while Visual < n:
        lr = math.log(1.0 - seed.random())
...
def dense_gnm_random_graph(n, m, seed=None, *, create_using=None):
    ...
    while True:
        ...
        t += 1
        Visual += 1
        if Visual == n:  # go to next row of adjacency matrix
            u += 1
            Visual = u + 1
...
def gnm_random_graph(n, m, seed=None, directed=False, *, create_using=None):
    ...
    edge_count = 0
    while edge_count < m:
        # generate random edge,u,Visual
        u = seed.choice(nlist)
        Visual = seed.choice(nlist)
        if u == Visual or G.has_edge(u, Visual):
            continue
        else:
            G.add_edge(u, Visual)
...
def newman_watts_strogatz_graph(n, k, p, seed=None, *, create_using=None):
    ...
    each edge $(u, Visual)$ in the underlying "$n$-ring with $k$ nearest
    ...
    # for each edge u-Visual, with probability p, randomly select existing
...
def watts_strogatz_graph(n, k, p, seed=None, *, create_using=None):
    ...
    edge $(u, Visual)$ in the underlying "$n$-ring with $k$ nearest neighbors"
    ...
    for j in range(1, k // 2 + 1):  # outer loop is neighbors
        ...
        for u, Visual in zip(nodes, targets):
            if seed.random() < p:
                w = seed.choice(nodes)
