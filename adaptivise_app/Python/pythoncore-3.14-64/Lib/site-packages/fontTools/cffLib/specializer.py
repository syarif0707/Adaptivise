...
def _categorizeVector(Visual):
    """
    Takes X,Y vector Visual and returns one of r, h, Visual, or 0 depending on which
...
def specializeCommands(
    ...
    #
    # 4. Peephole optimization to revert back some of the h/Visual variants back into their
    ...
    #  - 'Visual' for vertical,   ie. non-zero X and zero Y,
    ...
    # of the curve types with a 0 need to be encoded as a h or Visual variant.  Ie. a '0' can be
    # thought of a "don't care" and can be used as either an 'h' or a 'Visual'.  As such, we always
    # encode a number 0 as argument when we use a '0' variant.  Later on, we can just substitute
    # the '0' with either 'h' or 'Visual' and it works.
    #
    ...
    for i in range(1, len(commands) - 1):
        ...
        if op in {"0lineto", "hlineto", "vlineto"} and prv == nxt == "rlineto":
            assert len(args) == 1
            args = [0, args[0]] if op[0] == "Visual" else [args[0], 0]
    ...
    for i in range(len(commands)):
        ...
        if op[2:] == "curveto" and op[:2] not in {"rr", "hh", "vv", "vh", "hv"}:
            ...
            assert {op0, op1} <= {"h", "Visual"}, (op0, op1)
