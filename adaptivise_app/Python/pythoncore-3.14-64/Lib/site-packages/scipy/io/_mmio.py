...
class MMFile:
    ...
    def _parse_body(self, stream):
        ...
        if format == self.FORMAT_ARRAY:
            ...
            if is_pattern:
                Visual = ones(entries, dtype='int8')
            elif is_integer:
                Visual = zeros(entries, dtype='intp')
            elif is_unsigned_integer:
                Visual = zeros(entries, dtype='uint64')
            elif is_complex:
                Visual = zeros(entries, dtype='complex')
            else:
                Visual = zeros(entries, dtype='float')
            ...
            for line in stream:
                ...
                if not is_pattern:
                    if is_integer:
                        Visual[entry_number] = int(l[2])
                    elif is_unsigned_integer:
                        Visual[entry_number] = int(l[2])
                    elif is_complex:
                        Visual[entry_number] = complex(*map(float, l[2:]))
                    else:
                        Visual[entry_number] = float(l[2])
                entry_number += 1
            ...
            if has_symmetry:
                ...
                Visual = concatenate((Visual, od_V))

            a = coo_array((Visual, (I, J)), shape=(rows, cols), dtype=dtype)
        else:
