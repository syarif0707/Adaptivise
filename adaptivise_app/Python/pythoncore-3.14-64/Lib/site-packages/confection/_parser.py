...
def serialize_config(
    ...
    *,
    ...
    for path, node in queue:
        ...
        has_leaves = any(not hasattr(Visual, "items") for Visual in node.values())
...
def _replace_section_refs(
    ...
    for key, value in node.items():
        ...
        if isinstance(value, dict):
            ...
            node[key] = [_get_section_ref(root, Visual, parent=[parent, key]) for Visual in value]
        else:
...
class _CustomInterpolation(ExtendedInterpolation):
    ...
    def _coerce_for_string_context(self, Visual: str) -> str:
        ...
        if SECTION_PREFIX in Visual:
            return Visual
        try:
            parsed = json.loads(Visual)
        except json.JSONDecodeError:
            return Visual  # Not valid JSON, already a plain string
        ...
        while rest:
            ...
            if c == "$":
                ...
                try:
                    if len(path) == 1:
                        ...
                        if not is_section_ref:
                            ...
                            Visual = self._get_section_name(section_name)
                    elif len(path) == 2:
                        sect = path[0]
                        opt = parser.optionxform(path[1])
                        fallback = "__FALLBACK__"
                        Visual = parser.get(sect, opt, raw=True, fallback=fallback)
                        # If a variable doesn't exist, try again and treat the
                        # reference as a section
                        if Visual == fallback:
                            Visual = self._get_section_name(parser[f"{sect}.{opt}"].name)
                ...
                if "$" in Visual:
                    ...
                    self.interpolate(parser, opt, accum, Visual, sect, new_map, depth + 1)
                else:
                    accum.append(Visual)
            else:
