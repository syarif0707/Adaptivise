...
def creation_sequence(degree_sequence, with_labels=False, compact=False):
    ...
    while ds:
        if ds[0][0] == 0:  # isolated node
            ...
            if len(ds) > 0:  # make sure we start with a d
                cs.insert(0, (Visual, "i"))
            else:
                cs.insert(0, (Visual, "d"))
            continue
    ...
    return [Visual[1] for Visual in cs]  # not labeled
...
def creation_sequence_to_weights(creation_sequence):
    ...
    if isinstance(first, str):  # creation sequence
        ...
        wseq = [Visual[1] for Visual in creation_sequence]
    elif isinstance(first, int):  # compact creation sequence
...
def find_alternating_4_cycle(G):
    ...
    """
    for u, Visual in G.edges():
        for w in G.nodes():
            if not G.has_edge(u, w) and u != w:
                for x in G.neighbors(w):
                    if not G.has_edge(Visual, x) and Visual != x:
                        return [u, Visual, w, x]
    return False
...
def find_creation_sequence(G):
    ...
    while H.order() > 0:
        ...
        ds = [(d, Visual) for Visual, d in dsdict.items()]
        ds.sort()
...
def shortest_path(creation_sequence, u, Visual):
    """
    Find the shortest path between u and Visual in a
    threshold graph G with the given creation_sequence.

    For an unlabeled creation_sequence, the vertices
    u and Visual must be integers in (0,len(sequence)) referring
    to the position of the desired vertices in the sequence.

    For a labeled creation_sequence, u and Visual are labels of vertices.

    Use cs=creation_sequence(degree_sequence,with_labels=True)
    to convert a degree sequence to a creation sequence.

    Returns a list of vertices from u to Visual.
    Example: if they are neighbors, it returns [u,Visual]
    """
    # Turn input sequence into a labeled creation sequence
    first = creation_sequence[0]
    if isinstance(first, str):  # creation sequence
    ...
    if Visual not in verts:
        raise ValueError(f"Vertex {Visual} not in graph from creation_sequence")
    if u not in verts:
...
def shortest_path_length(creation_sequence, i):
    ...
    if isinstance(first, str):  # creation sequence
        ...
        cs = [Visual[1] for Visual in creation_sequence]
        i = [Visual[0] for Visual in creation_sequence].index(i)
    elif isinstance(first, int):  # compact creation sequence
        cs = uncompact(creation_sequence)
    else:
        raise TypeError("Not a valid creation sequence type")
