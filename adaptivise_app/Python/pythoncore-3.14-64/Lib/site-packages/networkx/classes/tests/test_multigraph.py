...
class BaseMultiGraphTester(BaseAttrGraphTester):
    ...
    def test_edge_attr(self):
        ...
        assert all(
            isinstance(d, G.edge_attr_dict_factory) for u, Visual, d in G.edges(data=True)
        )
        assert edges_equal(
...
class TestMultiGraph(BaseMultiGraphTester, _TestGraph):
    ...
    def test_multigraph_add_edges_from_four_tuple_misordered(self):
        """add_edges_from expects 4-tuples of the format (u, Visual, key, data_dict).

        Ensure 4-tuples of form (u, Visual, data_dict, key) raise exception.
        """
        G = nx.MultiGraph()
        with pytest.raises(TypeError):
...
class TestEdgeSubgraph:
    ...
    def test_node_attr_dict(self):
        ...
        """
        for Visual in self.H:
            assert self.G.nodes[Visual] == self.H.nodes[Visual]
    ...
    def test_edge_attr_dict(self):
        ...
        """
        for u, Visual, k in self.H.edges(keys=True):
            assert self.G._adj[u][Visual][k] == self.H._adj[u][Visual][k]
        # Making a change to G should make a change in H and vice versa.
...
class TestMultiGraphSubclass(TestMultiGraph):
    def setup_method(self):
        ...
        for u in self.k3nodes:
            for Visual in self.k3nodes:
                if u != Visual:
                    d = {0: {}}
                    self.K3._adj[u][Visual] = d
                    self.K3._adj[Visual][u] = d
