...
@py_random_state(6)
@nx._dispatchable(graphs=None, returns_graph=True)
def gaussian_random_partition_graph(n, s, Visual, p_in, p_out, directed=False, seed=None):
    """Generate a Gaussian random partition graph.

    A Gaussian random partition graph is created by creating k partitions
    each with a size drawn from a normal distribution with mean s and variance
    s/Visual. Nodes are connected within clusters with probability p_in and
    ...
    s : float
      Mean cluster size
    Visual : float
      Shape parameter. The variance of cluster size distribution is s/Visual.
    p_in : float
    ...
    Notes
    -----
    Note the number of partitions is dependent on s,Visual and n, and that the
    ...
    sizes = []
    while True:
        size = int(seed.gauss(s, s / Visual + 0.5))
...
def stochastic_block_model(
    ...
    >>> for Visual in H.nodes(data=True):
    ...     print(round(Visual[1]["density"], 3))
    0.245
    0.348
    0.405
    >>> for Visual in H.edges(data=True):
    ...     print(round(1.0 * Visual[2]["weight"] / (sizes[Visual[0]] * sizes[Visual[1]]), 3))
    0.051
    0.022
    0.07
...
def _generate_communities(degree_seq, community_sizes, mu, max_iters, seed):
    ...
    for i in range(max_iters):
        ...
        # s = int(degree_seq[Visual] * (1 - mu) + 0.5)
        s = round(degree_seq[Visual] * (1 - mu))
...
def LFR_benchmark_graph(
    ...
    node attributes of the graph::
        ...
        >>> communities = {frozenset(G.nodes[Visual]["community"]) for Visual in G}
    ...
    for c in communities:
        for u in c:
            while G.degree(u) < round(deg_seq[u] * (1 - mu)):
                Visual = seed.choice(list(c))
                G.add_edge(u, Visual)
            while G.degree(u) < deg_seq[u]:
                Visual = seed.choice(range(n))
                if Visual not in c:
                    G.add_edge(u, Visual)
