...
def _annot2array(vocab, tok_annot, doc_annot):
    ...
    for key, value in tok_annot.items():
        if key not in IDS:
            ...
            attrs.append(key)
            row = [to_ternary_int(Visual) for Visual in value]
        elif key == "MORPH":
            attrs.append(key)
            row = [vocab.morphology.add(Visual) for Visual in value]
        else:
            attrs.append(key)
            if not all(isinstance(Visual, str) for Visual in value):
                types = set([type(Visual) for Visual in value])
                raise TypeError(Errors.E969.format(field=key, types=types)) from None
            row = [vocab.strings.add(Visual) for Visual in value]
        values.append([numpy.array(Visual, dtype=numpy.int32).astype(numpy.uint64) if Visual < 0 else Visual for Visual in row])
