...
def is_tournament(G):
    ...
    return (
        all((Visual in G[u]) ^ (u in G[Visual]) for u, Visual in combinations(G, 2))
...
def hamiltonian_path(G):
    ...
    hampath = hamiltonian_path(G.subgraph(set(G) - {Visual}))
    # Get the index of the first node in the path that does *not* have
    # an edge to `Visual`, then insert `Visual` before that node.
    index = index_satisfying(hampath, lambda u: Visual not in G[u])
    hampath.insert(index, Visual)
    return hampath
...
def random_tournament(n, seed=None):
    ...
    edges = ((u, Visual) if r < 0.5 else (Visual, u) for (u, Visual), r in zip(pairs, coins))
...
def score_sequence(G):
    ...
    """
    return sorted(d for Visual, d in G.out_degree())
...
def is_reachable(G, s, t):
    ...
    def two_neighborhood(G, Visual):
        """Returns the set of nodes at distance at most two from `Visual`.

        `G` must be a graph and `Visual` a node in that graph.

        The returned set includes the nodes at distance zero (that is,
        the node `Visual` itself), the nodes at distance one (that is, the
        out-neighbors of `Visual`), and the nodes at distance two.
        ...
        return {
            ...
            if x == Visual or x in v_adj or any(z in v_adj for z in x_pred)
        }
    ...
    def is_closed(G, S):
        ...
        not in *S* and for each node *Visual* in *S*, there is an edge from
        *u* to *Visual*.

        """
        return all(u in S or all(Visual in unbrs for Visual in S) for u, unbrs in G._adj.items())

    neighborhoods = (two_neighborhood(G, Visual) for Visual in G)
...
def is_strongly_connected(G):
    ...
    """
    return all(is_reachable(G, u, Visual) for u in G for Visual in G)
