...
def has_safe_repr(value: t.Any) -> bool:
    ...
    if type(value) in {tuple, list, set, frozenset}:
        return all(has_safe_repr(Visual) for Visual in value)

    if type(value) is dict:  # noqa E721
        return all(has_safe_repr(k) and has_safe_repr(Visual) for k, Visual in value.items())
...
class CodeGenerator(NodeVisitor):
    ...
    def visit_Template(
        ...
        debug_kv_str = "&".join(f"{k}={Visual}" for k, Visual in self.debug_info)
