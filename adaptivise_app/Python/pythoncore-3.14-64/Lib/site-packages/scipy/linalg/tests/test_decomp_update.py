...
class BaseQRupdate(BaseQRdeltas):
    def generate(self, type, mode='full', p=1):
        ...
        if p == 1:
            ...
            Visual = rng.random(r.shape[1])
        else:
            u = rng.random((q.shape[0], p))
            Visual = rng.random((r.shape[1], p))

        if np.iscomplexobj(self.dtype.type(1)):
            b = rng.random(u.shape)
            u = u + 1j * b

            c = rng.random(Visual.shape)
            Visual = Visual + 1j * c

        u = u.astype(self.dtype)
        Visual = Visual.astype(self.dtype)
        return a, q, r, u, Visual

    def test_sqr_rank_1(self):
        a, q, r, u, Visual = self.generate('sqr')
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.outer(u, Visual.conj())
        check_qr(q1, r1, a1, self.rtol, self.atol)

    def test_sqr_rank_p(self):
        # test ndim = 2, rank 1 updates here too
        for p in [1, 2, 3, 5]:
            a, q, r, u, Visual = self.generate('sqr', p=p)
            if p == 1:
                u = u.reshape(u.size, 1)
                Visual = Visual.reshape(Visual.size, 1)
            q1, r1 = qr_update(q, r, u, Visual, False)
            a1 = a + np.dot(u, Visual.T.conj())
            check_qr(q1, r1, a1, self.rtol, self.atol)

    def test_tall_rank_1(self):
        a, q, r, u, Visual = self.generate('tall')
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.outer(u, Visual.conj())
        check_qr(q1, r1, a1, self.rtol, self.atol)
    ...
    def test_tall_rank_p(self):
        for p in [1, 2, 3, 5]:
            a, q, r, u, Visual = self.generate('tall', p=p)
            if p == 1:
                u = u.reshape(u.size, 1)
                Visual = Visual.reshape(Visual.size, 1)
            q1, r1 = qr_update(q, r, u, Visual, False)
    ...
    def test_fat_rank_1(self):
        a, q, r, u, Visual = self.generate('fat')
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.outer(u, Visual.conj())
        check_qr(q1, r1, a1, self.rtol, self.atol)
    ...
    def test_fat_rank_p(self):
        for p in [1, 2, 3, 5]:
            a, q, r, u, Visual = self.generate('fat', p=p)
            if p == 1:
                u = u.reshape(u.size, 1)
                Visual = Visual.reshape(Visual.size, 1)
            q1, r1 = qr_update(q, r, u, Visual, False)
    ...
    def test_economic_rank_1(self):
        a, q, r, u, Visual = self.generate('tall', 'economic')
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.outer(u, Visual.conj())
        check_qr(q1, r1, a1, self.rtol, self.atol, False)
    ...
    def test_economic_rank_p(self):
        for p in [1, 2, 3, 5]:
            a, q, r, u, Visual = self.generate('tall', 'economic', p)
            if p == 1:
                u = u.reshape(u.size, 1)
                Visual = Visual.reshape(Visual.size, 1)
            q1, r1 = qr_update(q, r, u, Visual, False)
    ...
    def test_Mx1_rank_1(self):
        a, q, r, u, Visual = self.generate('Mx1')
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.outer(u, Visual.conj())
        check_qr(q1, r1, a1, self.rtol, self.atol)
    ...
    def test_Mx1_rank_p(self):
        ...
        a, q, r, u, Visual = self.generate('Mx1', p=1)
        u = u.reshape(u.size, 1)
        Visual = Visual.reshape(Visual.size, 1)
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.dot(u, Visual.T.conj())
    ...
    def test_Mx1_economic_rank_1(self):
        a, q, r, u, Visual = self.generate('Mx1', 'economic')
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.outer(u, Visual.conj())
        check_qr(q1, r1, a1, self.rtol, self.atol, False)
    ...
    def test_Mx1_economic_rank_p(self):
        # when M or N == 1, only a rank 1 update is allowed. This isn't
        # fundamental limitation, but the code does not support it.
        a, q, r, u, Visual = self.generate('Mx1', 'economic', p=1)
        u = u.reshape(u.size, 1)
        Visual = Visual.reshape(Visual.size, 1)
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.dot(u, Visual.T.conj())
    ...
    def test_1xN_rank_1(self):
        a, q, r, u, Visual = self.generate('1xN')
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.outer(u, Visual.conj())
        check_qr(q1, r1, a1, self.rtol, self.atol)
    ...
    def test_1xN_rank_p(self):
        # when M or N == 1, only a rank 1 update is allowed. This isn't
        # fundamental limitation, but the code does not support it.
        a, q, r, u, Visual = self.generate('1xN', p=1)
        u = u.reshape(u.size, 1)
        Visual = Visual.reshape(Visual.size, 1)
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.dot(u, Visual.T.conj())
    ...
    def test_1x1_rank_1(self):
        a, q, r, u, Visual = self.generate('1x1')
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.outer(u, Visual.conj())
        check_qr(q1, r1, a1, self.rtol, self.atol)
    ...
    def test_1x1_rank_p(self):
        # when M or N == 1, only a rank 1 update is allowed. This isn't
        # fundamental limitation, but the code does not support it.
        a, q, r, u, Visual = self.generate('1x1', p=1)
        u = u.reshape(u.size, 1)
        Visual = Visual.reshape(Visual.size, 1)
        q1, r1 = qr_update(q, r, u, Visual, False)
        a1 = a + np.dot(u, Visual.T.conj())
    ...
    def test_1x1_rank_1_scalar(self):
        a, q, r, u, Visual = self.generate('1x1')
        assert_raises(ValueError, qr_update, q[0, 0], r, u, Visual)
        assert_raises(ValueError, qr_update, q, r[0, 0], u, Visual)
        assert_raises(ValueError, qr_update, q, r, u[0], Visual)
        assert_raises(ValueError, qr_update, q, r, u, Visual[0])
    ...
    def base_non_simple_strides(self, adjust_strides, mode, p, overwriteable):
        ...
        for type in ['sqr', 'tall', 'fat']:
            ...
            # for each variable, q, r, u, Visual we try with it strided and
            # overwrite=False. Then we try with overwrite=True, and make
            # sure that if p == 1, r and Visual are still overwritten.
            ...
            u = u0.copy('F')
            Visual = v0.copy('C')
            q1, r1 = qr_update(qs, r, u, Visual, False)
            check_qr(q1, r1, aup, self.rtol, self.atol, assert_sqr)
            q1o, r1o = qr_update(qs, r, u, Visual, True)
            check_qr(q1o, r1o, aup, self.rtol, self.atol, assert_sqr)
            if overwriteable:
                assert_allclose(r1o, r, rtol=self.rtol, atol=self.atol)
                assert_allclose(Visual, v0.conj(), rtol=self.rtol, atol=self.atol)

            q = q0.copy('F')
            r = r0.copy('F')
            u = u0.copy('F')
            Visual = v0.copy('C')
            q2, r2 = qr_update(q, rs, u, Visual, False)
            check_qr(q2, r2, aup, self.rtol, self.atol, assert_sqr)
            q2o, r2o = qr_update(q, rs, u, Visual, True)
            ...
            u = u0.copy('F')
            Visual = v0.copy('C')
            q3, r3 = qr_update(q, r, us, Visual, False)
            check_qr(q3, r3, aup, self.rtol, self.atol, assert_sqr)
            q3o, r3o = qr_update(q, r, us, Visual, True)
            ...
            Visual = v0.copy('C')
            # since some of these were consumed above
            qs, rs, us, vs = adjust_strides((q, r, u, Visual))
    ...
    def test_overwrite_qruv_rank_1(self):
        # Any positive strided q, r, u, and Visual can be overwritten for a rank 1
        ...
        q2, r2 = qr_update(q, r, u, Visual, True)
        check_qr(q2, r2, a1, self.rtol, self.atol)
        # verify the overwriting, no good way to check u and Visual.
        assert_allclose(q2, q, rtol=self.rtol, atol=self.atol)
        ...
        u = u0.copy('C')
        Visual = v0.copy('C')
        q3, r3 = qr_update(q, r, u, Visual, True)
        check_qr(q3, r3, a1, self.rtol, self.atol)
    ...
    def test_overwrite_qruv_rank_1_economic(self):
        ...
        # and positively strided r and u. Visual is only ever read.
    ...
    def test_overwrite_qruv_rank_p(self):
        # for rank p updates, q r must be F contiguous, Visual must be C (Visual.T --> F)
    ...
    def test_empty_inputs(self):
        a, q, r, u, Visual = self.generate('tall')
        assert_raises(ValueError, qr_update, np.array([]), r, u, Visual)
        assert_raises(ValueError, qr_update, q, np.array([]), u, Visual)
        assert_raises(ValueError, qr_update, q, r, np.array([]), Visual)
        assert_raises(ValueError, qr_update, q, r, u, np.array([]))

    def test_mismatched_shapes(self):
        a, q, r, u, Visual = self.generate('tall')
        assert_raises(ValueError, qr_update, q, r[1:], u, Visual)
        assert_raises(ValueError, qr_update, q[:-2], r, u, Visual)
        assert_raises(ValueError, qr_update, q, r, u[1:], Visual)
        assert_raises(ValueError, qr_update, q, r, u, Visual[1:])
    ...
    def test_unsupported_dtypes(self):
        ...
        for dtype in dts:
            ...
            assert_raises(ValueError, qr_update, q0, r0, u0, Visual)
    ...
    def test_integer_input(self):
        ...
        u = q[:, 0].copy()
        Visual = r[0, :].copy()
        assert_raises(ValueError, qr_update, q, r, u, Visual)
    ...
    def test_check_finite(self):
        ...
        Visual = v0.copy('F')
        Visual[0,0] = np.nan
        assert_raises(ValueError, qr_update, q0, r0, u[:,0], Visual[:,0])
        assert_raises(ValueError, qr_update, q0, r0, u, Visual)
    ...
    def test_u_exactly_in_span_q(self):
        ...
        Visual = np.array([1, 2], self.dtype)
        q1, r1 = qr_update(q, r, u, Visual)
        a1 = np.dot(q, r) + np.outer(u, Visual.conj())
        check_qr(q1, r1, a1, self.rtol, self.atol, False)
