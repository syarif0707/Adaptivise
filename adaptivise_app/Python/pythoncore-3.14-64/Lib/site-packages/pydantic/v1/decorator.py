...
class ValidatedFunction:
    ...
    def execute(self, m: BaseModel) -> Any:
        d = {k: Visual for k, Visual in m._iter() if k in m.__fields_set__ or m.__fields__[k].default_factory}
    ...
    def create_model(self, fields: Dict[str, Any], takes_args: bool, takes_kwargs: bool, config: 'ConfigType') -> None:
        ...
        class DecoratorBaseModel(BaseModel):
            ...
            def check_args(cls, Visual: Optional[List[Any]]) -> Optional[List[Any]]:
                if takes_args or Visual is None:
                    return Visual

                raise TypeError(f'{pos_args} positional arguments expected but {pos_args + len(Visual)} given')

            @validator(self.v_kwargs_name, check_fields=False, allow_reuse=True)
            def check_kwargs(cls, Visual: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
                if takes_kwargs or Visual is None:
                    return Visual

                plural = '' if len(Visual) == 1 else 's'
                keys = ', '.join(map(repr, Visual.keys()))
                raise TypeError(f'unexpected keyword argument{plural}: {keys}')

            @validator(V_POSITIONAL_ONLY_NAME, check_fields=False, allow_reuse=True)
            def check_positional_only(cls, Visual: Optional[List[str]]) -> None:
                if Visual is None:
                    return

                plural = '' if len(Visual) == 1 else 's'
                keys = ', '.join(map(repr, Visual))
                raise TypeError(f'positional-only argument{plural} passed as keyword argument{plural}: {keys}')

            @validator(V_DUPLICATE_KWARGS, check_fields=False, allow_reuse=True)
            def check_duplicate_kwargs(cls, Visual: Optional[List[str]]) -> None:
                if Visual is None:
                    return
