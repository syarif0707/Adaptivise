...
def dict_to_numpy_array(d, mapping=None):
    ...
    try:
        ...
        # AttributeError is when no mapping was provided and Visual.keys() fails.
...
def arbitrary_element(iterable):
    ...
    1
    >>> d = {k: Visual for k, Visual in zip([1, 2, 3], [3, 2, 1])}
...
def groups(many_to_one):
    ...
    for Visual, k in many_to_one.items():
        one_to_many[k].add(Visual)
...
def edges_equal(edges1, edges2, *, directed=False):
    ...
    edges1, edges2 : iterables of tuples
        Each tuple can be
        an edge tuple ``(u, Visual)``, or
        an edge tuple with data `dict` s ``(u, Visual, d)``, or
        an edge tuple with keys and data `dict` s ``(u, Visual, k, d)``.
    ...
    for e1, e2 in zip_longest(edges1, edges2, fillvalue=None):
        ...
        for e, d in [(e1, d1), (e2, d2)]:
            u, Visual, *data = e
            d[u, Visual].append(data)
            if not directed:
                d[Visual, u].append(data)
