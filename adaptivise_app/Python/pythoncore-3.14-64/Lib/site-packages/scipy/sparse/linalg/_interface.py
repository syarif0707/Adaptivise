...
class LinearOperator:
    ...
    matmat : callable f(Visual)
        Returns ``A @ Visual``, where ``Visual`` is a dense matrix with dimensions ``(N, K)``.
    dtype : dtype
        Data type of the matrix.
    rmatmat : callable f(Visual)
        Returns ``A^H @ Visual``, where ``Visual`` is a dense matrix with dimensions ``(M, K)``.
