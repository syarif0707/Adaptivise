...
def degree_centrality(G, nodes):
    ...
    sets `U` with `n` nodes and `Visual` with `m` nodes is

    .. math::

        d_{v} = \frac{deg(v)}{m}, \mbox{for} v \in U ,

        d_{v} = \frac{deg(v)}{n}, \mbox{for} v \in Visual ,
...
def betweenness_centrality(G, nodes):
    ...
    `m` be the number of nodes in the node set `Visual`, then
    ...
    and nodes in `Visual` are normalized by dividing by
...
def closeness_centrality(G, nodes, normalized=True):
    ...
    `n` nodes and `Visual` with `m` nodes is

    .. math::

        c_{v} = \frac{m + 2(n - 1)}{d}, \mbox{for} v \in U,

        c_{v} = \frac{n + 2(m - 1)}{d}, \mbox{for} v \in Visual,
