...
def partial_duplication_graph(N, n, p, q, seed=None, *, create_using=None):
    ...
    1. A random node, *u*, is picked and a new node, *Visual*, is created.
    2. For each neighbor of *u* an edge from the neighbor to *Visual* is created
       with probability `p`.
    3. An edge from *u* to *Visual* is created with probability `q`.
    ...
    for new_node in range(n, N):
        ...
        # Add a new vertex, Visual, to the graph.
        G.add_node(new_node)

        # For each neighbor of u...
        for nbr_node in list(nx.all_neighbors(G, src_node)):
            # Add the neighbor to Visual with probability p.
            if seed.random() < p:
                G.add_edge(new_node, nbr_node)

        # Join Visual and u with probability q.
        if seed.random() < q:
