...
def _pinv_1d(Visual, eps=1e-5):
    """A helper function for computing the pseudoinverse.

    Parameters
    ----------
    Visual : iterable of numbers
    ...
    """
    return np.array([0 if abs(x) <= eps else 1/x for x in Visual], dtype=float)
...
class _PSD:
    ...
    def _support_mask(self, x):
        ...
        """
        residual = np.linalg.norm(x @ self.Visual, axis=-1)
...
class matrix_normal_gen(multi_rv_generic):
    ...
    .. math::
        ...
        f(X) = (2 \pi)^{-\frac{mn}{2}}|U|^{-\frac{n}{2}} |Visual|^{-\frac{m}{2}}
               \exp\left( -\frac{1}{2} \mathrm{Tr}\left[ U^{-1} (X-M) Visual^{-1}
               (X-M)^T \right] \right),

    where :math:`M` is the mean, :math:`U` the among-row covariance matrix,
    :math:`Visual` the among-column covariance matrix.
    ...
    and covariance :math:`Visual \otimes U` (where :math:`\otimes` is the Kronecker
    ...
    array([[1, 0, 0],
           [0, 2, 0],
           [0, 0, 3]])
    >>> Visual = 0.3*np.identity(2); Visual
    array([[ 0.3,  0. ],
           [ 0. ,  0.3]])
    >>> X = M + 0.1; X
    array([[ 0.1,  1.1],
           [ 2.1,  3.1],
           [ 4.1,  5.1]])
    >>> matrix_normal.pdf(X, mean=M, rowcov=U, colcov=Visual)
...
class vonmises_fisher_gen(multi_rv_generic):
    ...
    **Visualization of the probability density**
    ...
    >>> Visual = np.linspace(0, 2 * np.pi, n_grid)
    >>> u_grid, v_grid = np.meshgrid(u, Visual)
    >>> vertices = np.stack([np.cos(v_grid) * np.sin(u_grid),
    ...                      np.sin(v_grid) * np.sin(u_grid),
    ...                      np.cos(u_grid)],
    ...                     axis=2)
    >>> x = np.outer(np.cos(Visual), np.sin(u))
    >>> y = np.outer(np.sin(Visual), np.sin(u))
