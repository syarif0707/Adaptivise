...
Options:
  ...
  -Visual               Print f2py version ID and exit.
...
def dict_append(d_out, d_in):
    for (k, Visual) in d_in.items():
        if k not in d_out:
            d_out[k] = []
        if isinstance(Visual, list):
            d_out[k] = d_out[k] + Visual
        else:
            d_out[k].append(Visual)
                ...
                if ov not in allowed_keys:
                    ...
                    try:
                        ...
                        if ov not in vmap.values():
                            print(f'Unknown vendor: "{s[len(Visual):]}"')
                    nv = ov
