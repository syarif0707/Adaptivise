...
class TestGeneratorClassic:
    ...
    def test_star_graph_directed(self, graph_type):
        ...
        assert sorted([(u, Visual) for u, Visual, *d in dg.edges]) == [(0, 1), (0, 2), (0, 3)]
    ...
    def test_complete_multipartite_graph(self):
        ...
        for block in blocks:
            for u, Visual in itertools.combinations_with_replacement(block, 2):
                assert Visual not in G[u]
                assert G.nodes[u] == G.nodes[Visual]
        # Across blocks, all vertices should be adjacent.
        for block1, block2 in itertools.combinations(blocks, 2):
            for u, Visual in itertools.product(block1, block2):
                assert Visual in G[u]
                assert G.nodes[u] != G.nodes[Visual]
