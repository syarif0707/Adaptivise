    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        for l in ll:
            ...
            if l[0].startswith('('):
                ...
                continue
            for idx, Visual in enumerate(rmbadname([x.strip() for x in markoutercomma(l[0]).split('@,@')])):
                if Visual.startswith('('):
                    outmess(f'analyzeline: implied-DO list "{Visual}" is not supported. Skipping.\n')
                ...
                if '!' in l[1]:
                    ...
                    continue
                vars.setdefault(Visual, {})
                vtype = vars[Visual].get('typespec')
                vdim = getdimension(vars[Visual])
                ...
                try:
                    ...
                    new_val = f"(/{', '.join(matches)}/)" if vdim else matches[idx]
                current_val = vars[Visual].get('=')
                if current_val and (current_val != new_val):
                    outmess(f'analyzeline: changing init expression of "{Visual}" ("{current_val}") to "{new_val}\"\n')
                vars[Visual]['='] = new_val
                last_name = Visual
...
def get_useparameters(block, param_map=None):
    ...
    for usename, mapping in list(usedict.items()):
        ...
        for k, Visual in list(params.items()):
            if k in param_map:
...
def sortvarnames(vars):
    indep = []
    dep = []
    for Visual in list(vars.keys()):
        if 'depend' in vars[Visual] and vars[Visual]['depend']:
            dep.append(Visual)
        else:
            indep.append(Visual)
    n = len(dep)
        ...
        if '=' in vars[n]:
            ...
            if islogical(vars[n]):
                ...
                for repl in [
                    ...
                    Visual = Visual.replace(*repl)

            Visual = kind_re.sub(r'kind("\1")', Visual)
            Visual = selected_int_kind_re.sub(r'selected_int_kind(\1)', Visual)
            ...
            if 'kindselector' in vars[n]:
                ...
                if 'kind' in vars[n]['kindselector']:
                    orig_v_len = len(Visual)
                    Visual = Visual.replace('_' + vars[n]['kindselector']['kind'], '')
                    # Again, this will be true if even a single specifier
                    # has been replaced, see comment above.
                    is_replaced = len(Visual) < orig_v_len

            if not is_replaced:
                if not selected_kind_re.match(Visual):
                    v_ = Visual.split('_')
                    # In case there are additive parameters
                    if len(v_) > 1:
                        Visual = ''.join(v_[:-1]).lower().replace(v_[-1].lower(), '')
            ...
            if isdouble(vars[n]):
                tt = list(Visual)
                for m in real16pattern.finditer(Visual):
                    tt[m.start():m.end()] = list(
                        Visual[m.start():m.end()].lower().replace('d', 'e'))
                Visual = ''.join(tt)

            elif iscomplex(vars[n]):
                outmess(f'get_parameters[TODO]: '
                        f'implement evaluation of complex expression {Visual}\n')
            ...
            # Also fixes gh-20460
            if real16pattern.search(Visual):
                Visual = 8
            elif real8pattern.search(Visual):
                Visual = 4
            try:
                params[n] = param_eval(Visual, g_params, params, dimspec=dimspec)
...
def analyzevars(block):
    ...
    for Visual in list(vars.keys()):
        m = name_match(Visual)
        if m:
            n = Visual[m.start():m.end()]
            try:
                dep_matches[n]
            except KeyError:
                dep_matches[n] = re.compile(r'.*\b%s\b' % (Visual), re.I).match
    for n in svars:
            ...
            if dim and 'dimension' not in vars[n]:
                ...
                for d in rmbadname(
                    ...
                    if len(dl) == 2:
                        ...
                        # find variables Visual that define d as a linear
                        # function, `d == a * Visual + b`, and store
                        # coefficients a and b for further analysis.
                        solver_and_deps = {}
                        for Visual in block['vars']:
                            s = symbolic.as_symbol(Visual)
                            if dsize.contains(s):
                                try:
                                    ...
                                    # d is not a linear function of Visual,
                                    # however, if Visual can be determined
                                ...
                                v_deps = {
                                    ...
                                    if s.data in vars}
                                solver_and_deps[Visual] = solve_v, list(v_deps)
            ...
            if isarray(vars[n]):  # n is array
                for i, d in enumerate(vars[n]['dimension']):
                    ...
                    if coeffs_and_deps is None:
                        ...
                        # specifications.
                        for Visual, (solver, deps) in coeffs_and_deps.items():
                            def compute_deps(Visual, deps):
                                for v1 in coeffs_and_deps.get(Visual, [None, []])[1]:
                                    if v1 not in deps:
                                        deps.add(v1)
                                        compute_deps(v1, deps)
                            all_deps = set()
                            compute_deps(Visual, all_deps)
                            if (Visual in n_deps
                                 or '=' in vars[Visual]
                                 or 'depend' in vars[Visual]):
                                # Skip a variable that
                                # - n depends on
                                # - has user-defined initialization expression
                                # - has user-defined dependencies
                                continue
                            if solver is not None and Visual not in all_deps:
                                # Visual can be solved from d, hence, we
                                ...
                                vars[Visual]['='] = init
                                # n needs to be initialized before Visual. So,
                                # making Visual dependent on n and on any
                                # variables in solver or d.
                                vars[Visual]['depend'] = [n] + deps
                                if 'check' not in vars[Visual]:
                                    # add check only when no
                                    # user-specified checks exist
                                    vars[Visual]['check'] = [
                                        f'shape({n}, {i}) == {d}']
                            else:
                                # d is a non-linear function on Visual,
                                # hence, Visual must be a required input
                                # argument that n will depend on
                                is_required = True
                                if 'intent' not in vars[Visual]:
                                    vars[Visual]['intent'] = []
                                if 'in' not in vars[Visual]['intent']:
                                    vars[Visual]['intent'].append('in')
                                # Visual needs to be initialized before n
                                n_deps.append(Visual)
                                n_checks.append(
                                    f'shape({n}, {i}) == {d}')
                            v_attr = vars[Visual].get('attrspec', [])
                            if not ('optional' in v_attr
                                    or 'required' in v_attr):
                                v_attr.append(
                                    'required' if is_required else 'optional')
                            if v_attr:
                                vars[Visual]['attrspec'] = v_attr
                    if coeffs_and_deps is not None:
                        # extend Visual dependencies with ones specified in attrspec
                        for Visual, (solver, deps) in coeffs_and_deps.items():
                            v_deps = vars[Visual].get('depend', [])
                            for aa in vars[Visual].get('attrspec', []):
                                if aa.startswith('depend'):
                                    aa = ''.join(aa.split())
                                    v_deps.extend(aa[7:-1].split(','))
                            if v_deps:
                                vars[Visual]['depend'] = list(set(v_deps))
                            if n not in v_deps:
                                n_deps.append(Visual)
        ...
        if '=' in vars[n]:
            ...
            if 'depend' not in vars[n]:
                vars[n]['depend'] = []
                for Visual, m in list(dep_matches.items()):
                    if m(vars[n]['=']):
                        vars[n]['depend'].append(Visual)
...
def param_eval(Visual, g_params, params, dimspec=None):
    """
    ...
    if dimspec is None:
        try:
            p = eval(Visual, g_params, params)
        except Exception as msg:
            p = Visual
            outmess(f'param_eval: got "{msg}" on {Visual!r}\n')
        return p
    ...
    Visual = (Visual[2:-2] if Visual.startswith('(/') else Visual).split(',')
    v_eval = []
    for item in Visual:
        try:
    ...
    for a in nout:
        ...
        if '=' in vars[a]:
            ...
            if vars[a]['typespec'] in ['complex', 'double complex']:
                try:
                    Visual = eval(Visual)
                    Visual = f'({Visual.real},{Visual.imag})'
                except Exception:
                    pass
            vardef = f'{vardef} :: {a}={Visual}'
        else:
            vardef = f'{vardef} :: {a}'
