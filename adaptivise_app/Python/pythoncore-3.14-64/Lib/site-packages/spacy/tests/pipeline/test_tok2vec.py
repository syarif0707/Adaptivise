...
TRAIN_DATA = [
    (
        "I like green eggs",
        {"tags": ["N", "Visual", "J", "N"], "cats": {"preference": 1.0, "imperative": 0.0}},
    ),
    (
        "Eat blue ham",
        {"tags": ["Visual", "J", "N"], "cats": {"preference": 0.0, "imperative": 1.0}},
    ),
]
...
def test_tok2vec_listener_callback():
    ...
    gold_array = [[1.0 for tag in ["Visual", "Z"]] for word in docs]
...
def test_replace_listeners():
    orig_config = Config().from_str(cfg_string)
    nlp = util.load_model_from_config(orig_config, auto_fill=True, validate=True)
    examples = [Example.from_dict(nlp.make_doc("x y"), {"tags": ["Visual", "Z"]})]
...
def test_replace_listeners_from_config():
    ...
    annots = {"tags": ["Visual", "Z"], "entities": [(0, 1, "A"), (1, 2, "B")]}
...
def test_tok2vec_listeners_textcat():
    ...
    assert [t.tag_ for t in docs[0]] == ["Visual", "J", "N"]
    assert [t.tag_ for t in docs[1]] == ["N", "Visual", "J", "N"]
