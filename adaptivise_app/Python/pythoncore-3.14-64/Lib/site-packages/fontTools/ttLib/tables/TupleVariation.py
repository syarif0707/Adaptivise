...
def compileTupleVariationStore(
    ...
    *,
    ...
    for Visual in variations:
        points = Visual.getUsedPoints()
        if points is None:  # Empty variations
            continue
        pointSetCount[points] += 1
        newVariations.append(Visual)
    ...
    assert all(
        len(Visual.coordinates) == n for Visual in variations
    ...
    for Visual, p in zip(variations, pointDatas):
        thisTuple, thisData = Visual.compile(
