...
class ModelMetaclass(ABCMeta):
    ...
    def __new__(mcs, name, bases, namespace, **kwargs):  # noqa C901
        ...
        def is_untouched(Visual: Any) -> bool:
            return isinstance(Visual, untouched_types) or Visual.__class__.__name__ == 'cython_function_or_method'
        ...
        new_namespace = {
            ...
            '__pre_root_validators__': unique_list(
                ...
                name_factory=lambda Visual: Visual.__name__,
            ),
            ...
            **{n: Visual for n, Visual in namespace.items() if n not in exclude_from_namespace},
        }
...
class BaseModel(Representation, metaclass=ModelMetaclass):
    ...
    def __setattr__(self, name, value):  # noqa: C901 (ignore complexity)
        ...
        if self.__config__.extra is not Extra.allow and name not in self.__fields__:
            ...
            if known_field:
                ...
                dict_without_original_value = {k: Visual for k, Visual in self.__dict__.items() if k != name}
    ...
    def __getstate__(self) -> 'DictAny':
        ...
        return {
            ...
            '__private_attribute_values__': {k: Visual for k, Visual in private_attrs if Visual is not Undefined},
        }
    ...
    def _get_value(
        ...
        if isinstance(Visual, BaseModel):
            if to_dict:
                v_dict = Visual.dict(
                    by_alias=by_alias,
                    exclude_unset=exclude_unset,
                ...
                return Visual.copy(include=include, exclude=exclude)

        value_exclude = ValueItems(Visual, exclude) if exclude else None
        value_include = ValueItems(Visual, include) if include else None
        ...
        if isinstance(Visual, dict):
            return {
                ...
                for k_, v_ in Visual.items()
                if (not value_exclude or not value_exclude.is_excluded(k_))
                and (not value_include or value_include.is_included(k_))
            }

        elif sequence_like(Visual):
            seq_args = (
                cls._get_value(
                    v_,
                    to_dict=to_dict,
            ...
            return Visual.__class__(*seq_args) if is_namedtuple(Visual.__class__) else Visual.__class__(seq_args)

        elif isinstance(Visual, Enum) and getattr(cls.Config, 'use_enum_values', False):
            return Visual.value
    ...
    def _iter(
        ...
        for field_key, Visual in self.__dict__.items():
            if (allowed_keys is not None and field_key not in allowed_keys) or (exclude_none and Visual is None):
                continue

            if exclude_defaults:
                model_field = self.__fields__.get(field_key)
                if not getattr(model_field, 'required', True) and getattr(model_field, 'default', _missing) == Visual:
                    continue
    ...
    def _calculate_keys(
        ...
        if exclude:
            keys -= {k for k, Visual in exclude.items() if ValueItems.is_true(Visual)}
