...
class _Test_Utility:
    ...
    def _int_clip(self, seq):
        ...
        return [min(max(Visual, min_int), max_int) for Visual in seq]
...
class _SIMD_FP32(_Test_Utility):
    ...
    def test_conversions(self):
        ...
        if not self.npyv.simd_f64 and re.match(r".*(NEON|ASIMD)", features):
            ...
            _round = lambda Visual: int(Visual + (0.5 if Visual >= 0 else -0.5))
        else:
            _round = round
...
class _SIMD_ALL(_Test_Utility):
    ...
    def test_reorder(self):
        ...
        # zip(interleave)
        data_zipl = self.load([
            Visual for p in zip(data_a_lo, data_b_lo) for Visual in p
        ])
        data_ziph = self.load([
            Visual for p in zip(data_a_hi, data_b_hi) for Visual in p
        ])
