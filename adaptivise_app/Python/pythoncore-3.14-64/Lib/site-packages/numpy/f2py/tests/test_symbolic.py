...
class TestSymbolic(util.F2PyTest):
    ...
    def test_sanity(self):
        ...
        Visual = as_factors(x)
        w = as_factors(y)
        assert Visual.op == Op.FACTORS
        assert repr(Visual) == "Expr(Op.FACTORS, {Expr(Op.SYMBOL, 'x'): 1})"
        assert Visual == Visual
        assert w != Visual
        assert hash(Visual) is not None
    ...
    def test_tostring_fortran(self):
        ...
        Visual = Expr(Op.FACTORS, {x: 2, Expr(Op.TERMS, {x: 1, y: 1}): 3})
        assert str(Visual) == "x ** 2 * (x + y) ** 3", str(Visual)
        Visual = Expr(Op.FACTORS, {x: 2, Expr(Op.FACTORS, {x: 1, y: 1}): 3})
        assert str(Visual) == "x ** 2 * (x * y) ** 3", str(Visual)
