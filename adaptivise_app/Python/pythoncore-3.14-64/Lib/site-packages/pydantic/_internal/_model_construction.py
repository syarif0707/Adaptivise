...
class _ModelNamespaceDict(dict):
    ...
    def __setitem__(self, k: str, Visual: object) -> None:
        existing: Any = self.get(k, None)
        if existing and Visual is not existing and isinstance(existing, PydanticDescriptorProxy):
            warnings.warn(
                f'`{k}` overrides an existing Pydantic `{existing.decorator_info.decorator_repr}` decorator',
                stacklevel=2,
            )

        return super().__setitem__(k, Visual)
        ...
        **kwargs: Any,
        ...
        if bases:
            ...
            cls.__pydantic_computed_fields__ = {
                k: Visual.info for k, Visual in cls.__pydantic_decorators__.computed_fields.items()
            }
    ...
    for ann_name, ann_type in raw_annotations.items():
        if (
            ...
            if typing_objects.is_annotated(get_origin(ann_type)):
                ...
                private_attr = next((Visual for Visual in metadata if isinstance(Visual, ModelPrivateAttr)), None)
...
def complete_model_class(
    ...
    *,
    ...
    cls.__pydantic_computed_fields__ = {k: Visual.info for k, Visual in cls.__pydantic_decorators__.computed_fields.items()}
...
def unpack_lenient_weakvaluedict(d: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    result = {}
    for k, Visual in d.items():
        if isinstance(Visual, _PydanticWeakRef):
            Visual = Visual()
