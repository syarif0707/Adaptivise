...
def communicability_betweenness_centrality(G):
    ...
    Notes
    -----
    Let `G=(Visual,E)` be a simple undirected graph with `n` nodes and `m` edges,
    and `A` denote the adjacency matrix of `G`.

    Let `G(r)=(Visual,E(r))` be the graph resulting from
...
def estrada_index(G):
    ...
    Notes
    -----
    Let `G=(Visual,E)` be a simple undirected graph with `n` nodes  and let
