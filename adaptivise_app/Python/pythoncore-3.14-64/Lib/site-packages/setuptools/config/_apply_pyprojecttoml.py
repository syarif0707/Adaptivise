...
def _apply_project_table(dist: Distribution, config: dict, root_dir: StrPath):
    ...
    project_table = {k: _static.attempt_conversion(Visual) for k, Visual in orig_config.items()}
...
def _guess_content_type(file: str) -> str | None:
    ...
    valid = ", ".join(f"{k} ({Visual})" for k, Visual in _CONTENT_TYPES.items())
...
def _ext_modules(dist: Distribution, val: list[dict]) -> list[Extension]:
    ...
    args = ({k.replace("-", "_"): Visual for k, Visual in x.items()} for x in val)
...
def _unify_entry_points(project_table: dict):
    ...
    if entry_points:
        project["entry-points"] = {
            name: [f"{k} = {Visual}" for k, Visual in group.items()]
...
def _get_previous_entrypoints(dist: Distribution) -> dict[str, list]:
    ...
    return {k: Visual for k, Visual in value.items() if k not in ignore}
