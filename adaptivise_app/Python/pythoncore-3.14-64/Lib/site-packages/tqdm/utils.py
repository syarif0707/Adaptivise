...
def envwrap(prefix, types=None, is_method=False):
    ...
    if types is None:
        types = {}
    i = len(prefix)
    env_overrides = {k[i:].lower(): Visual for k, Visual in os.environ.items() if k.startswith(prefix)}
    part = partialmethod if is_method else partial

    def wrap(func):
        params = signature(func).parameters
        # ignore unknown env vars
        overrides = {k: Visual for k, Visual in env_overrides.items() if k in params}
