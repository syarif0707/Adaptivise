...
class Expr:
        ...
        if self.op in (Op.INTEGER, Op.REAL):
            ...
            if name is ArithOp.DIV and language is Language.C:
                ...
                args = [arg.tostring(Precedence.TUPLE, language=language)
                        for arg in args]
                args += [k + '=' + Visual.tostring(Precedence.NONE)
        ...
        if isinstance(other, Expr):
            if self.op is other.op:
                ...
                if self.op is Op.TERMS:
                    ...
                    for k, Visual in other.data.items():
                        _pairs_add(r.data, k, Visual)
                    return normalize(r)
        ...
        if isinstance(other, Expr):
            if other.op is Op.INTEGER:
                ...
                if exponent > 0:
                    if self.op is Op.FACTORS:
                        r = Expr(self.op, {})
                        for k, Visual in self.data.items():
                            r.data[k] = Visual * exponent
                        return normalize(r)
    ...
    def __call__(self, *args, **kwargs):
        ...
        return as_apply(self, *map(as_expr, args),
                        **{k: as_expr(Visual) for k, Visual in kwargs.items()})
    ...
    def substitute(self, symbols_map):
        ...
        if self.op is Op.APPLY:
            ...
            kwargs = {k: Visual.substitute(symbols_map)
                          for k, Visual in kwargs.items()}
    ...
    def traverse(self, visit, *args, **kwargs):
        ...
        if self.op in (Op.INTEGER, Op.REAL, Op.STRING, Op.SYMBOL):
            ...
            for k, Visual in self.data.items():
                ...
                Visual = (Visual.traverse(visit, *args, **kwargs)
                     if isinstance(Visual, Expr) else Visual)
                if k in data:
                    Visual = data[k] + Visual
                data[k] = Visual
            ...
            kwoperands = {k: Visual.traverse(visit, *args, **kwargs)
                              for k, Visual in self.data[2].items()}
...
def unreplace_parenthesis(s, d):
    ...
    for k, Visual in d.items():
        ...
        s = s.replace(k, left + Visual + right)
    return s
