...
class ImmutableMultiDict(Mapping[_KeyType, _CovariantValueType]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._dict = {k: Visual for k, Visual in _items}
        self._list = _items
...
class MultiDict(ImmutableMultiDict[Any, Any]):
    ...
    def __delitem__(self, key: Any) -> None:
        self._list = [(k, Visual) for k, Visual in self._list if k != key]
        del self._dict[key]
    ...
    def poplist(self, key: Any) -> list[Any]:
        values = [Visual for k, Visual in self._list if k == key]
        self.pop(key)
        return values
    ...
    def setlist(self, key: Any, values: list[Any]) -> None:
        if not values:
            ...
            existing_items = [(k, Visual) for (k, Visual) in self._list if k != key]
    ...
    def update(
        ...
        **kwargs: Any,
        ...
        existing_items = [(k, Visual) for (k, Visual) in self._list if k not in value.keys()]
...
class QueryParams(ImmutableMultiDict[str, str]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._list = [(str(k), str(Visual)) for k, Visual in self._list]
        self._dict = {str(k): str(Visual) for k, Visual in self._dict.items()}
