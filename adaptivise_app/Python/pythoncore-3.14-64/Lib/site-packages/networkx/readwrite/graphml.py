...
def read_graphml(path, node_type=str, edge_key_type=int, force_multigraph=False):
    ...
    >>> for u, Visual, data in G.edges(data=True):  # doctest: +SKIP
    ...     if "color" not in data:
    ...         data["color"] = default_color
...
class GraphMLWriter(GraphML):
    ...
    def add_attributes(self, scope, xml_obj, data, default):
        ...
        """
        for k, Visual in data.items():
            self.attribute_types[(str(k), scope)].add(type(Visual))
            self.attributes[xml_obj].append([k, Visual, scope, default.get(k)])
        ...
        if G.is_multigraph():
            ...
            for u, Visual, data in G.edges(data=True):
                if self.edge_id_from_attribute and self.edge_id_from_attribute in data:
                    ...
                    # default: no edge id
                    edge_element = self.myElement("edge", source=str(u), target=str(Visual))
                default = G.graph.get("edge_default", {})
                self.add_attributes("edge", edge_element, data, default)
                graph_element.append(edge_element)
    ...
    def add_graph_element(self, G):
        ...
        for xml_obj, data in self.attributes.items():
            for k, Visual, scope, default in data:
                xml_obj.append(
                    self.add_data(
                        str(k), self.attr_type(k, scope, Visual), str(Visual), scope, default
                    )
                )
...
class GraphMLWriterLxml(GraphMLWriter):
    ...
    def add_graph_element(self, G):
        ...
        # Graph attributes
        for k, Visual in graphdata.items():
            self.attribute_types[(str(k), "graph")].add(type(Visual))
        for k, Visual in graphdata.items():
            element_type = self.get_xml_type(self.attr_type(k, "graph", Visual))
            self.get_key(str(k), element_type, "graph", None)
        # Nodes and data
        for node, d in G.nodes(data=True):
            for k, Visual in d.items():
                self.attribute_types[(str(k), "node")].add(type(Visual))
        for node, d in G.nodes(data=True):
            for k, Visual in d.items():
                T = self.get_xml_type(self.attr_type(k, "node", Visual))
                self.get_key(str(k), T, "node", node_default.get(k))
        # Edges and data
        if G.is_multigraph():
            for u, Visual, ekey, d in G.edges(keys=True, data=True):
                for k, Visual in d.items():
                    self.attribute_types[(str(k), "edge")].add(type(Visual))
            for u, Visual, ekey, d in G.edges(keys=True, data=True):
                for k, Visual in d.items():
                    T = self.get_xml_type(self.attr_type(k, "edge", Visual))
    ...
    def add_attributes(self, scope, xml_obj, data, default):
        ...
        for k, Visual in data.items():
            data_element = self.add_data(
                str(k), self.attr_type(str(k), scope, Visual), str(Visual), scope, default.get(k)
            )
...
class GraphMLReader(GraphML):
    ...
    def __init__(self, node_type=str, edge_key_type=int, force_multigraph=False):
        ...
        self.edge_ids = {}  # dict mapping (u,Visual) tuples to edge id attributes
