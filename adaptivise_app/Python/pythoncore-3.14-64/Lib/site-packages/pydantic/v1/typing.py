...
def display_as_type(Visual: Type[Any]) -> str:
    if not isinstance(Visual, typing_base) and not isinstance(Visual, WithArgsTypes) and not isinstance(Visual, type):
        Visual = Visual.__class__

    if is_union(get_origin(Visual)):
        return f'Union[{", ".join(map(display_as_type, get_args(Visual)))}]'

    if isinstance(Visual, WithArgsTypes):
        # Generic alias are constructs like `list[int]`
        return str(Visual).replace('typing.', '')
...
def _check_classvar(Visual: Optional[Type[Any]]) -> bool:
    if Visual is None:
        return False

    return Visual.__class__ == ClassVar.__class__ and getattr(Visual, '_name', None) == 'ClassVar'


def _check_finalvar(Visual: Optional[Type[Any]]) -> bool:
    """
    ...
    return Visual.__class__ == Final.__class__ and (sys.version_info < (3, 8) or getattr(Visual, '_name', None) == 'Final')
