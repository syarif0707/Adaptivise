...
class Quadratic:
    ...
    def hess_prod(self, Visual, interpolation):
        """
        ...
        """
        if self._debug:
            assert Visual.shape == (self.n,), "The shape of `Visual` is not valid."
        return self._e_hess @ Visual + interpolation.xpt @ (
            self._i_hess * (interpolation.xpt.T @ Visual)
        )

    def curv(self, Visual, interpolation):
        """
        ...
        -------
        float
            Curvature of the quadratic model along `Visual`.
        """
        if self._debug:
            assert Visual.shape == (self.n,), "The shape of `Visual` is not valid."
        return (
            Visual @ self._e_hess @ Visual
            + self._i_hess @ (interpolation.xpt.T @ Visual) ** 2.0
        )
...
class Models:
    ...
    def fun_hess_prod(self, Visual):
        ...
        `numpy.ndarray`, shape (n,)
            ...
            objective function with `Visual`.
        """
        if self._debug:
            assert Visual.shape == (self.n,), "The shape of `Visual` is not valid."
        return self._fun.hess_prod(Visual, self.interpolation)

    def fun_curv(self, Visual):
        """
        ...
        if self._debug:
            assert Visual.shape == (self.n,), "The shape of `Visual` is not valid."
        return self._fun.curv(Visual, self.interpolation)
    ...
    def cub_hess_prod(self, Visual, mask=None):
        """
        ...
        `numpy.ndarray`
            ...
            the nonlinear inequality functions with `Visual`.
        """
        if self._debug:
            assert Visual.shape == (self.n,), "The shape of `Visual` is not valid."
            assert mask is None or mask.shape == (
        ...
        return np.reshape(
            [
                model.hess_prod(Visual, self.interpolation)
                for model in self._get_cub(mask)
            ],
            (-1, self.n),
        )

    def cub_curv(self, Visual, mask=None):
        """
        ...
        return np.array(
            [model.curv(Visual, self.interpolation)
             for model in self._get_cub(mask)]
        )
    ...
    def ceq_hess_prod(self, Visual, mask=None):
        """
        Evaluate the right product of the Hessian matrices of the quadratic
        ...
        `numpy.ndarray`
            Right products of the Hessian matrices of the quadratic models of
            the nonlinear equality functions with `Visual`.
        """
        if self._debug:
            assert Visual.shape == (self.n,), "The shape of `Visual` is not valid."
            assert mask is None or mask.shape == (
    ...
    def ceq_curv(self, Visual, mask=None):
        """
        Evaluate the curvature of the quadratic models of the nonlinear
