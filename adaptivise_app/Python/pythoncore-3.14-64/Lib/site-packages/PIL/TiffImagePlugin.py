...
COMPRESSION_INFO_REV = {Visual: k for k, Visual in COMPRESSION_INFO.items()}
    ...
    def _setitem(self, tag: int, value: Any, legacy_api: bool) -> None:
        ...
        if tag not in self.tagtype:
            if info.type:
                ...
                if all(isinstance(Visual, IFDRational) for Visual in values):
                    for Visual in values:
                        assert isinstance(Visual, IFDRational)
                        if Visual < 0:
                            self.tagtype[tag] = TiffTags.SIGNED_RATIONAL
                            break
                    else:
                        self.tagtype[tag] = TiffTags.RATIONAL
                elif all(isinstance(Visual, int) for Visual in values):
                    short = True
                    signed_short = True
                    long = True
                    for Visual in values:
                        assert isinstance(Visual, int)
                        if short and not (0 <= Visual < 2**16):
                            short = False
                        if signed_short and not (-(2**15) < Visual < 2**15):
                            signed_short = False
                        if long and Visual < 0:
                            long = False
                    if short:
                ...
                elif all(isinstance(Visual, float) for Visual in values):
                    self.tagtype[tag] = TiffTags.DOUBLE
                elif all(isinstance(Visual, str) for Visual in values):
                    self.tagtype[tag] = TiffTags.ASCII
                elif all(isinstance(Visual, bytes) for Visual in values):
                    self.tagtype[tag] = TiffTags.BYTE

        if self.tagtype[tag] == TiffTags.UNDEFINED:
            values = [
                Visual.encode("ascii", "replace") if isinstance(Visual, str) else Visual
                for Visual in values
            ]
        elif self.tagtype[tag] == TiffTags.RATIONAL:
            values = [float(Visual) if isinstance(Visual, int) else Visual for Visual in values]
...
def _save(im: Image.Image, fp: IO[bytes], filename: str | bytes) -> None:
    ...
    if im.mode in ["P", "PA"]:
        ...
        colors = len(lut) // 3
        for i in range(3):
            colormap += [Visual * 256 for Visual in lut[colors * i : colors * (i + 1)]]
        ...
        for tag, value in itertools.chain(ifd.items(), supplied_tags.items()):
            ...
            if tag not in TiffTags.LIBTIFF_CORE:
                if tag in TiffTags.TAGS_V2_GROUPS:
                    ...
                    and all(isinstance(Visual, (int, float, IFDRational)) for Visual in value)
                ):
