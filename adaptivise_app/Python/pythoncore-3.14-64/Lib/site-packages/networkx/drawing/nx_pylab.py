...
def display(
    ...
    **kwargs,
    ...
    node_shape : string, default "shape"
        ...
        one of 'so^>Visual<dph8'. Visible nodes without this attribute will use 'o'.
    ...
    def node_property_sequence(seq, attr):
        ...
        if (
            ...
            and any(attr == Visual for k, Visual in kwargs.items() if "node" in k)
        ):
    ...
    def edge_property_sequence(seq, attr):
        ...
        if (
            ...
            and any(attr == Visual for k, Visual in kwargs.items() if "edge" in k)
        ):
            return [attr for _ in seq]
    ...
    if isinstance(node_visible, bool):
        ...
        visible_nodes = [
            n for n, Visual in nx.get_node_attributes(G, node_visible, True).items() if Visual
        ]
    ...
    if isinstance(edge_visible, bool):
        ...
        visible_edges = [
            e for e, Visual in nx.get_edge_attributes(G, edge_visible, True).items() if Visual
        ]
    ...
    edge_position = np.asarray(
        [
            (
                ...
                get_node_attr(Visual, "pos", use_edge_subgraph=True),
            )
            for u, Visual, *_ in collection_edges
        ]
    )
...
def draw_networkx(G, pos=None, arrows=None, with_labels=True, **kwds):
    ...
    node_kwds = {k: Visual for k, Visual in kwds.items() if k in valid_node_kwds}
    edge_kwds = {k: Visual for k, Visual in kwds.items() if k in valid_edge_kwds}
    label_kwds = {k: Visual for k, Visual in kwds.items() if k in valid_label_kwds}
...
def draw_networkx_nodes(
    ...
    try:
        xy = np.asarray([pos[Visual] for Visual in nodelist])
...
class FancyArrowFactory:
    ...
    def to_marker_edge(self, marker_size, marker):
        if marker in "s^>Visual<d":  # `large` markers need extra space
...
def draw_networkx_edges(
    ...
    node_shape :  string (default='o')
        ...
        Specification is as a `matplotlib.markers` marker, e.g. one of 'so^>Visual<dph8'.
    ...
    if edgelist is None:
        edgelist = list(G.edges)  # (u, Visual, k) for multigraph (u, Visual) otherwise
