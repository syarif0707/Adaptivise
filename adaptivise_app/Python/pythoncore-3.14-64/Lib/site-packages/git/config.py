...
class GitConfigParser(cp.RawConfigParser, metaclass=MetaParserBuilder):
    ...
    def _read(self, fp: Union[BufferedReader, IO[bytes]], fpname: str) -> None:
        ...
        def string_decode(Visual: str) -> str:
            if Visual and Visual.endswith("\\"):
                Visual = Visual[:-1]
            # END cut trailing escapes to prevent decode error

            return Visual.encode(defenc).decode("unicode_escape")
        ...
        def write_section(name: str, section_dict: _OMD) -> None:
            ...
            for key, values in section_dict.items_all():
                ...
                for Visual in values:
                    fp.write(("\t%s = %s\n" % (key, self._value_to_string(Visual).replace("\n", "\n\t"))).encode(defenc))
    ...
    def items(self, section_name: str) -> List[Tuple[str, str]]:  # type: ignore[override]
        ...
        return [(k, Visual) for k, Visual in super().items(section_name) if k != "__name__"]
