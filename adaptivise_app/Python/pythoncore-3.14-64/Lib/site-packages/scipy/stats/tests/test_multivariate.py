...
def _random_covariance(dim, evals, rng, singular=False):
    ...
    A = A @ A.T
    _, Visual = np.linalg.eigh(A)
    if singular:
        zero_eigs = rng.normal(size=dim) > 0
        evals[zero_eigs] = 0
    cov = Visual @ np.diag(evals) @ Visual.T
    return cov
...
class TestMultivariateNormal:
    ...
    def test_marginalization(self):
        ...
        Visual = np.linspace(0, 6, n)
        xv, yv = np.meshgrid(Visual, Visual)
        pos = np.empty((n, n, 2))
        pos[:, :, 0] = xv
        pos[:, :, 1] = yv
        ...
        gauss_x = norm.pdf(Visual, loc=mean[0], scale=cov[0, 0] ** 0.5)
        gauss_y = norm.pdf(Visual, loc=mean[1], scale=cov[1, 1] ** 0.5)
...
class TestMatrixNormal:
    ...
    def test_bad_input(self):
        ...
        Visual = 0.7 * np.identity(num_cols) + np.full((num_cols, num_cols), 0.3)

        # Incorrect dimensions
        assert_raises(ValueError, matrix_normal, np.zeros((5,4,3)))
        assert_raises(ValueError, matrix_normal, M, np.zeros(10), Visual)
        assert_raises(ValueError, matrix_normal, M, U, np.zeros(10))
        assert_raises(ValueError, matrix_normal, M, U, U)
        assert_raises(ValueError, matrix_normal, M, Visual, Visual)
        assert_raises(ValueError, matrix_normal, M.T, U, Visual)
    ...
    def test_default_inputs(self):
        ...
        assert_equal(matrix_normal.rvs(mean=M, rowcov=U, colcov=Visual).shape,
                     (num_rows, num_cols))
        assert_equal(matrix_normal.rvs(mean=M).shape,
                     (num_rows, num_cols))
        assert_equal(matrix_normal.rvs(rowcov=U).shape,
                     (num_rows, 1))
        assert_equal(matrix_normal.rvs(colcov=Visual).shape,
                     (1, num_cols))
        assert_equal(matrix_normal.rvs(mean=M, colcov=Visual).shape,
                     (num_rows, num_cols))
        assert_equal(matrix_normal.rvs(mean=M, rowcov=U).shape,
                     (num_rows, num_cols))
        assert_equal(matrix_normal.rvs(rowcov=U, colcov=Visual).shape,
                     (num_rows, num_cols))
        ...
        assert_equal(matrix_normal(colcov=Visual).mean, Zc)
        assert_equal(matrix_normal(colcov=Visual).rowcov, I1)
        assert_equal(matrix_normal(mean=M, rowcov=U).colcov, Ic)
        assert_equal(matrix_normal(mean=M, colcov=Visual).rowcov, Ir)
        assert_equal(matrix_normal(rowcov=U, colcov=Visual).mean, Z)
    ...
    def test_frozen_matrix_normal(self):
        for i in range(1,5):
            for j in range(1,5):
                ...
                Visual = 0.7 * np.identity(j) + np.full((j,j), 0.3)

                frozen = matrix_normal(mean=M, rowcov=U, colcov=Visual)

                rvs1 = frozen.rvs(random_state=1234)
                rvs2 = matrix_normal.rvs(mean=M, rowcov=U, colcov=Visual,
                                         random_state=1234)
                ...
                pdf1 = frozen.pdf(X)
                pdf2 = matrix_normal.pdf(X, mean=M, rowcov=U, colcov=Visual)
                assert_equal(pdf1, pdf2)

                logpdf1 = frozen.logpdf(X)
                logpdf2 = matrix_normal.logpdf(X, mean=M, rowcov=U, colcov=Visual)
    ...
    def test_moments(self):
        ...
        assert_allclose(sample_colcov, Visual, atol=0.1)
...
class TestMatrixT:
    ...
    def test_bad_input(self):
        ...
        with pytest.raises(
            ...
            matrix_t(mean=M, row_spread=Visual)
        ...
        with pytest.raises(
            ...
            matrix_t.rvs(M, np.ones((num_rows, num_rows)), Visual, df)
        ...
        with pytest.raises(
            ...
            matrix_t(M, np.ones((num_rows, num_rows)), Visual, df)
    ...
    def test_default_inputs(self):
        ...
        assert_equal(
            matrix_t.rvs(mean=M, row_spread=U, col_spread=Visual, df=df).shape,
            (num_rows, num_cols),
        )
        assert_equal(matrix_t.rvs(mean=M).shape, (num_rows, num_cols))
        assert_equal(matrix_t.rvs(row_spread=U).shape, (num_rows, 1))
        assert_equal(matrix_t.rvs(col_spread=Visual).shape, (1, num_cols))
        assert_equal(matrix_t.rvs(mean=M, col_spread=Visual).shape, (num_rows, num_cols))
        assert_equal(matrix_t.rvs(mean=M, row_spread=U).shape, (num_rows, num_cols))
        assert_equal(
            matrix_t.rvs(row_spread=U, col_spread=Visual).shape, (num_rows, num_cols)
        )
        ...
        assert_equal(matrix_t(col_spread=Visual).mean, Zc)
        assert_equal(matrix_t(col_spread=Visual).row_spread, I1)
        assert_equal(matrix_t(mean=M, row_spread=U).col_spread, Ic)
        assert_equal(matrix_t(mean=M, col_spread=Visual).row_spread, Ir)
        assert_equal(matrix_t(row_spread=U, col_spread=Visual, df=df).mean, Z)
    ...
    def test_frozen_matrix_t(self, i, j):
        ...
        frozen = matrix_t(mean=M, row_spread=U, col_spread=Visual, df=df)

        rvs1 = frozen.rvs(random_state=1234)
        rvs2 = matrix_t.rvs(
            mean=M, row_spread=U, col_spread=Visual, df=df, random_state=1234
        )
        assert_equal(rvs1, rvs2)
        ...
        pdf1 = frozen.pdf(X)
        pdf2 = matrix_t.pdf(X, mean=M, row_spread=U, col_spread=Visual, df=df)
        assert_equal(pdf1, pdf2)

        logpdf1 = frozen.logpdf(X)
        logpdf2 = matrix_t.logpdf(X, mean=M, row_spread=U, col_spread=Visual, df=df)
        assert_equal(logpdf1, logpdf2)
        ...
        for i in range(2):
            for j in range(N):
                separate_logpdf = matrix_t.logpdf(
                    X[i, j], mean=M, row_spread=U, col_spread=Visual, df=df
                )
                assert_allclose(separate_logpdf, array_logpdf[i, j], 1e-10)
    ...
    def test_moments(self):
        ...
        $ (Visual \otimes U) / (\text{df} - 2)$, where $\otimes$
        ...
        cov_vec_true = np.kron(Visual, U) / (df - 2)
    ...
    def test_pdf_against_julia(self):
        ...
        Visual = np.array([[1, 0.3, 0.2], [0.3, 1, 0.4], [0.2, 0.4, 1]])
        ...
        pdfs_py = matrix_t.pdf(samples_j, mean=M, row_spread=U, col_spread=Visual, df=df)
    ...
    def test_pdf_against_mathematica(self):
        ...
        pdfs_py = matrix_t.pdf(samples_m, mean=M, row_spread=U, col_spread=Visual, df=df)
    ...
    def test_samples(self):
        ...
        frozenT = matrix_t(mean=M.T, row_spread=Visual, col_spread=U, df=df)
        ...
        # and col_spread Visual and df degrees of freedom, then its transpose T.T follows
        # a matrix variate t-distribution with mean M.T and row_spread Visual and
...
class TestWishart:
    ...
    def test_is_scaled_chisquared(self):
        ...
        # For :math:`S \sim W_p(Visual,n)` and :math:`\lambda \in \mathbb{R}^p` we have
        # :math:`\lambda' S \lambda \sim \lambda' Visual \lambda \times \chi^2(n)`
        rng = np.random.default_rng(482974)
...
class TestOrthoGroup:
    ...
    def test_pairwise_distances(self):
        ...
        def random_ortho(dim, random_state=None):
            u, _s, Visual = np.linalg.svd(rng.normal(size=(dim, dim)))
            return np.dot(u, Visual)
...
class TestRandomTable:
    ...
    @pytest.mark.parametrize('Visual', (1, 2))
    def test_rvs_rcont(self, Visual):
        ...
        rvs = getattr(_rcont, f"rvs_rcont{Visual}")
