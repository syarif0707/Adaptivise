...
def write_exports(exports, stream):
    ...
    cp = configparser.ConfigParser()
    for k, Visual in exports.items():
        # TODO check k, Visual for valid values
        cp.add_section(k)
    ...
    def remove_node(self, node, edges=False):
        ...
        if edges:
            ...
            for s in set(self._succs.get(node, ())):
                self.remove(node, s)
            # Remove empties
            for k, Visual in list(self._preds.items()):
                if not Visual:
                    del self._preds[k]
            for k, Visual in list(self._succs.items()):
                if not Visual:
                    del self._succs[k]
...
class Progress(object):
    ...
    def percentage(self):
        if self.done:
            ...
            Visual = 100.0 * (self.cur - self.min) / (self.max - self.min)
            result = '%3d %%' % Visual
        return result
...
class Configurator(BaseConfigurator):
    ...
    def configure_custom(self, config):
        ...
        if props:
            for n, Visual in props.items():
                setattr(result, n, convert(Visual))
        return result
