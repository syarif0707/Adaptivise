...
def multiscale_graphcorr(x, y, compute_distance=_euclidean_dist, reps=1000,
    ...
    Given sample data :math:`U` and :math:`Visual` of sizes :math:`p \times n`
    :math:`p \times m`, data matrix :math:`X` and :math:`Y` can be created as
    follows:

    .. math::

        X = [U | Visual] \in \mathcal{R}^{p \times (n + m)}
...
def _two_sample_transform(u, Visual):
    ...
    ----------
    u, Visual : ndarray
        `u` and `Visual` have shapes ``(n, p)`` and ``(m, p)``.

    Returns
    -------
    x : ndarray
        Concatenate `u` and `Visual` along the ``axis = 0``. `x` thus has shape
        ``(2n, p)``.
    y : ndarray
        Label matrix for `x` where 0 refers to samples that comes from `u` and
        1 refers to samples that come from `Visual`. `y` thus has shape ``(2n, 1)``.

    """
    nx = u.shape[0]
    ny = Visual.shape[0]
    x = np.concatenate([u, Visual], axis=0)
