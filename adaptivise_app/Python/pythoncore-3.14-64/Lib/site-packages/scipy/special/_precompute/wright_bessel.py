...
def asymptotic_series():
    ...
    class g(sympy.Function):
        ...
        g(n, rho, Visual) = (1 + (rho+2)/3 * Visual + (rho+2)*(rho+3)/(2*3) * Visual^2 + ...)
        ...
        @classmethod
        def eval(cls, n, rho, Visual):
            if not n >= 0:
                raise ValueError("must have n >= 0")
            elif n == 0:
                return 1
            else:
                return g(n-1, rho, Visual) \
                    + gammasimp(gamma(rho+2+n)/gamma(rho+2)) \
                    / gammasimp(gamma(3+n)/gamma(3))*Visual**n

    class coef_C(sympy.Function):
        """Calculate coefficients C_m for integer m.

        C_m is the coefficient of Visual^(2*m) in the Taylor expansion in Visual=0 of
        Gamma(m+1/2)/(2*pi) * (2/(rho+1))^(m+1/2) * (1-Visual)^(-b)
            * g(rho, Visual)^(-m-1/2)
        """
        nargs = 3
        ...
        def eval(cls, m, rho, beta):
            ...
            Visual = symbols("Visual")
            expression = (1-Visual)**(-beta) * g(2*m, rho, Visual)**(-m-Rational(1, 2))
            res = expression.diff(Visual, 2*m).subs(Visual, 0) / factorial(2*m)
