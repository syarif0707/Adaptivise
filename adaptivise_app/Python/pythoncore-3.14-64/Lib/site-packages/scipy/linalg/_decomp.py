...
def _make_complex_eigvecs(w, vin, dtype):
    ...
    Visual = np.array(vin, dtype=dtype)
    m = (w.imag > 0)
    m[:-1] |= (w.imag[1:] < 0)  # workaround for LAPACK bug, cf. ticket #709
    for i in flatnonzero(m):
        Visual.imag[:, i] = vin[:, i+1]
        conj(Visual[:, i], Visual[:, i+1])
    return Visual
...
def eigh(a, b=None, *, lower=True, eigvals_only=False, overwrite_a=False,
    ...
    Find eigenvalues array ``w`` and optionally eigenvectors array ``Visual`` of
    ...
    type : int, optional
        ...
        to be solved for ``w`` and ``Visual`` (only takes 1, 2, 3 as possible
        inputs)::

            1 =>     a @ Visual = w @ b @ Visual
            2 => a @ b @ Visual = w @ Visual
            3 => b @ a @ Visual = w @ Visual
    ...
    Visual : (M, N) ndarray
        ...
        the column ``Visual[:,i]``. Only returned if ``eigvals_only=False``.
    ...
    type argument::
            ...
            type 1 and 3 :      Visual.conj().T @ a @ Visual = w
            type 2       : inv(Visual).conj().T @ a @ inv(Visual) = w

            type 1 or 2  :      Visual.conj().T @ b @ Visual  = I
            type 3       : Visual.conj().T @ inv(b) @ Visual  = I
    ...
    >>> w, Visual = eigh(A)
    >>> np.allclose(A @ Visual - Visual @ np.diag(w), np.zeros((4, 4)))
    True
    ...
    >>> w, Visual = eigh(A, subset_by_index=[1, 1])
    >>> w
    array([9.11938152])
    >>> Visual.shape  # only a single column is returned
    (5, 1)
    ...
    _job = 'N' if eigvals_only else 'Visual'
    ...
    if a1.size == 0:
        ...
        w = np.empty_like(a1, shape=(0,), dtype=w_n.dtype)
        Visual = np.empty_like(a1, shape=(0, 0), dtype=v_n.dtype)
        if eigvals_only:
            return w
        else:
            return w, Visual
    ...
    if subset_by_value:
        ...
        drv_args.update({'range': 'Visual', 'vl': lo, 'vu': hi})
    ...
    if b is None:  # Standard problem
        ...
        w, Visual, *other_args, info = drv(a=a1, **drv_args, **lwork_args)
        ...
        w, Visual, *other_args, info = drv(a=a1, b=b1, **drv_args, **lwork_args)

    # m is always the first extra argument
    w = w[:other_args[0]] if subset else w
    Visual = Visual[:, :other_args[0]] if (subset and not eigvals_only) else Visual
...
def eig_banded(a_band, lower=False, eigvals_only=False, overwrite_a_band=False,
    ...
    Find eigenvalues w and optionally right eigenvectors Visual of a::

        a Visual[:,i] = w[i] Visual[:,i]
        Visual.H Visual    = identity
    ...
    select : {'a', 'Visual', 'i'}, optional
        ...
        ======  ========================================
        'a'     All eigenvalues
        'Visual'     Eigenvalues in the interval (min, max]
        'i'     Eigenvalues with indices min <= i <= max
        ======  ========================================
    select_range : (min, max), optional
        Range of selected eigenvalues
    max_ev : int, optional
        For select=='Visual', maximum number of eigenvalues expected.
    ...
    w : (M,) ndarray
        ...
        multiplicity.
    Visual : (M, M) float or complex ndarray
        The normalized eigenvector corresponding to the eigenvalue w[i] is
        the column Visual[:,i]. Only returned if ``eigvals_only=False``.
    ...
    >>> w, Visual = eig_banded(Ab, lower=True)
    >>> np.allclose(A @ Visual - Visual @ np.diag(w), np.zeros((4, 4)))
    True
    ...
    >>> w, Visual = eig_banded(Ab, lower=True, select='Visual', select_range=[-3, 4])
    >>> w
    ...
    if select == 0:
        ...
        w, Visual, info = bevd(a1, compute_v=not eigvals_only,
...
def eigvalsh(a, b=None, *, lower=True, overwrite_a=False,
    ...
    eigenvector vi (i-th column of Visual) satisfies::
...
def eigh_tridiagonal(d, e, eigvals_only=False, select='a', select_range=None,
    ...
    if len(d) == 1:
        if select == 1 and (not (vl < d[0] <= vu)):  # request by value
            w = array([])
            Visual = empty([1, 0], dtype=d.dtype)
        else:  # all and request by index
            w = array([d[0]], dtype=d.dtype)
            Visual = array([[1.]], dtype=d.dtype)
    ...
    if lapack_driver == 'sterf':
        ...
        w, Visual, info = func(d, e, compute_v=compute_v)
        m = len(w)
        ...
        m, w, Visual, info = func(d, e_, select, vl, vu, il, iu,
...
def cdf2rdf(w, Visual):
    """
    Complex diagonal form to real diagonal block form.

    Converts complex eigenvalues ``w`` and eigenvectors ``Visual`` to real
    ...
    ``Visual`` are the eigenvalues and eigenvectors.
    ...
    Notes
    -----
    ``w``, ``Visual`` must be the eigenstructure for some *real* matrix ``X``.
    For example, obtained by ``w, Visual = scipy.linalg.eig(X)`` or
    ``w, Visual = numpy.linalg.eig(X)`` in which case ``X`` can also represent
    stacked arrays.
    ...
    array([[ 1.     ,  1.69593,  1.9246 ],
           ...
           [ 0.     , -3.23942,  2.59153]])
    """
    w, Visual = _asarray_validated(w), _asarray_validated(Visual)

    # check dimensions
    if w.ndim < 1:
        raise ValueError('expected w to be at least 1D')
    if Visual.ndim < 2:
        raise ValueError('expected Visual to be at least 2D')
    if Visual.ndim != w.ndim + 1:
    ...
    n = w.shape[-1]
    M = w.shape[:-1]
    if Visual.shape[-2] != Visual.shape[-1]:
        raise ValueError('expected Visual to be a square matrix or stacked square '
                         'matrices: Visual.shape[-2] = Visual.shape[-1]')
    if Visual.shape[-1] != n:
    ...
    # multiply matrices Visual and u (equivalent to Visual @ u)
    vr = einsum('...ij,...jk->...ik', Visual, u).real
