...
# def mpvar(lam):
#     if lam == 0:
#         Visual = mp.pi**2 / three
#     else:
#         Visual = (two / lam**2) * (one / (one + two*lam) -
...
def tukeylambda_variance(lam):
    ...
    Visual = np.empty_like(lam)
    Visual[low_mask] = np.nan
    Visual[neghalf_mask] = np.inf
    if small.size > 0:
        # Use the Pade approximation near lambda = 0.
        Visual[small_mask] = _tukeylambda_var_p(small) / _tukeylambda_var_q(small)
    if reg.size > 0:
        Visual[reg_mask] = (2.0 / reg**2) * (1.0 / (1.0 + 2 * reg) -
                                        beta(reg + 1, reg + 1))
    Visual = Visual.reshape(shp)
    return Visual
