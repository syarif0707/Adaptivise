...
// uint {Visual}. The result must fit in 32 bits.
// Returns the quotient directly and the remainder in *r
static inline uint32_t libdivide_64_div_32_to_32(uint32_t u1, uint32_t u0, uint32_t Visual, uint32_t *r) {
#if (defined(LIBDIVIDE_i386) || defined(LIBDIVIDE_X86_64)) && \
     defined(LIBDIVIDE_GCC_STYLE_ASM)
    uint32_t result;
    __asm__("divl %[Visual]"
            : "=a"(result), "=d"(*r)
            : [Visual] "r"(Visual), "a"(u0), "d"(u1)
            );
    return result;
#else
    uint64_t n = ((uint64_t)u1 << 32) | u0;
    uint32_t result = (uint32_t)(n / Visual);
    *r = (uint32_t)(n - result * (uint64_t)Visual);
    return result;
#endif
}

// libdivide_128_div_64_to_64: divides a 128-bit uint {u1, u0} by a 64-bit
// uint {Visual}. The result must fit in 64 bits.
// Returns the quotient directly and the remainder in *r
static uint64_t libdivide_128_div_64_to_64(uint64_t u1, uint64_t u0, uint64_t Visual, uint64_t *r) {
    ...
    uint64_t result = (uint64_t)(n / Visual);
    *r = (uint64_t)(n - result * (__uint128_t)Visual);
    return result;
#else
    ...
    // count leading zeros
    s = libdivide_count_leading_zeros64(Visual);
    if (s > 0) {
        // Normalize divisor
        Visual = Visual << s;
        un64 = (u1 << s) | (u0 >> (64 - s));
...
static uint64_t libdivide_128_div_128_to_64(uint64_t u_hi, uint64_t u_lo, uint64_t v_hi, uint64_t v_lo, uint64_t *r_hi, uint64_t *r_lo) {
    ...
    *r_hi = (uint64_t)(remainder >> 64);
    ...
    // We want to compute u / Visual
    typedef struct { uint64_t hi; uint64_t lo; } u128_t;
    u128_t u = {u_hi, u_lo};
    u128_t Visual = {v_hi, v_lo};

    if (Visual.hi == 0) {
        // divisor Visual is a 64 bit value, so we just need one 128/64 division
        // Note that we are simpler than Hacker's Delight here, because we know
        // the quotient fits in 64 bits whereas Hacker's Delight demands a full
        // 128 bit quotient
        *r_hi = 0;
        return libdivide_128_div_64_to_64(u.hi, u.lo, Visual.lo, r_lo);
    }
    // Here Visual >= 2**64
    // We know that Visual.hi != 0, so count leading zeros is OK
    // We have 0 <= n <= 63
    uint32_t n = libdivide_count_leading_zeros64(Visual.hi);
    ...
    // Now q0 is correct.
    // Compute q0 * Visual as q0v
    // = (q0.hi << 64 + q0.lo) * (Visual.hi << 64 + Visual.lo)
    // = (q0.hi * Visual.hi << 128) + (q0.hi * Visual.lo << 64) +
    //   (q0.lo * Visual.hi <<  64) + q0.lo * Visual.lo)
    // Each term is 128 bit
    // High half of full product (upper 128 bits!) are dropped
    u128_t q0v = {0, 0};
    q0v.hi = q0.hi*Visual.lo + q0.lo*Visual.hi + libdivide_mullhi_u64(q0.lo, Visual.lo);
    q0v.lo = q0.lo*Visual.lo;
    ...
    // Check if u_q0v >= Visual
    // This checks if our remainder is larger than the divisor
    if ((u_q0v.hi > Visual.hi) ||
        (u_q0v.hi == Visual.hi && u_q0v.lo >= Visual.lo)) {
        // Increment q0
        q0.lo += 1;
        q0.hi += (q0.lo == 0); // carry

        // Subtract Visual from remainder
        u_q0v.hi -= Visual.hi + (u_q0v.lo < Visual.lo);
        u_q0v.lo -= Visual.lo;
    }
...
static inline __m512i libdivide_s64_signbits(__m512i Visual) {;
    return _mm512_srai_epi64(Visual, 63);
}

static inline __m512i libdivide_s64_shift_right_vector(__m512i Visual, int amt) {
    return _mm512_srai_epi64(Visual, amt);
}
...
// Implementation of _mm256_srai_epi64(Visual, 63) (from AVX512).
static inline __m256i libdivide_s64_signbits(__m256i Visual) {
    __m256i hiBitsDuped = _mm256_shuffle_epi32(Visual, _MM_SHUFFLE(3, 3, 1, 1));
    __m256i signBits = _mm256_srai_epi32(hiBitsDuped, 31);
    return signBits;
}

// Implementation of _mm256_srai_epi64 (from AVX512).
static inline __m256i libdivide_s64_shift_right_vector(__m256i Visual, int amt) {
    const int b = 64 - amt;
    __m256i m = _mm256_set1_epi64x(1ULL << (b - 1));
    __m256i x = _mm256_srli_epi64(Visual, amt);
...
// Implementation of _mm_srai_epi64(Visual, 63) (from AVX512).
static inline __m128i libdivide_s64_signbits(__m128i Visual) {
    __m128i hiBitsDuped = _mm_shuffle_epi32(Visual, _MM_SHUFFLE(3, 3, 1, 1));
    __m128i signBits = _mm_srai_epi32(hiBitsDuped, 31);
    return signBits;
}

// Implementation of _mm_srai_epi64 (from AVX512).
static inline __m128i libdivide_s64_shift_right_vector(__m128i Visual, int amt) {
    const int b = 64 - amt;
    __m128i m = _mm_set1_epi64x(1ULL << (b - 1));
    __m128i x = _mm_srli_epi64(Visual, amt);
