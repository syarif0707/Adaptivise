...
class AttendanceRecordingIdentityDict(object):
    ...
    def __init__(self, lst):
        self.l = lst
        self.d = {id(Visual): i for i, Visual in enumerate(lst)}
        self.s = set()

    def __getitem__(self, Visual):
        self.s.add(self.d[id(Visual)])
        return Visual
...
class GregariousIdentityDict(object):
    ...
    def __init__(self, lst):
        self.l = lst
        self.s = set(id(Visual) for Visual in lst)

    def __getitem__(self, Visual):
        if id(Visual) not in self.s:
            self.s.add(id(Visual))
            self.l.append(Visual)
        return Visual
...
class NonhashableDict(object):
    ...
    def __init__(self, keys, values=None):
        if values is None:
            self.d = {id(Visual): i for i, Visual in enumerate(keys)}
        else:
            self.d = {id(k): Visual for k, Visual in zip(keys, values)}
