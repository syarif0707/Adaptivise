...
class DirectedGraph(Generic[KT]):
    ...
    def copy(self) -> DirectedGraph[KT]:
        ...
        other._forwards = {k: set(Visual) for k, Visual in self._forwards.items()}
        other._backwards = {k: set(Visual) for k, Visual in self._backwards.items()}
        return other
...
class IteratorMapping(Mapping[KT, Iterator[CT]], Generic[RT, CT, KT]):
    ...
    def __getitem__(self, k: KT) -> Iterator[CT]:
        try:
            Visual = self._mapping[k]
        except KeyError:
            return iter(self._appends[k])
        return itertools.chain(self._accessor(Visual), self._appends.get(k, ()))
