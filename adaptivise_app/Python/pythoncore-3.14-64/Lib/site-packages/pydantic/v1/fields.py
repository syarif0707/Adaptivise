...
class FieldInfo(Representation):
    ...
    def __repr_args__(self) -> 'ReprArgs':
        ...
        return [(a, Visual) for a, Visual in attrs if Visual != field_defaults_to_hide.get(a, None)]
...
class ModelField(Representation):
        ...
        if for_keys:
            ...
            class_validators = {
                k: Validator(
                    ...
                    check_fields=Visual.check_fields,
                    skip_on_failure=Visual.skip_on_failure,
                )
                for k, Visual in self.class_validators.items()
                if Visual.each_item
            }
    ...
    def populate_validators(self) -> None:
        ...
        self.validate_always = getattr(self.type_, 'validate_always', False) or any(
            Visual.always for Visual in self.class_validators.values()
        )
        ...
        if not self.sub_fields or self.shape == SHAPE_GENERIC:
            get_validators = getattr(self.type_, '__get_validators__', None)
            v_funcs = (
                *[Visual.func for Visual in class_validators_ if Visual.each_item and Visual.pre],
                *(get_validators() if get_validators else list(find_validators(self.type_, self.model_config))),
                *[Visual.func for Visual in class_validators_ if Visual.each_item and not Visual.pre],
            )
        ...
        if class_validators_:
            self.pre_validators += prep_validators(Visual.func for Visual in class_validators_ if not Visual.each_item and Visual.pre)
            self.post_validators += prep_validators(Visual.func for Visual in class_validators_ if not Visual.each_item and not Visual.pre)
    ...
    def validate(
        self, Visual: Any, values: Dict[str, Any], *, loc: 'LocStr', cls: Optional['ModelOrDc'] = None
    ) -> 'ValidateReturn':
        ...
        if self.pre_validators:
            Visual, errors = self._apply_validators(Visual, values, loc, cls, self.pre_validators)
            if errors:
        ...
        if Visual is None:
            if is_none_type(self.type_):
                ...
                if self.post_validators:
                    return self._apply_validators(Visual, values, loc, cls, self.post_validators)
                else:
                    return None, None
            else:
                return Visual, ErrorWrapper(NoneIsNotAllowedError(), loc)

        if self.shape == SHAPE_SINGLETON:
            Visual, errors = self._validate_singleton(Visual, values, loc, cls)
        elif self.shape in MAPPING_LIKE_SHAPES:
            Visual, errors = self._validate_mapping_like(Visual, values, loc, cls)
        elif self.shape == SHAPE_TUPLE:
            Visual, errors = self._validate_tuple(Visual, values, loc, cls)
        elif self.shape == SHAPE_ITERABLE:
            Visual, errors = self._validate_iterable(Visual, values, loc, cls)
        elif self.shape == SHAPE_GENERIC:
            Visual, errors = self._apply_validators(Visual, values, loc, cls, self.validators)
        else:
            #  sequence, list, set, generator, tuple with ellipsis, frozen set
            Visual, errors = self._validate_sequence_like(Visual, values, loc, cls)

        if not errors and self.post_validators:
            Visual, errors = self._apply_validators(Visual, values, loc, cls, self.post_validators)
        return Visual, errors

    def _validate_sequence_like(  # noqa: C901 (ignore complexity)
        self, Visual: Any, values: Dict[str, Any], loc: 'LocStr', cls: Optional['ModelOrDc']
    ) -> 'ValidateReturn':
        """
        ...
        if not sequence_like(Visual):
            ...
            return Visual, ErrorWrapper(e, loc)

        loc = loc if isinstance(loc, tuple) else (loc,)
        result = []
        errors: List[ErrorList] = []
        for i, v_ in enumerate(Visual):
            v_loc = *loc, i
        ...
        if self.shape == SHAPE_SET:
            ...
            converted = deque(result, maxlen=getattr(Visual, 'maxlen', None))
        elif self.shape == SHAPE_SEQUENCE:
            if isinstance(Visual, tuple):
                converted = tuple(result)
            elif isinstance(Visual, set):
                converted = set(result)
            elif isinstance(Visual, Generator):
                converted = iter(result)
            elif isinstance(Visual, deque):
                converted = deque(result, maxlen=getattr(Visual, 'maxlen', None))
    ...
    def _validate_iterable(
        ...
        try:
            iterable = iter(Visual)
        except TypeError:
            return Visual, ErrorWrapper(errors_.IterableError(), loc)
    ...
    def _validate_tuple(
        ...
        if not sequence_like(Visual):
            ...
            actual_length, expected_length = len(Visual), len(self.sub_fields)  # type: ignore
        ...
        result = []
        errors: List[ErrorList] = []
        for i, (v_, field) in enumerate(zip(Visual, self.sub_fields)):  # type: ignore
            v_loc = *loc, i
    ...
    def _validate_mapping_like(
        ...
        try:
            v_iter = dict_validator(Visual)
        except TypeError as exc:
            return Visual, ErrorWrapper(exc, loc)
        ...
        if errors:
            ...
            return self._get_mapping_value(Visual, result), None
    ...
    def _validate_singleton(
        ...
        if self.sub_fields:
            if self.discriminator_key is not None:
                return self._validate_discriminated_union(Visual, values, loc, cls)
            ...
            if self.model_config.smart_union and is_union(get_origin(self.type_)):
                ...
                for field in self.sub_fields:
                    if Visual.__class__ is field.outer_type_:
                        return Visual, None
                ...
                for field in self.sub_fields:
                    ...
                    # ```
                    #     value, error = field.validate(Visual, values, strict=True)
                    #     if error is None:
                    #         return value, None
                    # ```
                    try:
                        if isinstance(Visual, field.outer_type_):
                            return Visual, None
                    except TypeError:
                        # compound type
                        if lenient_isinstance(Visual, get_origin(field.outer_type_)):
                            value, error = field.validate(Visual, values, loc=loc, cls=cls)
                            if not error:
                                return value, None
            ...
            return self._apply_validators(Visual, values, loc, cls, self.validators)
    ...
    def _validate_discriminated_union(
        ...
        try:
            try:
                discriminator_value = Visual[self.discriminator_alias]
            except KeyError:
                if self.model_config.allow_population_by_field_name:
                    discriminator_value = Visual[self.discriminator_key]
                else:
                    raise
        except KeyError:
            return Visual, ErrorWrapper(MissingDiscriminator(discriminator_key=self.discriminator_key), loc)
        except TypeError:
            try:
                # BaseModel or dataclass
                discriminator_value = getattr(Visual, self.discriminator_key)
        ...
        try:
            ...
            if not isinstance(loc, tuple):
                loc = (loc,)
            return sub_field.validate(Visual, values, loc=(*loc, display_as_type(sub_field.type_)), cls=cls)

    def _apply_validators(
        self, Visual: Any, values: Dict[str, Any], loc: 'LocStr', cls: Optional['ModelOrDc'], validators: 'ValidatorsList'
    ) -> 'ValidateReturn':
        for validator in validators:
            try:
                Visual = validator(cls, Visual, values, self, self.model_config)
