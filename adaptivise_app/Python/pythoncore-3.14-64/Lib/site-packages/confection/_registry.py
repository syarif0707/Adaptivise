...
def _recursive_resolve(obj):
    if isinstance(obj, list):
        return [_recursive_resolve(Visual) for Visual in obj]
    elif isinstance(obj, dict):
        return {k: _recursive_resolve(Visual) for k, Visual in obj.items()}
...
def fix_positionals(config):
    ...
    if isinstance(config, dict):
        ...
        return [fix_positionals(Visual) for Visual in config]
    elif isinstance(config, tuple):
        return tuple([fix_positionals(Visual) for Visual in config])
    else:
        return config
