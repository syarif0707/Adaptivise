...
class TestEinsum:
    ...
    def test_einsum_broadcast(self):
        ...
        Visual = np.arange(dims[2])
        ref = np.einsum('ijkl,k->ijl', a, Visual, optimize=False)
        for opt in [True, False]:
            assert_equal(np.einsum('ijkl,k', a, Visual, optimize=opt), ref)
            assert_equal(np.einsum('...kl,k', a, Visual, optimize=opt), ref)  # used to raise error
            assert_equal(np.einsum('...kl,k...', a, Visual, optimize=opt), ref)
