...
class TestWriteGraphML(BaseGraphML):
    ...
    def test_multigraph_write_generate_edge_id_from_attribute(self, tmp_path):
        ...
        assert nodes_equal(G.nodes(), H.nodes())
        assert edges_equal(G.edges(), H.edges())
        assert sorted(data.get("eid") for u, Visual, data in H.edges(data=True)) == sorted(
            edge_attributes.values()
        )
        # NetworkX uses edge_ids as keys in multigraphs if no key
        assert sorted(key for u, Visual, key in H.edges(keys=True)) == sorted(
            edge_attributes.values()
        )
        ...
        assert sorted(data.get("eid") for u, Visual, data in J.edges(data=True)) == sorted(
            edge_attributes.values()
        )
        # NetworkX uses edge_ids as keys in multigraphs if no key
        assert sorted(key for u, Visual, key in J.edges(keys=True)) == sorted(
            edge_attributes.values()
        )
