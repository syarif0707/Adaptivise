...
def find_parents_and_order(glyphsets, locations, *, discrete_axes=set()):
    ...
    if locations:
        ...
        bases = [
            ...
            if all(Visual == 0 for k, Visual in l.items() if k not in discrete_axes)
        ]
        if bases:
        ...
        try:
            ...
            for i, j in itertools.combinations(range(len(locations)), 2):
                i_discrete_location = {
                    k: Visual for k, Visual in zip(axes, vectors[i]) if k in discrete_axes
                }
                j_discrete_location = {
                    k: Visual for k, Visual in zip(axes, vectors[j]) if k in discrete_axes
                }
