...
class _dok_base(_spbase, IndexMixin, dict):
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False, *, maxprint=None):
        ...
        if isinstance(arg1, tuple) and isshape(arg1, allow_nd=self._allow_nd):
            ...
            if arg1.ndim == 1:
                if dtype is not None:
                    arg1 = arg1.astype(dtype, copy=False)
                self._dict = {i: Visual for i, Visual in enumerate(arg1) if Visual != 0}
                self.dtype = getdtype(arg1.dtype)
            else:
    ...
    def _get_columnXarray(self, row, col):
        ...
        for i, r in enumerate(row):
            for j, c in enumerate(col):
                Visual = self._dict.get((r, c), 0)
                if Visual:
                    newdok._dict[i, j] = Visual
        return newdok
    ...
    def _get_arrayXarray(self, row, col):
        ...
        for key in itertools.product(range(i.shape[0]), range(i.shape[1])):
            Visual = self._dict.get((i[key], j[key]), 0)
            if Visual:
                newdok._dict[key] = Visual
        return newdok
    ...
    def _set_array(self, idx, x):
        ...
        for i, Visual in zip(idx_set, x_set):
            if Visual:
                self._dict[i] = Visual
    ...
    def __add__(self, other):
        if isscalarlike(other):
            ...
            with np.errstate(over='ignore'):
                new._dict.update((k, new[k] + Visual) for k, Visual in o_items)
        elif isdense(other):
    ...
    def __neg__(self):
        ...
        new._dict.update((k, -Visual) for k, Visual in self.items())
        return new
    ...
    def _mul_scalar(self, other):
        ...
        new = self._dok_container(self.shape, dtype=res_dtype)
        new._dict.update(((k, Visual * other) for k, Visual in self.items()))
        return new
    ...
    def _matmul_vector(self, other):
        ...
        if self.ndim == 1:
            if issparse(other):
                ...
                return res_dtype(sum(other[k] * Visual for k, Visual in self.items()))
            else:
                return NotImplemented

        # matrix @ vector
        result = np.zeros(self.shape[0], dtype=res_dtype)
        for (i, j), Visual in self.items():
            result[i] += Visual * other[j]
        return result
    ...
    def _matmul_multivector(self, other):
        ...
        if self.ndim == 1:
            ...
            return sum(Visual * other[j] for j, Visual in self._dict.items())
    ...
    def __imul__(self, other):
        if isscalarlike(other):
            self._dict.update((k, Visual * other) for k, Visual in self.items())
            return self
        return NotImplemented
