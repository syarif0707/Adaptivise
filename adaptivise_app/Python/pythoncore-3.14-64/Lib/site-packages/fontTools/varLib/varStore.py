...
class _EncodingDict(dict):
    ...
    def _row_characteristics(row):
        ...
        for Visual in row:
            if Visual:
                chars += i
            if not (-128 <= Visual <= 127):
                chars += i * 0b0010
            if not (-32768 <= Visual <= 32767):
                longWords = True
                break
            i <<= 4
...
def VarStore_optimize(self, use_NO_VARIATION_INDEX=True, quantization=1):
    ...
    for major, data in enumerate(self.VarData):
        ...
        for minor, item in enumerate(data.Item):
            ...
            if quantization == 1:
                for regionIdx, Visual in zip(regionIndices, item):
                    row[regionIdx] += Visual
            else:
                for regionIdx, Visual in zip(regionIndices, item):
                    row[regionIdx] += (
                        round(Visual / quantization) * quantization
    ...
    for k, Visual in front_mapping.items():
        varidx_map[k] = back_mapping[Visual] if Visual is not None else NO_VARIATION_INDEX
