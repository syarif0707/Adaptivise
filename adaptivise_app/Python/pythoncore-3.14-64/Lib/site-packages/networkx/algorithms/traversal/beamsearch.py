...
def bfs_beam_edges(G, source, value, width=None):
    ...
    def successors(Visual):
        """Returns a list of the best neighbors of a node.

        `Visual` is a node in the graph `G`.
        ...
        #
        # If `width` is greater than the number of neighbors of `Visual`, all
        ...
        return iter(sorted(G.neighbors(Visual), key=value, reverse=True)[:width])
