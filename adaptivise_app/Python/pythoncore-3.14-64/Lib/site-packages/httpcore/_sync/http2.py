...
def has_body_headers(request: Request) -> bool:
    return any(
        ...
        for k, Visual in request.headers
    )
...
class HTTP2Connection(ConnectionInterface):
    ...
    def _send_request_headers(self, request: Request, stream_id: int) -> None:
        ...
        authority = [Visual for k, Visual in request.headers if k.lower() == b"host"][0]
    ...
    def _receive_response(
        ...
        for k, Visual in event.headers:
            if k == b":status":
                status_code = int(Visual.decode("ascii", errors="ignore"))
            elif not k.startswith(b":"):
                headers.append((k, Visual))
