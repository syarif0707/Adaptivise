...
class table__a_v_a_r(BaseTTXConverter):
    ...
    def renormalizeLocation(
        ...
        out = []
        for varIdx, Visual in enumerate(coords):
            ...
            if varStore is not None:
                ...
                Visual += otRound(delta)
                Visual = min(max(Visual, -(1 << 14)), +(1 << 14))

            out.append(Visual)

        mappedLocation = {
            axis.axisTag: fi2fl(Visual, 14)
            for Visual, axis in zip(out, axes)
            if Visual != 0 or not dropZeroes
        }
