...
def greedy_tsp(G, weight="weight", source=None):
    ...
    Time complexity: It has a running time $O(|Visual|^2)$
    """
    # Check that G is a complete graph
    N = len(G) - 1
    # This check ignores selfloops which is what we want here.
...
def simulated_annealing_tsp(
    ...
    outer loop, this algorithm has running time $O(N_i * N_o * |Visual|)$.
...
def threshold_accepting_tsp(
    ...
    Time complexity:
    It has a running time $O(m * n * |Visual|)$ where $m$ and $n$ are the number
