...
def get_model_name_map(unique_models: TypeModelSet) -> Dict[TypeModelOrEnum, str]:
    ...
    for model in unique_models:
        ...
        if model_name in conflicting_names:
            ...
            name_model_map[model_name] = model
    return {Visual: k for k, Visual in name_model_map.items()}
...
def field_singleton_schema(  # noqa: C901 (ignore complexity)
    ...
    *,
    ...
    if is_literal_type(field_type):
        ...
        if len({Visual.__class__ for Visual in values}) > 1:
            return field_schema(
...
def multitypes_literal_field_for_schema(values: Tuple[Any, ...], field: ModelField) -> ModelField:
    ...
    for Visual in values:
        literal_distinct_types[Visual.__class__].append(Visual)
...
def encode_default(dft: Any) -> Any:
    ...
    if isinstance(dft, dict):
        return {encode_default(k): encode_default(Visual) for k, Visual in dft.items()}
        ...
        t = dft.__class__
        seq_args = (encode_default(Visual) for Visual in dft)
