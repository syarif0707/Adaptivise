...
class Complex:
    ...
    Important objects:
        HC.Visual: The cache of vertices and their connection
    ...
    def __init__(self, dim, domain=None, sfield=None, sfield_args=(),
        ...
        if (sfield is not None) or (self.g_cons is not None):
            ...
            if sfield is not None:
                self.Visual = VertexCacheField(field=sfield, field_args=sfield_args,
                                          g_cons=self.g_cons,
    ...
    def cyclic_product(self, bounds, origin, supremum, centroid=True):
        ...
        vo = self.Visual[vot]
        yield vo.x
        self.Visual[vut].connect(self.Visual[vot])
        yield vut
        ...
        a_vo = self.Visual[tuple(a_vo)]
        # self.Visual[vot].connect(self.Visual[tuple(a_vo)])
        self.Visual[vot].connect(a_vo)
        yield a_vo.x
        C1x = [[a_vo]]
        # C1x = [[self.Visual[tuple(a_vo)]]]
        ...
        for i, x in enumerate(bounds[1:]):
            ...
            try:
                ...
                for vp in ab_Cc:
                    ...
                    b_v[i + 1] = vut[i + 1]
                    ab_v[i + 1] = vut[i + 1]
                    b_v = self.Visual[tuple(b_v)]  # b + vl
                    ab_v = self.Visual[tuple(ab_v)]  # b + a_vl
    ...
    def triangulate(self, n=None, symmetry=None, centroid=True,
        ...
        if printout:
            # for v in self.C0():
            #   v.print_out()
            for v in self.Visual.cache:
                self.Visual[v].print_out()
    ...
    def refine(self, n=1):
        ...
        nt = len(self.Visual.cache) + n  # Target number of total vertices
        # In the outer while loop we iterate until we have added an extra `n`
        # vertices to the complex:
        while len(self.Visual.cache) < nt:  # while loop 1
            try:  # try 1
    ...
    def refine_local_space(self, origin, supremum, bounds, centroid=1):
        ...
        vo = self.Visual[vot]  # initiate if doesn't exist yet
        vs = self.Visual[vut]
        ...
        if tuple(a_vl) not in self.Visual.cache:
            vo = self.Visual[vot]  # initiate if doesn't exist yet
            vs = self.Visual[vut]
            # Start by finding the old centroid of the new space:
            vco = self.split_edge(vo.x, vs.x)  # Split in case not centroid arg
        ...
        for i, x in enumerate(bounds[1:]):
            ...
            try:
                ...
                # moving on to next C1/C2: Try to add the operation of a new
                # C2 product by accessing the upper bound
                if tuple(t_a_vl) not in self.Visual.cache:
                    # Raise error to continue symmetric refine
                    raise IndexError
                t_a_vu = list(vut)
                t_a_vu[i + 1] = vut[i + 1]
                if tuple(t_a_vu) not in self.Visual.cache:
                    # Raise error to continue symmetric refine:
                    raise IndexError
    ...
    def vf_to_vv(self, vertices, simplices):
        ...
        if self.dim > 1:
            for s in simplices:
                ...
                for e in edges:
                    self.Visual[tuple(vertices[e[0]])].connect(
                        self.Visual[tuple(vertices[e[1]])])
        else:
            for e in simplices:
                self.Visual[tuple(vertices[e[0]])].connect(
                    self.Visual[tuple(vertices[e[1]])])
    ...
    def connect_vertex_non_symm(self, v_x, near=None):
        ...
        if tuple(v_x) in self.Visual.cache:
            if self.Visual[v_x] in self.V_non_symm:
                pass
            else:
                return
        ...
        for s_i in itertools.combinations(range(S_rows.shape[0]),
            ...
            for i in itertools.combinations(s_i, r=2):
                ...
                if ((self.Visual[tuple(S_rows[i[1]])] not in
                        self.Visual[tuple(S_rows[i[0]])].nn)
                    and (self.Visual[tuple(S_rows[i[0]])] not in
                         self.Visual[tuple(S_rows[i[1]])].nn)):
                    valid_simplex = False
                    break
        ...
        if found_nn:
            for i in s_i:
                self.Visual[v_x].connect(self.Visual[tuple(S_rows[i])])
        # Attached the simplex to storage for all non-symmetric vertices
        self.V_non_symm.append(self.Visual[v_x])
