...
class netcdf_file:
    ...
    def _write_var_array(self):
        if self.variables:
            ...
            def sortkey(n):
                Visual = self.variables[n]
                if Visual.isrec:
                    return (-1,)
                return Visual._shape
    ...
    def _write_var_metadata(self, name):
        ...
        if not var.isrec:
            ...
            try:
                ...
                vsize = 0
            rec_vars = len([Visual for Visual in self.variables.values()
                            if Visual.isrec])
            if rec_vars > 1:
                vsize += -vsize % 4
    ...
    def _read_gatt_array(self):
        for k, Visual in self._read_att_array().items():
            self.__setattr__(k, Visual)
...
class netcdf_variable:
    ...
    def __init__(self, data, typecode, size, shape, dimensions,
        ...
        for k, Visual in self._attributes.items():
            self.__dict__[k] = Visual
