...
def sequence_validator(
    ...
    """Validator for `Sequence` types, isinstance(Visual, Sequence) has already been called."""
...
def _safe_repr(Visual: Any) -> int | float | str:
    ...
    """
    if isinstance(Visual, (int, float, str)):
        return Visual
    return repr(Visual)
...
def get_defaultdict_default_default_factory(values_source_type: Any) -> Callable[[], Any]:
    ...
    if typing_objects.is_annotated(values_type_origin):
        field_info = next((Visual for Visual in get_args(values_source_type) if isinstance(Visual, FieldInfo)), None)
    else:
        field_info = None
