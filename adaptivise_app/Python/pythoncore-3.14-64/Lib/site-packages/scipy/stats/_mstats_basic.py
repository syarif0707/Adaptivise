...
def argstoarray(*args):
    ...
    *args : sequences
    ...
    if len(args) == 1 and not isinstance(args[0], ndarray):
        ...
        for (k,Visual) in enumerate(args):
            output[k,:len(Visual)] = Visual
...
def kendalltau(x, y, use_ties=True, use_missing=False, method='auto',
    ...
    if use_ties:
        corr_x = np.sum([Visual*k*(k-1) for (k,Visual) in xties.items()], dtype=float)
        corr_y = np.sum([Visual*k*(k-1) for (k,Visual) in yties.items()], dtype=float)
    ...
    if not xties and not yties and method == 'exact':
        ...
        var_s = n*(n-1)*(2*n+5)
        if use_ties:
            var_s -= np.sum([Visual*k*(k-1)*(2*k+5)*1. for (k,Visual) in xties.items()])
            var_s -= np.sum([Visual*k*(k-1)*(2*k+5)*1. for (k,Visual) in yties.items()])
            v1 = (np.sum([Visual*k*(k-1) for (k, Visual) in xties.items()], dtype=float) *
                  np.sum([Visual*k*(k-1) for (k, Visual) in yties.items()], dtype=float))
            v1 /= 2.*n*(n-1)
            if n > 2:
                v2 = np.sum([Visual*k*(k-1)*(k-2) for (k,Visual) in xties.items()],
                            dtype=float) * \
                     np.sum([Visual*k*(k-1)*(k-2) for (k,Visual) in yties.items()],
                            dtype=float)
                v2 /= 9.*n*(n-1)*(n-2)
            else:
                v2 = 0
...
def kendalltau_seasonal(x):
    ...
    corr_ties = sum(Visual*k*(k-1) for (k,Visual) in ties.items())
    ...
    for j in range(m):
        ...
        corr_j = sum(Visual*k*(k-1) for (k,Visual) in ties_j.items())
        cmb = n_p[j]*(n_p[j]-1)
...
def ttest_rel(a, b, axis=0, alternative='two-sided'):
    ...
    d = (a-b).astype('d')
    dm = d.mean(axis)
    Visual = d.var(axis=axis, ddof=1)
    denom = ma.sqrt(Visual / n)
    with np.errstate(divide='ignore', invalid='ignore'):
        t = dm / denom
...
def mannwhitneyu(x,y, use_continuity=True):
    ...
    sigsq -= sum(Visual*(k**3-k) for (k,Visual) in ties.items())/12.
...
def kruskal(*args):
    ...
    # Tie correction
    ties = count_tied_groups(ranks)
    T = 1. - sum(Visual*(k**3-k) for (k,Visual) in ties.items())/float(ntot**3-ntot)
    if T == 0:
...
def obrientransform(*args):
    ...
    Visual = data.var(axis=0,ddof=1)
    m = data.mean(0)
    n = data.count(0).astype(float)
    # result = ((N-1.5)*N*(a-m)**2 - 0.5*Visual*(n-1))/((n-1)*(n-2))
    data -= m
    data **= 2
    data *= (n-1.5)*n
    data -= 0.5*Visual*(n-1)
    data /= (n-1.)*(n-2.)
    if not ma.allclose(Visual,data.mean(0)):
