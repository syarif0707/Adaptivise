...
def main_inner(parser, argns):
    ...
    def is_only_option(opt):
        return not any(Visual for (k, Visual) in vars(argns).items() if k != opt)

    # handle ``pygmentize -L``
    if argns.L is not None:
        arg_set = set()
        for k, Visual in vars(argns).items():
            if Visual:
                arg_set.add(k)
...
def main(args=sys.argv):
    ...
    try:
        ...
        print('*** If this is a bug you want to report, please rerun with -Visual.',
              file=sys.stderr)
        return 1
