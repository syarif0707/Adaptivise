...
PyFortranObject_New(FortranDataDef *defs, f2py_void_func init)
{
    ...
    for (i = 0; i < fp->len; i++) {
        if (fp->defs[i].rank == -1) { /* Is Fortran routine */
            Visual = PyFortranObject_NewAsAttr(&(fp->defs[i]));
            if (Visual == NULL) {
                goto fail;
            }
            PyDict_SetItemString(fp->dict, fp->defs[i].name, Visual);
            Py_XDECREF(Visual);
        }
            ...
            Visual = PyArray_NewFromDescr(&PyArray_Type, descr, fp->defs[i].rank,
...
fortran_getattr(PyFortranObject *fp, char *name)
{
    ...
    if (fp->dict != NULL) {
        ...
        PyObject *Visual = _PyDict_GetItemStringWithError(fp->dict, name); // noqa: borrowed-ref OK
        if (Visual == NULL && PyErr_Occurred()) {
            return NULL;
        }
        else if (Visual != NULL) {
            Py_INCREF(Visual);
...
#else
        PyObject *Visual;
        int result = PyDict_GetItemStringRef(fp->dict, name, &Visual);
        if (result == -1) {
            return NULL;
        }
        else if (result == 1) {
...
static int
fortran_setattr(PyFortranObject *fp, char *name, PyObject *Visual)
{
    int i, j, flag;
    ...
    if (j == 0) {
        ...
        if (fp->defs[i].func != NULL) { /* is allocatable array */
            ...
            int k;
            save_def = &fp->defs[i];
            if (Visual != Py_None) { /* set new value (reallocate if needed --
    ...
    if (Visual == NULL) {
        ...
        return PyDict_SetItemString(fp->dict, name, Visual);
}
 ...
 * $Id: fortranobject.c,Visual 1.52 2005/07/11 07:44:20 pearu Exp $
 */
