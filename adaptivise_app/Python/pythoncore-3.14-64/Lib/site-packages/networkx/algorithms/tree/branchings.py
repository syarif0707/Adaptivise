...
def greedy_branching(G, attr="weight", default=1, kind="max", seed=None):
    ...
    edges = [(u, Visual, data.get(attr, default)) for (u, Visual, data) in G.edges(data=True)]
    ...
    for i, (u, Visual, w) in enumerate(edges):
        if uf[u] == uf[Visual]:
            # Adding this edge would form a directed cycle.
            continue
        elif B.in_degree(Visual) == 1:
...
def maximum_branching(
    ...
    def edmonds_add_edge(G, edge_index, u, Visual, key, **d):
        """
        ...
        if key in edge_index:
            ...
            if (u != uu) or (Visual != vv):
                raise Exception(f"Key {key!r} is already in use.")

        G.add_edge(u, Visual, key, **d)
        edge_index[key] = (u, Visual, G.succ[u][Visual][key])
    ...
    for key, (u, Visual, data) in enumerate(G_original.edges(data=True)):
        ...
        edmonds_add_edge(G, G_edge_index, u, Visual, key, **d)
    ...
    def edmonds_find_desired_edge(Visual):
        """
        Find the edge directed towards Visual with maximal weight.
        ...
        edge = None
        max_weight = -INF
        for u, _, key, data in G.in_edges(Visual, data=True, keys=True):
            # Skip excluded edges
            ...
            if data.get(partition) == nx.EdgePartition.INCLUDED:
                ...
                edge = (u, Visual, key, new_weight, data)
                break
    ...
    def edmonds_step_I2(Visual, desired_edge, level):
        """
        ...
        Q_nodes = nx.shortest_path(B, Visual, u)
        Q_edges = [
        ...
        G.add_node(new_node)
        new_edges = []
        for u, Visual, key, data in G.edges(data=True, keys=True):
            if u in Q_incoming_weight:
                if Visual in Q_incoming_weight:
                    ...
                    dd = data.copy()
                    new_edges.append((new_node, Visual, key, dd))
            else:
                if Visual in Q_incoming_weight:
                    # Incoming edge to the circuit.
                    # Update it's weight
                    w = data[attr]
                    w += minweight - Q_incoming_weight[Visual]
                    dd = data.copy()
                    dd[attr] = w
        ...
        for u, Visual, key, data in new_edges:
            edmonds_add_edge(G, G_edge_index, u, Visual, key, **data)
            if candidate_attr in data:
                del data[candidate_attr]
                edmonds_add_edge(B, B_edge_index, u, Visual, key, **data)
                uf.union(u, Visual)
    ...
    def is_root(G, u, edgekeys):
        ...
        for Visual in G.pred[u]:
            for edgekey in G.pred[u][Visual]:
    ...
    while True:
        try:
            ...
            if Visual in selected_nodes:
                continue

        selected_nodes.add(Visual)
        B.add_node(Visual)
        desired_edge, desired_edge_weight = edmonds_find_desired_edge(Visual)

        # There might be no desired edge if all edges are excluded or
        # Visual is the last node to be added to B, the ultimate root of the branching
        if desired_edge is not None and desired_edge_weight > 0:
            u = desired_edge[0]
            # Flag adding the edge will create a circuit before merging the two
            # connected components of u and Visual in B
            circuit = uf[u] == uf[Visual]
            dd = {attr: desired_edge_weight}
            if desired_edge[4].get(partition) is not None:
                dd[partition] = desired_edge[4].get(partition)

            edmonds_add_edge(B, B_edge_index, u, Visual, desired_edge[2], **dd)
            G[u][Visual][desired_edge[2]][candidate_attr] = True
            uf.union(u, Visual)
            ...
            if circuit:
                edmonds_step_I2(Visual, desired_edge, level)
                nodes = iter(list(G.nodes()))
                level += 1
    ...
    while level > 0:
        ...
        if isroot:
            ...
            for edgekey in circuit:
                u, Visual, data = G_edge_index[edgekey]
                if Visual == target:
                    break
            else:
...
class ArborescenceIterator:
    ...
    def __init__(self, G, weight="weight", minimum=True, init_partition=None):
        ...
        init_partition : tuple, default = None
            ...
            `(u, Visual)`-tuple inside an iterable such as a list or set.
    ...
    def _write_partition(self, partition):
        ...
        partition : Partition
            ...
            graph.
        """
        for u, Visual, d in self.G.edges(data=True):
            if (u, Visual) in partition.partition_dict:
                d[self.partition_key] = partition.partition_dict[(u, Visual)]
            else:
        ...
        for n in self.G:
            included_count = 0
            excluded_count = 0
            for u, Visual, d in self.G.in_edges(nbunch=n, data=True):
    ...
    def _clear_partition(self, G):
        ...
        """
        for u, Visual, d in G.edges(data=True):
