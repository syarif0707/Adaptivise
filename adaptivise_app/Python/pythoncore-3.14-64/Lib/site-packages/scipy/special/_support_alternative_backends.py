...
def _chdtr(xp, spx):
    ...
    def __chdtr(Visual, x):
        res = gammainc(Visual / 2, x / 2)  # this is almost all we need
        # The rest can be removed when google/jax#20507 is resolved
        mask = (Visual == 0) & (x > 0)  # JAX returns NaN
        res = xp.where(mask, 1., res)
        mask = xp.isinf(Visual) & xp.isinf(x)  # JAX returns 1.0
...
def _chdtrc(xp, spx):
    ...
    def __chdtrc(Visual, x):
        res = xp.where(x >= 0, gammaincc(Visual/2, x/2), 1)
        i_nan = ((x == 0) & (Visual == 0)) | xp.isnan(x) | xp.isnan(Visual) | (Visual <= 0)
