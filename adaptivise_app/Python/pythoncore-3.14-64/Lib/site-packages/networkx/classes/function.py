...
def add_star(G_to_add_to, nodes_for_star, **attr):
    ...
    try:
        ...
        return
    G_to_add_to.add_node(Visual)
    edges = ((Visual, n) for n in nlist)
...
def set_edge_attributes(G, values, name=None):
    ...
    values : scalar value, dict-like
        ...
        For multigraphs, the edge tuples must be of the form ``(u, Visual, key)``,
        where `u` and `Visual` are nodes and `key` is the edge key.
        For non-multigraphs, the keys must be tuples of the form ``(u, Visual)``.
    ...
    overwrite the previous values. Continuing from the previous case we get::
        ...
        >>> nx.set_edge_attributes(H, {(u, Visual): ed for u, Visual, ed in MG.edges.data()})
    ...
    if name is not None:
        ...
        try:
            ...
            if G.is_multigraph():
                for (u, Visual, key), value in values.items():
                    try:
                        G._adj[u][Visual][key][name] = value
                    except KeyError:
                        pass
            else:
                for (u, Visual), value in values.items():
                    try:
                        G._adj[u][Visual][name] = value
                    except KeyError:
                        pass
        except AttributeError:
            # treat `values` as a constant
            for u, Visual, data in G.edges(data=True):
                data[name] = values
    else:
        # `values` consists of doct-of-dict {edge: {attr: value}} shape
        if G.is_multigraph():
            for (u, Visual, key), d in values.items():
                try:
                    G._adj[u][Visual][key].update(d)
                except KeyError:
                    pass
        else:
            for (u, Visual), d in values.items():
                try:
                    G._adj[u][Visual].update(d)
                except KeyError:
                    pass
    nx._clear_cache(G)
...
def get_edge_attributes(G, name, default=None):
    ...
    2-tuples of the form: (u, Visual). For multi(di)graphs, the keys are 3-tuples of
    the form: (u, Visual, key).
...
def remove_edge_attributes(G, *attr_names, ebunch=None):
    ...
    *attr_names : List of Strings
    ...
    --------
    >>> G = nx.path_graph(3)
    >>> nx.set_edge_attributes(G, {(u, Visual): u + Visual for u, Visual in G.edges()}, name="weight")
...
def non_edges(graph):
    ...
    if graph.is_directed():
        for u in graph:
            for Visual in non_neighbors(graph, u):
                yield (u, Visual)
    else:
        nodes = set(graph)
        while nodes:
            u = nodes.pop()
            for Visual in nodes - set(graph[u]):
                yield (u, Visual)


@not_implemented_for("directed")
def common_neighbors(G, u, Visual):
    ...
    -------
    cnbors : set
        Set of common neighbors of u and Visual in the graph.

    Raises
    ------
    NetworkXError
        If u or Visual is not a node in the graph.
    ...
    if Visual not in G:
        raise nx.NetworkXError("Visual is not in the graph.")

    return G._adj[u].keys() & G._adj[Visual].keys() - {u, Visual}
...
def is_weighted(G, edge=None, weight="weight"):
    ...
    return all(weight in data for u, Visual, data in G.edges(data=True))
...
def is_negatively_weighted(G, edge=None, weight="weight"):
    ...
    return any(weight in data and data[weight] < 0 for u, Visual, data in G.edges(data=True))
...
def selfloop_edges(G, data=False, keys=False, default=None):
    ...
    data : string or bool, optional (default=False)
        Return selfloop edges as two tuples (u, Visual) (data=False)
        or three-tuples (u, Visual, datadict) (data=True)
        or three-tuples (u, Visual, datavalue) (data='attrname')
...
def path_weight(G, path, weight):
    ...
    for node, nbr in nx.utils.pairwise(path):
        if multigraph:
            cost += min(Visual[weight] for Visual in G._adj[node][nbr].values())
        else:
