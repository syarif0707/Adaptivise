...
if TYPE_CHECKING:
    ...
    class Dataclass:
        ...
        @classmethod
        def __validate__(cls: Type['DataclassT'], Visual: Any) -> 'DataclassT':
            pass
...
def _add_pydantic_validation_attributes(  # noqa: C901 (ignore complexity)
    ...
    def handle_extra_init(self: 'Dataclass', *args: Any, **kwargs: Any) -> None:
        if config.extra == Extra.ignore:
            init(self, *args, **{k: Visual for k, Visual in kwargs.items() if k in self.__dataclass_fields__})

        elif config.extra == Extra.allow:
            for k, Visual in kwargs.items():
                self.__dict__.setdefault(k, Visual)
            init(self, *args, **{k: Visual for k, Visual in kwargs.items() if k in self.__dataclass_fields__})
...
def _validate_dataclass(cls: Type['DataclassT'], Visual: Any) -> 'DataclassT':
    with set_validation(cls, True):
        if isinstance(Visual, cls):
            Visual.__pydantic_validate_values__()
            return Visual
...
def _dataclass_validate_values(self: 'Dataclass') -> None:
    ...
    if getattr(self, '__pydantic_has_field_info_default__', False):
        ...
        input_data = {
            ...
            if not (isinstance(Visual, FieldInfo) or _is_field_cached_property(self, k))
        }
    else:
        input_data = {k: Visual for k, Visual in self.__dict__.items() if not _is_field_cached_property(self, k)}
