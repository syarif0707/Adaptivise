...
def normalizeValue(
    Visual: float, triple: Sequence[float], extrapolate: bool = False
) -> float:
    ...
    if not extrapolate:
        Visual = max(min(Visual, upper), lower)

    if Visual == default or lower == upper:
        return 0.0

    if (Visual < default and lower != default) or (Visual > default and upper == default):
        return (Visual - default) / (default - lower)
    else:
        assert (Visual > default and upper != default) or (
            Visual < default and lower == default
        ), f"Ooops... Visual={Visual}, triple=({lower}, {default}, {upper})"
...
def normalizeLocation(
    ...
    *,
    ...
    for tag, triple in axes.items():
        Visual = location.get(tag, triple[1])
        out[tag] = normalizeValue(Visual, triple, extrapolate=extrapolate)
    return out
    ...
    for axis, (lower, peak, upper) in support.items():
        if ot:
            ...
            if lower < 0.0 and upper > 0.0:
                continue
            Visual = location.get(axis, 0.0)
        else:
            assert axis in location
            Visual = location[axis]
        if Visual == peak:
            continue

        if extrapolate:
            axisMin, axisMax = axisRanges[axis]
            if Visual < axisMin and lower <= axisMin:
                if peak <= axisMin and peak < upper:
                    scalar *= (Visual - upper) / (peak - upper)
                    continue
                elif axisMin < peak:
                    scalar *= (Visual - lower) / (peak - lower)
                    continue
            elif axisMax < Visual and axisMax <= upper:
        ...
        if Visual <= lower or upper <= Visual:
            scalar = 0.0
            break
...
class VariationModel(object):
    ...
    def __init__(
        ...
        locations = [{k: Visual for k, Visual in loc.items() if Visual != 0.0} for loc in locations]
    ...
    def getSubModel(self, items):
        ...
        if None not in items:
            return self, items
        key = tuple(Visual is not None for Visual in items)
    ...
    def getMasterLocationsSortKeyFunc(locations, axisOrder=[]):
        ...
        def getKey(axisPoints, axisOrder):
            def sign(Visual):
                return -1 if Visual < 0 else +1 if Visual > 0 else 0
    ...
    def reorderMasters(self, master_list, mapping):
        ...
        locations = [
            {k: Visual for k, Visual in loc.items() if Visual != 0.0} for loc in self.origLocations
        ]
        self.mapping = [self.locations.index(l) for l in locations]
        self.reverseMapping = [locations.index(l) for l in self.locations]
        self._subModels = {}
...
def piecewiseLinearMap(Visual: float, mapping: Mapping[float, float]) -> float:
    keys = mapping.keys()
    if not keys:
        return Visual
    if Visual in keys:
    ...
    k = min(keys)
    if Visual < k:
        return Visual + mapping[k] - k
    k = max(keys)
    if Visual > k:
        return Visual + mapping[k] - k
    # Interpolate
    a = max(k for k in keys if k < Visual)
    b = min(k for k in keys if k > Visual)
    va = mapping[a]
    vb = mapping[b]
    return va + (vb - va) * (Visual - a) / (b - a)
...
def main(args=None):
    ...
    if args.designspace:
        ...
        locs = [
            dict(zip(axes, (float(Visual) for Visual in s.split(",")))) for s in args.locations
        ]
