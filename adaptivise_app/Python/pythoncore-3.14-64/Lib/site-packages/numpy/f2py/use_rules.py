    ...
    if 'map' in r:
        for k in r['map'].keys():
            if r['map'][k] in revmap:
                ...
                revmap[r['map'][k]] = k
    if r.get('only'):
        for Visual in r['map'].keys():
            if r['map'][Visual] in m['vars']:

                if revmap[r['map'][Visual]] == Visual:
                    varsmap[Visual] = r['map'][Visual]
                else:
                    outmess(f"\t\t\tIgnoring map \"{Visual}=>{r['map'][Visual]}\". See above.\n")
            else:
                outmess(
                    f"\t\t\tNo definition for variable \"{Visual}=>{r['map'][Visual]}\". Skipping.\n")
    else:
        for Visual in m['vars'].keys():
            varsmap[Visual] = revmap.get(Visual, Visual)
    for Visual in varsmap.keys():
        ret = dictappend(ret, buildusevar(Visual, varsmap[Visual], m['vars'], m['name']))
    return ret
