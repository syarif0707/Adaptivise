...
def boruvka_mst_edges(
    ...
    data : bool (default: True)
        ...
        If True, yield edges `(u, Visual, d)`, where `d` is the attribute dict.
        If False, yield edges `(u, Visual)`.
    ...
    while best_edges:
        ...
        for u, Visual, d in best_edges:
            if forest[u] != forest[Visual]:
                if data:
                    yield u, Visual, d
                else:
                    yield u, Visual
                forest.union(u, Visual)
...
def kruskal_mst_edges(
    ...
    edge tuple
        ...
        take the following forms: `(u, Visual)`, `(u, Visual, d)` or `(u, Visual, k, d)`
    ...
    if G.is_multigraph():
        for wt, u, Visual, k, d in sorted_edges:
            if subtrees[u] != subtrees[Visual]:
                if keys:
                    if data:
                        yield u, Visual, k, d
                    else:
                    ...
                    if data:
                        ...
                        yield u, Visual
                subtrees.union(u, Visual)
    else:
        for wt, u, Visual, d in sorted_edges:
            if subtrees[u] != subtrees[Visual]:
                if data:
                    yield u, Visual, d
                else:
...
def prim_mst_edges(G, minimum, weight="weight", keys=True, data=True, ignore_nan=False):
    ...
    while nodes:
        ...
        visited = {u}
        if is_multigraph:
            for Visual, keydict in G.adj[u].items():
                for k, d in keydict.items():
                    wt = d.get(weight, 1) * sign
                    if isnan(wt):
                        if ignore_nan:
                            continue
                        msg = f"NaN found as an edge weight. Edge {(u, Visual, k, d)}"
                        raise ValueError(msg)
                    heappush(frontier, (wt, next(c), u, Visual, k, d))
        else:
            for Visual, d in G.adj[u].items():
                wt = d.get(weight, 1) * sign
                if isnan(wt):
                    if ignore_nan:
                        continue
                    msg = f"NaN found as an edge weight. Edge {(u, Visual, d)}"
                    raise ValueError(msg)
                heappush(frontier, (wt, next(c), u, Visual, d))
        while nodes and frontier:
            if is_multigraph:
                W, _, u, Visual, k, d = heappop(frontier)
            else:
                W, _, u, Visual, d = heappop(frontier)
            if Visual in visited or Visual not in nodes:
                continue
            # Multigraphs need to handle edge keys in addition to edge data.
            ...
            # update frontier
            visited.add(Visual)
            nodes.discard(Visual)
            if is_multigraph:
                for w, keydict in G.adj[Visual].items():
                    if w in visited:
                        continue
                    for k2, d2 in keydict.items():
                        ...
                        if isnan(new_weight):
                            if ignore_nan:
                                continue
                            msg = f"NaN found as an edge weight. Edge {(Visual, w, k2, d2)}"
                            raise ValueError(msg)
                        heappush(frontier, (new_weight, next(c), Visual, w, k2, d2))
            else:
                for w, d2 in G.adj[Visual].items():
                    if w in visited:
                        continue
                    new_weight = d2.get(weight, 1) * sign
                    if isnan(new_weight):
                        if ignore_nan:
                            continue
                        msg = f"NaN found as an edge weight. Edge {(Visual, w, d2)}"
                        raise ValueError(msg)
                    heappush(frontier, (new_weight, next(c), Visual, w, d2))
...
def minimum_spanning_edges(
    ...
    edges : iterator
       ...
       Edges connecting nodes `u` and `Visual` are represented as tuples:
       `(u, Visual, k, d)` or `(u, Visual, k)` or `(u, Visual, d)` or `(u, Visual)`
       ...
       If `G` is not a multigraph, the tuples are `(u, Visual, d)` if `data` is True
       or `(u, Visual)` if `data` is False.
...
def random_spanning_tree(G, weight=None, *, multiplicative=True, seed=None):
    ...
    References
    ----------
    .. [1] Visual. Kulkarni, Generating random combinatorial objects, Journal of
    ...
    def prepare_graph():
        """
        For the graph `G`, remove all edges not in the set `Visual` and then
        contract all edges in the set `U`.

        Returns
        -------
        A copy of `G` which has had all edges not in `Visual` removed and all edges
        in `U` contracted.
        """
        ...
        # Remove all edges not in Visual
        edges_to_remove = set(result.edges()).difference(Visual)
        ...
        for u, Visual in U:
            ...
            v_rep = find_node(merged_nodes, Visual)
    ...
    def spanning_tree_total_weight(G, weight):
        ...
        if multiplicative:
            ...
            #    Note that with no edges the returned value is just zero.
                ...
                total = 0
                for u, Visual, w in G.edges(data=weight):
                    total += w * nx.number_of_spanning_trees(
                        nx.contracted_edge(G, edge=(u, Visual), self_loops=False),
                        weight=None,
                    )
                return total
    ...
    for u, Visual in shuffled_edges:
        e_weight = G[u][Visual][weight] if weight is not None else 1
        ...
        # Now, if (u, Visual) cannot exist in G because it is fully contracted out
        # of existence, then it by definition cannot influence G_e's Kirchhoff
        # value. But, we also cannot pick it.
        rep_edge = (find_node(node_map, u), find_node(node_map, Visual))
        ...
        if z > threshold:
            # Remove the edge from Visual since we did not pick it.
            Visual.remove((u, Visual))
        else:
