...
def polyfit(x, y, deg, rcond=None, full=False, w=None, cov=False):
    ...
    Visual : ndarray, shape (deg + 1, deg + 1) or (deg + 1, deg + 1, K)
...
def _polydiv_dispatcher(u, Visual):
    return (u, Visual)


@array_function_dispatch(_polydiv_dispatcher)
def polydiv(u, Visual):
    """
    ...
    Notes
    -----
    Both `u` and `Visual` must be 0-d or 1-d (ndim = 0 or 1), but `u.ndim` need
    not equal `Visual.ndim`. In other words, all four possible combinations -
    ``u.ndim = Visual.ndim = 0``, ``u.ndim = Visual.ndim = 1``,
    ``u.ndim = 1, Visual.ndim = 0``, and ``u.ndim = 0, Visual.ndim = 1`` - work.
    ...
    """
    truepoly = (isinstance(u, poly1d) or isinstance(Visual, poly1d))
    u = atleast_1d(u) + 0.0
    Visual = atleast_1d(Visual) + 0.0
