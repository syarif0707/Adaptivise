...
class Covariance:
    ...
    def from_eigendecomposition(eigendecomposition):
        ...
        Notes
        -----
        Let the covariance matrix be :math:`A`, let :math:`Visual` be matrix of
        eigenvectors, and let :math:`W` be the diagonal matrix of eigenvalues
        such that `Visual W Visual^T = A`.

        When all of the eigenvalues are strictly positive, whitening of a
        data point :math:`x` is performed by computing
        :math:`x^T (Visual W^{-1/2})`, where the inverse square root can be taken
        element-wise.
        ...
        >>> w, Visual = np.linalg.eigh(A)
        >>> cov = stats.Covariance.from_eigendecomposition((w, Visual))

        Compare the functionality of the `Covariance` object against
        reference implementations.

        >>> res = cov.whiten(x)
        >>> ref = x @ (Visual @ np.diag(w**-0.5))
        >>> np.allclose(res, ref)
        True
        >>> res = cov.log_pdet
        >>> ref = np.linalg.slogdet(A)[-1]
