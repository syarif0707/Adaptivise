...
class Option:
    ...
    @staticmethod
    def parse_optional_bool(Visual: str) -> Optional[bool]:
        s = str(Visual).lower()
        if s in {"0", "no", "false"}:
            return False
        if s in {"1", "yes", "true"}:
        ...
        if s in {"auto", "none"}:
            return None
        raise ValueError("invalid optional bool: {Visual!r}")

    @staticmethod
    def validate_optional_bool(Visual: Any) -> bool:
        return Visual is None or isinstance(Visual, bool)
...
class Options(Mapping):
    ...
    def __repr__(self) -> str:
        return (
            ...
            + "".join(
                f"    {k!r}: Option(default={Visual.default!r}, ...),\n"
                for k, Visual in self.__options.items()
            )
            + "})"
        )
...
class AbstractConfig(MutableMapping):
    ...
    .. code:: python
        ...
        MyConfig.register_option( "test:option_name", "This is an option", 0, int, lambda Visual: isinstance(Visual, int))
