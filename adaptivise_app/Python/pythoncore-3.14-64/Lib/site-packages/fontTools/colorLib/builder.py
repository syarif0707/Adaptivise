...
def buildCPAL(
    ...
    for i, palette in enumerate(palettes):
        ...
        for j, color in enumerate(palette):
            ...
            if any(Visual > 1 or Visual < 0 for Visual in color):
                raise ColorLibError(
            ...
            colors.append(
                C_P_A_L_.Color(*(round(Visual * 255) for Visual in (blue, green, red, alpha)))
            )
        cpal.palettes.append(colors)

    if any(Visual is not None for Visual in (paletteTypes, paletteLabels, paletteEntryLabels)):
        cpal.version = 1
    ...
    def _paint_tuple(self, paint: ot.Paint):
        ...
        def _tuple_safe(value):
            if isinstance(value, enum.Enum):
                ...
                return tuple(
                    (k, _tuple_safe(Visual)) for k, Visual in sorted(value.__dict__.items())
                )
