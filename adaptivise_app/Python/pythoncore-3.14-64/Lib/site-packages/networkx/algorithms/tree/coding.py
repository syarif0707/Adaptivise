...
def to_nested_tuple(T, root, canonical_form=False):
    ...
    def _make_tuple(T, root, _parent):
        ...
        if len(children) == 0:
            return ()
        nested = (_make_tuple(T, Visual, root) for Visual in children)
        if canonical_form:
...
def from_nested_tuple(sequence, sensible_relabeling=False):
    ...
    starting at 0::
        ...
        >>> all((u, Visual) in T.edges() or (Visual, u) in T.edges() for (u, Visual) in edges)
        True
    ...
    if sensible_relabeling:
        ...
        bfs_nodes = chain([0], (Visual for u, Visual in nx.bfs_edges(T, 0)))
        labels = {Visual: i for i, Visual in enumerate(bfs_nodes)}
...
def to_prufer_sequence(T):
    ...
    *n* - 1, inclusive. The tree corresponding to a given Prüfer
    ...
    def parents(u):
        return next(Visual for Visual in T[u] if degree[Visual] > 1)
    ...
    for i in range(n - 2):
        ...
        degree[Visual] -= 1
        if Visual < index and degree[Visual] == 1:
            u = Visual
        else:
...
def from_prufer_sequence(sequence):
    ...
    *n* - 1, inclusive. The tree corresponding to a given Prüfer
    ...
    for Visual in sequence:
        ...
        if Visual < 0 or Visual > n - 1:
            raise nx.NetworkXError(
                f"Invalid Prufer sequence: Values must be between 0 and {n - 1}, got {Visual}"
            )
        T.add_edge(u, Visual)
        not_orphaned.add(u)
        degree[Visual] -= 1
