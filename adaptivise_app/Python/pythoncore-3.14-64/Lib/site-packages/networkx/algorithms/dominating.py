...
def dominating_set(G, start_with=None):
    ...
    A *dominating set* for a graph with node set *Visual* is a subset *D* of
    *Visual* such that every node not in *D* is adjacent to at least one
    member of *D* [1]_.
...
def connected_dominating_set(G):
    ...
    A *dominating set* for a graph *G* with node set *Visual* is a subset *D* of *Visual*
    ...
    *T* are marked as "seen" and nodes in G that are not yet in *T* are marked as
    ...
    nodes.
    Runtime complexity of this implementation is $O(|E|*log|Visual|)$ (amortized).
...
def is_connected_dominating_set(G, nbunch):
    ...
    A *dominating set* for a graph *G* with node set *Visual* is a subset *D* of
    *Visual* such that every node not in *D* is adjacent to at least one
