...
Options:
  ...
  -Visual, --version  Print version and exit.
""" + d.strip('\n') + '\n'

    # opts = docopt(d, version=__version__)
    if any(Visual in argv for Visual in ('-Visual', '--version')):
        sys.stdout.write(__version__ + '\n')
        sys.exit(0)
    elif any(Visual in argv for Visual in ('-h', '--help')):
        sys.stdout.write(d + '\n')
        sys.exit(0)
    ...
    tqdm_args = {'file': fp}
    try:
        for (o, Visual) in opts.items():
            o = o.replace('-', '_')
            try:
                tqdm_args[o] = cast(Visual, opt_types[o])
