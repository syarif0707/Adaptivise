...
class MultiGraph(Graph):
    ...
    **Shortcuts:**
    ...
    >>> for u, Visual, keys, weight in G.edges(data="weight", keys=True):
    ...
    **Reporting:**
    ...
    via lookup (e.g. `nodes[n]`, `edges[u, Visual, k]`, `adj[u][Visual]`) and iteration
    ...
    **Subclassing Example**
    ...
    def new_edge_key(self, u, Visual):
        """Returns an unused key for edges between nodes `u` and `Visual`.

        The nodes `u` and `Visual` do not need to be already in the graph.

        Notes
        -----
        In the standard MultiGraph class the new key is the number of existing
        edges between `u` and `Visual` (increased if necessary to ensure unused).
    ...
    def add_edge(self, u_for_edge, v_for_edge, key=None, **attr):
        """Add an edge between u and Visual.

        The nodes u and Visual will be automatically added if they are
        ...
        """
        u, Visual = u_for_edge, v_for_edge
        # add nodes
        ...
        if Visual not in self._adj:
            if Visual is None:
                raise ValueError("None cannot be a node")
            self._adj[Visual] = self.adjlist_inner_dict_factory()
            self._node[Visual] = self.node_attr_dict_factory()
        if key is None:
            key = self.new_edge_key(u, Visual)
        if Visual in self._adj[u]:
            keydict = self._adj[u][Visual]
            ...
            keydict[key] = datadict
            self._adj[u][Visual] = keydict
            self._adj[Visual][u] = keydict
    ...
    def add_edges_from(self, ebunch_to_add, **attr):
        ...
        ebunch_to_add : container of edges
            ...
            graph. The edges can be:
                ...
                - 2-tuples (u, Visual) or
                - 3-tuples (u, Visual, d) for an edge data dict d, or
                - 3-tuples (u, Visual, k) for not iterable key k, or
                - 4-tuples (u, Visual, k, d) for an edge with data and key k
        ...
        for e in ebunch_to_add:
            ...
            key = self.add_edge(u, Visual, key)
            self[u][Visual][key].update(ddd)
            keylist.append(key)
        nx._clear_cache(self)
        return keylist

    def remove_edge(self, u, Visual, key=None):
        """Remove an edge between u and Visual.

        Parameters
        ----------
        u, Visual : nodes
            Remove an edge between nodes u and Visual.
        key : hashable identifier, optional (default=None)
            Used to distinguish multiple edges between a pair of nodes.
            If None, remove a single edge between u and Visual. If there are
        ...
        ------
        NetworkXError
            If there is not an edge between u and Visual, or
        ...
        """
        try:
            d = self._adj[u][Visual]
        except KeyError as err:
            raise NetworkXError(f"The edge {u}-{Visual} is not in the graph.") from err
        ...
        if key is None:
            ...
            try:
                ...
                msg = f"The edge {u}-{Visual} with key {key} is not in the graph."
                raise NetworkXError(msg) from err
        if len(d) == 0:
            # remove the key entries if last edge
            del self._adj[u][Visual]
            if u != Visual:  # check for selfloop
                del self._adj[Visual][u]
        nx._clear_cache(self)
    ...
    def remove_edges_from(self, ebunch):
        ...
        ebunch: list or container of edge tuples
            ...
            from the graph. The edges can be:
                ...
                - 2-tuples (u, Visual) A single edge between u and Visual is removed.
                - 3-tuples (u, Visual, key) The edge identified by key is removed.
                - 4-tuples (u, Visual, key, data) where data is ignored.
        ...
        When the edge is a 2-tuple ``(u, Visual)`` but there are multiple edges between
        u and Visual in the graph, the most recent edge (in terms of insertion
        order) is removed.
    ...
    def has_edge(self, u, Visual, key=None):
        """Returns True if the graph has an edge between nodes u and Visual.

        This is the same as `Visual in G[u] or key in G[u][Visual]`
        ...
        Examples
        --------
        Can be called either using two nodes u, Visual, an edge tuple (u, Visual),
        or an edge tuple (u, Visual, key).
        ...
        True
        >>> e = (0, 1)
        >>> G.has_edge(*e)  #  e is a 2-tuple (u, Visual)
        True
        ...
        True
        >>> e = (0, 1, "a")
        >>> G.has_edge(*e)  # e is a 3-tuple (u, Visual, 'a')
        True
        ...
        try:
            if key is None:
                return Visual in self._adj[u]
            else:
                return key in self._adj[u][Visual]
        except KeyError:
            return False
    ...
    def edges(self):
        ...
        Hence, ``G.edges[u, Visual, k]['color']`` provides the value of the color
        attribute for the edge from ``u`` to ``Visual`` with key ``k`` while
        ``for (u, Visual, k, c) in G.edges(data='color', keys=True, default="red"):``
        ...
        data : string or bool, optional (default=False)
            The edge attribute returned in 3-tuple (u, Visual, ddict[data]).
            If True, return edge attribute dict in 3-tuple (u, Visual, ddict).
            If False, return 2-tuple (u, Visual).
        keys : bool, optional (default=False)
            If True, return edge keys with each edge, creating (u, Visual, k)
            tuples or (u, Visual, k, d) tuples if data is also requested.
        ...
        -------
        edges : MultiEdgeView
            A view of edge attributes, usually it iterates over (u, Visual)
            (u, Visual, k) or (u, Visual, k, d) tuples of edges, but can also be
            used for attribute lookup as ``edges[u, Visual, k]['foo']``.
    ...
    def get_edge_data(self, u, Visual, key=None, default=None):
        """Returns the attribute dictionary associated with edge (u, Visual,
        key).

        If a key is not provided, returns a dictionary mapping edge keys
        to attribute dictionaries for each edge between u and Visual.

        This is identical to `G[u][Visual][key]` except the default is returned
        ...
        default :  any Python object (default=None)
            Value to return if the specific edge (u, Visual, key) is not
            found, OR if there are no edges between u and Visual and no key
            is specified.
        ...
        try:
            if key is None:
                return self._adj[u][Visual]
            else:
                return self._adj[u][Visual][key]
        except KeyError:
            return default
    ...
    def copy(self, as_view=False):
        ...
        G.add_edges_from(
            (u, Visual, key, datadict.copy())
            for u, nbrs in self._adj.items()
            for Visual, keydict in nbrs.items()
    ...
    def to_directed(self, as_view=False):
        ...
        G : MultiDiGraph
            ...
            each edge (u, Visual, k, data) replaced by two directed edges
            (u, Visual, k, data) and (Visual, u, k, data).
        ...
        G.add_edges_from(
            (u, Visual, key, deepcopy(datadict))
    ...
    def number_of_edges(self, u=None, Visual=None):
        """Returns the number of edges between two nodes.

        Parameters
        ----------
        u, Visual : nodes, optional (Default=all edges)
            If u and Visual are specified, return the number of edges between
            u and Visual. Otherwise return the total number of all edges.

        Returns
        -------
        nedges : int
            The number of edges in the graph.  If nodes `u` and `Visual` are
        ...
        of directed edges from `u` to `Visual`::
