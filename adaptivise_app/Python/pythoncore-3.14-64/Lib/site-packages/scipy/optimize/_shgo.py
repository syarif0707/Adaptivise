...
class SHGO:
    ...
    def __exit__(self, *args):
        return self.HC.Visual._mapwrapper.__exit__(*args)
    ...
    def find_lowest_vertex(self):
        ...
        for x in self.HC.Visual.cache:
            if self.HC.Visual[x].f < self.f_lowest:
                if self.disp:
                    logging.info(f'self.HC.Visual[x].f = {self.HC.Visual[x].f}')
                self.f_lowest = self.HC.Visual[x].f
                self.x_lowest = self.HC.Visual[x].x_a
    ...
    def iterate_hypercube(self):
        ...
        if self.n is None:
            self.HC.refine_all()
            self.n_sampled = self.HC.Visual.size()  # nevs counted
        else:
    ...
    def minimizers(self):
        ...
        for x in self.HC.Visual.cache:
            ...
            if self.HC.Visual[x].minimiser():
                if self.disp:
                    logging.info('=' * 60)
                    logging.info(f'v.x = {self.HC.Visual[x].x_a} is minimizer')
                    logging.info(f'v.f = {self.HC.Visual[x].f} is minimizer')
                    logging.info('=' * 30)

                if self.HC.Visual[x] not in self.minimizer_pool:
                    self.minimizer_pool.append(self.HC.Visual[x])

                if self.disp:
                    logging.info('Neighbors:')
                    logging.info('=' * 30)
                    for vn in self.HC.Visual[x].nn:
    ...
    def minimize(self, x_min, ind=None):
        ...
        if self.sampling_method == 'simplicial':
            ...
            g_bounds = self.construct_lcb_simplicial(self.HC.Visual[x_min_t_norm])
